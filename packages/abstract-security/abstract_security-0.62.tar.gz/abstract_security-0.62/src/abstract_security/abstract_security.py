...
from abstract_utilities import eatAll,eatOuter,SingletonMeta
def split_eq(line):
    """
...
class AbstractEnv(metaclass=SingletonMeta):
    def __init__(self,key='MY_PASSWORD',file_name=None,path=os.getcwd(),deep_scan=False,env_path=None):
        if not hasattr(self, 'initialized'):
            self.initialized=True
            self.env_path = env_path
            self.file_name = file_name or '.env'
            self.re_initialize(file_name=self.file_name,key=key,path=self.env_path,deep_scan=deep_scan)
    def re_initialize(self,key='MY_PASSWORD',file_name=None,path=None,deep_scan=False):
        """
        ...
        """
        self.file_name = file_name or self.file_name or '.env'
        self.key = key or 'MY_PASSWORD'
        self.deep_scan=deep_scan
        self.start_path = self.get_start_path(path=path)
    ...
    def get_start_path(self,file_name=None,path=None):
        path= self.env_path or path or os.getcwd()
    ...
    def find_and_read_env_file(self,key:str=None, file_name:str=None, path=None,initialize=True,deep_scan=False):
        ...
        key = key or self.key
        path= path or self.env_path or os.getcwd()
        file_name = file_name or self.file_name
        if initialize:
