...
def pull(console, model_name):
    ...
    if not model_name:
        ...
        return
    tti_model=tti_models.get(model_name,None)
    if not tti_model:
    ...
    local_dir=f"{app_dir}/models/"+tti_model["local_dir"]
    if tti_model["type"]=="huggingface":
        snapshot_download(
            repo_id=tti_model["repo_id"],
            local_dir=local_dir,
            revision=tti_model["revision"],
            )
    elif tti_model["type"]=="civitai":
        httpx_download(
            download_url=tti_model["download"],
            output_dir=local_dir
        )
    elif tti_model["type"]=="others" and model_name=="xttsv2-coqui":
        import os
