...
def create_license():
    today = datetime.now()
    one_year_later = today + timedelta(days=1)
...
class PCA_Widget_Addon:
    ...
    def __init__(self, host, url, workbook_id, worksheet_id):
        ...
        logging.debug("CREATES SNACKBAR FOR ISSUES")
        ...
        logging.debug("CHECKED LICENSE AND LOADED URLS")
        # Initiate initialization variables
        self.current_page = 'Batch'
        self.current_index = 1
        self.plots = []
        self.plot_trend = []
        logging.debug("INITIAL PAGE DEFINED")
        ...
        logging.debug("DEFINED THE DEFAULT INPUT SETTINGS AND VIS.")
        # Create base widgets
        ...
        logging.debug("CREATES CONTROL BUTTONS FOR BATCH AND PHASES")
        # Create control buttons
        logging.debug("CREATE BATCH BUTTON")
        self.Batch = self.CreateUpdatePageButton('Batch', 'seeq_primary')
        logging.debug("CREATE PHASE BUTTON")
        ...
        logging.debug("CREATE TIME MIN, TIME SEC, PCA RED. AND LAYOUT BOX")
        ...
        logging.debug("CREATE EVENT SLIDER")
        ...
        logging.debug("PACKAGE EVERYTHING FOR LAYOUT")
        ...
        if not self.expired:
            logging.debug("LAUNCH FINAL PAGE CREATION")
            ...
            logging.debug("IT'S DISPLAYING WITH A LICENSE ERROR")
    ...
    def CreateUpdatePageButton(self, page_name, color):
        logging.debug("CALLING BUTTON CREATION WITH PAGE CREATION DEPENDENCY")
    ...
    def Switch_Page(self, Page, index=0):
        logging.debug("CREATEING PAGE WITH THESE INDICATIONS:")
        ...
        if Page == 'Batch':
            ...
            logging.debug("plot contributions")
            self.Ptrain_Box.hide()
            self.Pmaster_Box.hide()
        ...
        logging.debug("RETURNING A NICE DISPLAY")
    ...
    def CleanTable(self, X):
        ...
        try:
            ...
            logging.debug("training error launched")
    ...
    def submit_formula(self):
        ...
        logging.debug("Train: ", train_signal, "-MTest: ", m_test_signal, "Test: ", test_signal)
        ...
        try:
            ...
            logging.debug("Name: ", train_signal, " --- start", train_start_date, "---end", train_end_date)
            ...
            for current_train, period in enumerate(Periods):
                logging.debug("current period retrieved: ", period[0], " - ", period[1])
            ...
            if len(Events_Master) == 0:
                logging.debug("testing error launched")
            ...
            for current_master, Master in enumerate(Master_Periods):
                logging.debug("loading: ", Master[0], " -- ", Master[1])
