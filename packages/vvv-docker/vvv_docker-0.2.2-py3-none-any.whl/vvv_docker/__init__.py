...
def install(imagename, force=False):
    ...
    if imagename == "v_chrome": 
        fpath = [get_fpath('vvv_docker_v_chrome_1'), get_fpath('vvv_docker_v_chrome_2')]
        if not fpath: return
        update_a_docker(workspace, 'cilame/v2_chrome_bak:latest', fpath, 'v2_chrome_bak.tar.xz', ziptype='xz', force=force)
        wrun_cmd, wrun_cmd_loop, wexist, wls, wdownload, wupdate, wremove = make_workspace_run_cmd(workspace)
        def write_in_server(code, filename):
            with tempfile.NamedTemporaryFile(delete=False, mode='w+', encoding='utf-8') as f:
                tfilename = f.name
