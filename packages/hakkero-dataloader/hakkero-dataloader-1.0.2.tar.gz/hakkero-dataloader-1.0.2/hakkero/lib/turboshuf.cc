...
const char TMP_NAME_TEMPLATE[] = "/turboshuftmpXXXXXX";
...
ll shufFlushBuf(char *buf, std::vector<ll> &shufIndexes, std::mt19937_64 &rng, char sep, FILE *f) {
    ...
    for (auto line : shufIndexes) {
        ...
        if (fwrite(buf + line, sizeof(char), bytesToWrite, f) != size_t(bytesToWrite)) {
...
int main() {
    ...
    std::cerr << "SKIP: " << skipLines << std::endl;
    ...
    while (!reachedEof) {
        ...
        fprintf(stderr, "\rlines read: %zu, gb read: %zu", totalLinesRead, totalBytesRead / (1024 * 1024 * 1024));
    ...
    while (linesRemaining) {
        ...
        if (!linesRemaining || totalBytesWrittenForProgress >= bufBytes) {
            fprintf(stderr, "\rlines written: %zu, gb written: %zu", totalLinesRead - linesRemaining, totalBytesWritten / (1024 * 1024 * 1024));
