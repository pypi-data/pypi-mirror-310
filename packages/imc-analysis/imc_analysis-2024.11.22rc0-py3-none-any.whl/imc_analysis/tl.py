...
def spatial_distance (adata,
    ...
    def spatial_distance_internal (adata_subset,x_coordinate,y_coordinate,z_coordinate,
                                   celltype,subset,roi_key,label):

        if verbose:
            print("Processing Image: " + str(adata_subset.obs[roi_key].unique()[0]))
        # Create a dataFrame with the necessary inforamtion
        data = pd.DataFrame({'x': adata_subset.obs[x_coordinate], 'y': adata_subset.obs[y_coordinate], 'celltype': adata_subset.obs[celltype]})

        # Function to identify shortest distance for each celltype of interest
        def distance (pheno):
            pheno_interest = data[data['celltype'] == pheno]
        ...
        # Run in parallel for all celltypes
        celltype_list = list(data['celltype'].unique())
        # print(celltype_list)
        # Apply function
        final_dist = Parallel(n_jobs=-1)(delayed(distance)(pheno=i) for i in celltype_list)     
        final_dist = pd.DataFrame(final_dist, index = celltype_list, columns = data.index).T

        return final_dist

    # subset a particular subset of cells if the user wants else break the adata into list of anndata objects
    adata_list = [adata[adata.obs[roi_key] == i] for i in adata.obs[roi_key].unique()]
    ...
    r_spatial_distance_internal = lambda x: spatial_distance_internal (adata_subset=x,
                                                                       ...
                                                                       celltype=celltype,subset=subset,roi_key=roi_key,label=label) 
