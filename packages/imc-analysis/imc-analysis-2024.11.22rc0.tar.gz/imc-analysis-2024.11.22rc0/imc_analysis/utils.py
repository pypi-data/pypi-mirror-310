...
def adata_to_dist_df(
    ...
    roi_key: str = 'roi',
    celltype_of_interest: str = '', # celltype of interest. if not in celltype, ignore
    ):

    diatance_map = adata.uns['spatial_distance'].copy()
    pheno_df = pd.DataFrame(
        {'imageid': adata.obs[roi_key], 'celltype': adata.obs[celltype]}
    )  # image id and celltype
    data = pd.merge(
        pheno_df, diatance_map, how='outer', left_index=True, right_index=True
    )  # merge with the distance map

    if celltype_of_interest in data['celltype'].cat.categories:
        data = data[data['celltype'] == celltype_of_interest]  # subset the pheno of interest
        print(f'Calculating distance density for {celltype_of_interest}')
    else:
        print('Calculating distance density for all cells')

    data = data.drop(
        ['celltype', 'imageid'], axis=1
    )  # drop the celltype column before stacking
    ...
    )  # bring back the imageid and celltype

    # Convert columns to str
    for col in ['imageid', 'group', 'celltype']:
