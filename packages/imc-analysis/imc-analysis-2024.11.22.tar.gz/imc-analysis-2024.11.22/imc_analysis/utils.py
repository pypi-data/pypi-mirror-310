...
def adata_to_dist_df(
    ...
    roi_key: str = 'roi',
    celltype: str = '', # celltype of interest. if not in celltype, ignore
    ):

    diatance_map = adata.uns['spatial_distance'].copy()
    pheno_df = pd.DataFrame(
        {'imageid': adata.obs[roi_key], 'celltype': adata.obs[celltype_name]}
    )  # image id and celltype
    data = pd.merge(
        pheno_df, diatance_map, how='outer', left_index=True, right_index=True
    )  # merge with the distance map

    if celltype in data['celltype'].cat.categories:
        data = data[data['celltype'] == celltype]  # subset the pheno of interest
