...
"""
    ...
    def __init__(self, document: dict,
        ...
        self.schema_def_prefix = (self.schema_def_prefix.rstrip('.') + '.') if self.schema_def_prefix else ''
        self.response_def_prefix = (self.response_def_prefix.rstrip('.') + '.') if self.response_def_prefix else ''
...
"""
    ...
    def generate_paths(self):
        ...
        for path, methods in self.openapi.paths.items():
            ...
            for method, operation in methods.items():
                if str(method).lower() in HTTP_METHODS_LOWER:
                    operations.append(
                        tab_for(self.generate_path_item(
                            method=method,
                            path=path,
                            operation=operation,
                            summary=summary,
        ...
        if self.openapi.info:
            client_lines.append(tab_for(f'__info__ = {self.represent_data(self.openapi.info)}', tabs=1))
        if self.openapi.servers:
            client_lines.append(tab_for(f'__servers__ = {self.represent_data(self.openapi.servers)}', tabs=1))
    ...
    def get_schema_annotations(self, json_schema: dict,
        ...
        schema_contents = [
            ...
            for arg in args if self.required_generate(arg)
        ]
    ...
    def get_parameter(self, param: dict, excludes: list = None) -> Optional[inspect.Parameter]:
    ...
    def get_body_parameters(self, body: dict,
                            endpoint_name: str = None,
                            excludes: List[str] = None) -> List[inspect.Parameter]:
        ...
        for content_type, content in body_content.items():
            ...
            schema_name = ''.join([v.capitalize() for v in endpoint_name.split('_')] + [param_name.capitalize()]) \
                if endpoint_name else param_name.capitalize()
    ...
    def get_responses_annotation(self, responses: dict, endpoint_name: str = None):
        ...
        for name, resp in sorted(responses.items(), key=lambda x: x[0]):
            ...
            if ref:
                ...
                resp_name = ''.join([v.capitalize() for v in endpoint_name.split('_')] + ['Response']) \
                    if endpoint_name else 'Response'
                resp_def_name = self.register_response_ref(
                    name=resp_name, schema=resp
                )
    ...
    def get_operation_function(self, operation: dict, path_parameters: list = None):
        ...
        func_parameters = [inspect.Parameter(
            name='self',
    ...
    def generate_path_item(self,
        ...
        decorator = f'@api.{method}({repr(path)}%s)' % ((', ' + api_kwargs_str) if api_kwargs else '')
