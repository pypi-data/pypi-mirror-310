...
_backgroundplotkwargs = {
    "linewidth": _normalizedplotkwargs["linewidth"] * 4,
    "linestyle": "-",
    "alpha": 0.45,
}
...
def _plot_accumulated_diff_on_axis(
    ...
    ax.set_ylabel(period + " " + ordinate.name + " diff")
    if prettify:
        ax.grid(True)
        ax.tick_params(axis="x", labelrotation=45)
        ...
        if prettify and not single:
            if i == 0:
                try:
                    ...
                    bglabel = data["Title"].at[0]
                except KeyError:
                    print(
                        "You're trying to access a work title while plotting total user stats???"
                    )
                    bglabel = "ERROR IN LABEL GENERATION"
...
def plot_total_stats(tsfiles: list, conf: Config):
    ...
    _plot_all_normalized_on_axis(ax8, alldata, ax9, prettify, single=True)
    ...
    for i, ID in enumerate(IDlist):
        ...
        if diff:
            ...
            _plot_accumulated_diff_on_axis(
                ax1twin, time, words, title, prettify=prettify, index=i
            )
            _plot_accumulated_diff_on_axis(
                ax2twin, time, hits, title, prettify=prettify, index=i
            )
            _plot_accumulated_diff_on_axis(
                ax3twin, time, kudos, title, prettify=prettify, index=i
            )
            _plot_accumulated_diff_on_axis(
                ax4twin, time, comments, title, prettify=prettify, index=i
            )
            _plot_accumulated_diff_on_axis(
                ax5twin, time, bookmarks, title, prettify=prettify, index=i
            )
            _plot_accumulated_diff_on_axis(
                ax6twin, time, subscriptions, title, prettify=prettify, index=i
            )

        _plot_all_normalized_on_axis(
            ax21, alldata, ax22, prettify, index=i, single=single
        )
