...
def export_to_csv(self, csv_file='phone_data.csv'):
    with open(csv_file, 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['Phone', 'Province', 'City', 'Zip Code', 'Area Code', 'Phone Type'])

        for i in range(self.phone_record_count):
            current_offset = (self.first_phone_record_offset + i * self.phone_fmt_length)
            ...
            phone_num = str(cur_phone).zfill(7)  # Ensure phone number is 7 digits

            record_content = get_record_content(self.buf, record_offset)
            formatted_content = self._format_phone_content(phone_num, record_content, phone_type)

            writer.writerow([
                formatted_content['phone'],
                formatted_content['province'],
                formatted_content['city'],
                formatted_content['zip_code'],
                formatted_content['area_code'],
                formatted_content['phone_type']
            ])
...
if __name__ == "__main__":
    phone = Phone()
    # 在__init__中添加新方法
    Phone.export_to_csv = export_to_csv
    # print(phone.find(1529900))
    phone.export_to_csv()
