...
class Auth:
    ...
    async def check_user(self, request):
        ...
        if user is None:
            log_and_raise_service(
                ...
                label="AUTH",
                label2=label2,
                msg=f"USER_NOT_REGISTERED: {email}",
                status_code=401,
            )
        if user["ustatus"] == ACTIVE:
    ...
    async def get_users(self, request):
        ...
        if role == "admin":
            filters.append({"col": "org", "val": org})
        kwargs["filters"] = filters
        # res = await self.db.select('users', **kwargs)
        res = await self.db.select(
            "users", lookups=[{"col": "org", "tbl": "orgs"}], **kwargs
        )
        res_to_recs(res, ["org"])
        if res is None or not "data" in res or not "nrows" in res:
            log_and_raise_service(
                self.LOG, label="AUTH", label2=label2, msg="DB access error"
            )
