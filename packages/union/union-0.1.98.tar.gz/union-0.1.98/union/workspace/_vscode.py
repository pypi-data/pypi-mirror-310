...
class WorkspaceConfig(DataClassYAMLMixin, DataClassJSONMixin):
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.container_image, str) or os.getenv("UNION_DEV"):
            ...
            self.container_image = ImageSpecConfig(
                name=f"workspace-{_user_image_spec.name}",
...
@vscode
def workspace(config_content: str, host: str, state_dir: Optional[FlyteDirectory] = None) -> WorkspaceOutput:
    ...
    start_vscode_service(workspace_dir, workspace_config.ttl_seconds, workspace_config.working_dir)
    ...
    config = {
        ...
        "commands.commands": {
            "⏹️ Terminate Workspace": {
                "sequence": [
                    {
                        ...
                        "args": {
                            "text": "python /home/union/workspace/terminate_workspace.py",
                            "name": "terminate",
                            "waitForExit": False,
                            "reuse": "newest",
                        },
                ...
                "statusBar": {"alignment": "left", "text": "$(stop) Terminate Workspace", "priority": -9999},
            }
        },
    }
...
def prepare_terminate_workspace_python(workspace_root: Path, pid: int):
    """
    ...
    if __name__ == "__main__":
        ...
        if answer in ['Y', '']:
            ...
            time.sleep(3)
            webbrowser.open("https://serverless.union.ai/", new=0)
...
def start_vscode_service(workspace_root: Path, ttl_seconds: int, working_dir: Optional[str] = None):
    ...
    prepare_terminate_workspace_python(workspace_root, child_process.pid)
