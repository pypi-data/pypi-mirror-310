...
def load_rooster_instance (instance='rooster_default', filename=None,
                           on_the_fly=True, verbose=False, seed=None) :
  '''
  ...
  on_the_fly : bool
    ...
    a default dataset. Optional, default ``True``.
  ...
  if filename is None :
    if not on_the_fly :
      warnings.warn ("Using ROOSTER default instance is deprecated and will be removed in star-privateer>=1.2.", DeprecationWarning)
      filename = sp.internal_path (sp.rooster_instances, 
                                   instance)
      with filename as f :
        if verbose :
