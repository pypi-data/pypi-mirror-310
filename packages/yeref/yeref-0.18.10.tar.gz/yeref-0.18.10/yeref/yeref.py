        ...
        for item in data:
            try:
                ...
                if sign_ == "+":
                    ...
                    dt_cur = dt_now - timedelta(hours=int(h_), minutes=int(m_))
                timedelta_ = (dt_cur - datetime.strptime(OFFER_DT, "%d-%m-%Y %H:%M"))
...
async def get_list_of_send_folder(CONF_P, EXTRA_D):
    ...
    for k, v in file_list_dic.items():
        try:
            ...
            if parent_folder == '' and datetime_ < datetime.strptime(name_folder, "%d-%m-%Y %H:%M"):
        ...
        for item in data:
            try:
                ...
                if USER_ISPAID == 1 and USER_DTPAID and (
                        dt_ - datetime.strptime(USER_DTPAID, '%d-%m-%Y_%H-%M-%S')).days > 31:
                    await asyncio.sleep(round(random.uniform(0, 1), 2))
...
async def check_sub_pay(chat_id, lz):
    is_paid = False
    till_paid = ''
    try:
        USER_ISPAID = 0
...
async def del_extra_files(UNKNOWN_ERRORS_TXT, EXTRA_D):
    try:
        ...
        for item in arr:
            ...
            cur_dt = datetime.strptime(f"{parts_dt[0]}_{parts_dt[1]}", '%Y-%m-%d_%H-%M-%S')
