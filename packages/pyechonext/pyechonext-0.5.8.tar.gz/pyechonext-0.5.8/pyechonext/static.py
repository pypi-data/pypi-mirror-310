...
class StaticFile:
	...
	def __init__(self, settings: Settings, filename: str):
		...
		self.abs_filename = Path(os.path.join(self.settings.BASE_DIR, self.settings.STATIC_DIR, filename))

		if not self.abs_filename.exists():
			raise StaticFileNotFoundError(f'Static file "{self.abs_filename}" not found.')
