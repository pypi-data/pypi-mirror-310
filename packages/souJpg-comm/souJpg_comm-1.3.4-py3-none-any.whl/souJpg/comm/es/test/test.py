...
from souJpg.comm.es.es import EsBasedOps


class EsClientTestCase(unittest.TestCase):
    def __init__(self, methodName="runTest"):
        super().__init__(methodName=methodName)

        self.esClient = EsBasedOps()
    ...
    def reCreateIndexAndPopulate(self, indexName=None):
        if self.esClient.esClient.indices.exists(indexName):
            self.esClient.esClient.indices.delete(index=indexName)
        # index settings
        settings = self.indexMappingMap.get(indexName)
        # create index
        self.esClient.esClient.indices.create(index=indexName, body=settings)
        op_type = "index"
        docs = []
        i = 1
        action = {}
    ...
    def test_getAllIndexNames(self):
        indexNames = self.esClient.getAllIndexNames()
        logger1.info(indexNames)
    ...
    def test_deleteImage(self):
        indexName = "images_all_dev_9"
        source = "userUpload"
        imageId = "7zme2pDnmscuZLHHPNQhu8"
        self.esClient.deleteImage(indexName=indexName, source=source, imageId=imageId)
