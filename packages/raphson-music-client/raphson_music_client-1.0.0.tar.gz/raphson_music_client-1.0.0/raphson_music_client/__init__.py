...
class RaphsonMusicClient:
    player_id: str
    session: ClientSession  # pyright: ignore[reportUninitializedInstanceVariable]
    ...
    def _track_from_json(self, json: Any) -> Track:
        assert self.session
        return Track(
            self.session,
            json["path"],
            ...
            json["album_artist"],
            json["year"],
            json["artists"] if json["artists"] else [],
        )

    async def choose_track(self, playlist: str) -> Track:
