...
""",
    ...
    def create_structure(base_path, structure):
        total_files = sum([len(v) if isinstance(v, dict) else 1 for v in structure.values()])
        current_file = 0
        
        # Spinner and message output
        with click.spinner('Creating project files...') as spinner:
            for name, content in structure.items():
                ...
                if isinstance(content, dict):
                    ...
                    current_file += 1
                    spinner.update()
                    create_structure(path, content)  # Recursive call
                else:
