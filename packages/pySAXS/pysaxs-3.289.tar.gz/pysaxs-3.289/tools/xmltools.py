...
def string2data(st):
    ...
    # print("--------------------------------")
    # print(st)
    # if re.search(r"np\.float64\(", chaine):
    if "np.float64(" in st:
        # Oh ! we have a problem
        # Extraction des nombres a l'interieur des np.float64
        valeurs_str = re.findall(r"np\.float64\((.*?)\)", st)

        # Conversion en float et arrondi à 2 décimales
        valeurs_float = [float(val) for val in valeurs_str]
        arr = array(valeurs_float)
        # print(arr)
    else:
        # it is normal
        #print("NORMAL")
