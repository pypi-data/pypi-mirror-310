...
def Analyze_1D(q,I,E,Bounds=[],Nsph=[],Maxiter=[],Rpfactor=1.5/3,Nreps=100,qlims=[0,inf],\
    ...
    A['q'] = q
    A['Niter'] = numpy.mean(Niters) #average number of iterations for all repetitions
    A['Conval']=ConVal  #modified by OT 29/04/2024
    return True,A  #modified by OT 30/10/2019

######################################## end ###############################################

def easyMcSas(q, i, error, qmin=None, qmax=None, nbSphere=500, nbReps=3, histogramBin=50, minDiam=1, maxDiam=100, \
              convergenceValue=1, histogramScale='lin',silent=True):
    '''
    easy mc sas algorythm call
    ----------------------------
    q, i, error : data to process
    qmin, qmax : qrange in A-1
    nbSphere : nb of sphere to use
    nbReps : nb of iteration
    histogramBin : nb of data in the final histogram
    minDiam, maxDiam : sphere diameters
    convergenceVale : important to optimize
    ...
    q = q[numpy.nonzero(i)]
    i = i[numpy.nonzero(i)]
    error = error[numpy.nonzero(i)]

    # qmin index
    qminIndex = numpy.where((q >= qmin))[0][0]
    # qmax index
    qmaxIndex = numpy.where((q < qmax))[0][-1]

    # cut the datas
    mcQ = q[qminIndex:qmaxIndex] * 10  # A-1 ->nm
    mcI = i[qminIndex:qmaxIndex]
    mcE = error[qminIndex:qmaxIndex]

    test, resA = Analyze_1D(mcQ, mcI, numpy.maximum(0.01 * mcI, mcE), Nsph=nbSphere, Convcrit=convergenceValue, \
                                    Bounds=numpy.array([minDiam / 2, maxDiam / 2]), \
                                    Rpfactor=1.5 / 3, Maxiter=1e4, Histscale=histogramScale, drhosqr=1, Nreps=nbReps,
                                    Histbins=histogramBin,silent=silent)
    if test:
        resA['test'] = test
        s = resA['Hx'][0:-1] * 2  # diameter
        V = resA['Hmean'] / sum(resA['Hmean'])
        # Volume from radius
        ...
        resA['histo_mean'] = mean
        resA['histo_median'] = median
        resA['histo_maxi'] = maxi
        resA['histo_average'] = average
        resA['variance'] = variance
        resA['diameter'] = resA['Hx'][0:-1] * 2  # diameter
        resA['width_diameter'] = resA['Hwidth'] * 2
        resA['volume_weight'] = resA['Hmean'] / sum(resA['Hmean'])  # histogram volumeweight
        resA['volume_weight_error'] = resA['Hstd'] / sum(resA['Hmean'])
    else:
        resA = {'test': test, 'convergenceValue': resA}
    return resA
