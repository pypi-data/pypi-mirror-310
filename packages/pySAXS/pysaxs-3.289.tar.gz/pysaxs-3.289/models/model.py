...
class Model:
    ...
    def printArg(self):
        ...
        '''
        for i in range(len(self.Arg)):
            print(self.Doc[i] + " : " + self.Format[i] % self.Arg[i])
    ...
    def easyFit(self,q, i, error, arg=None, fitexp=2, qmin=None, qmax=None,istofit=None):
        '''
        simple function to fit with qmin qmax
        return the calculated parameters, and set the intensities, q, arg in the model
        '''
        if istofit is not None:
            self.istofit=istofit
        qminIndex = 0
        qmaxIndex = q[-1]
        if qmin is not None:
        ...
        # cut the datas
        newq = q[qminIndex:qmaxIndex]
        newi = i[qminIndex:qmaxIndex]
        newerror = error[qminIndex:qmaxIndex]
        if arg is not None:
            self.Arg = arg
        self.q = newq
        res, resLMFIT, uncertainties = self.fitLMFIT(newi, fitexp, err=newerror, bounds=None)
        self.Arg = res
        self.i = self.getIntensity()
        return res
