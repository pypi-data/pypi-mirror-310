...
class Model(BaseModel):
    ...
    def objective_sense(self, sense: CplexEnum | str) -> None:
    ...
    def continuous_var(self, lb: float = 0, ub: float = float("Inf"), name: str = "", coeff: float = 0) -> Variable:
        ...
        """
        return self.add_variable(lb, ub, name, coeff, CplexEnum.CONTINUOUS)
    ...
    def integer_var(self, lb: float = 0, ub: float = float("Inf"), name: str = "", coeff: float = 0) -> Variable:
        ...
        Raises:
            ModelError: If the variable addition fails.
        """
        return self.add_variable(lb, ub, name, coeff, CplexEnum.INTEGER)
    ...
    def binary_var(self, name: str = "", coeff: float = 0) -> Variable:
        ...
        Raises:
            ModelError: If the variable addition fails.
        """
        return self.add_variable(0, 1, name, coeff, CplexEnum.BINARY)
    ...
    def binary_var_dict(
        ...
        for _, key in enumerate(keys):
            ...
            binary_vars[var_name] = self.binary_var(var_name)
        return binary_vars
    ...
    def continuous_var_dict(
        ...
        for i, key in enumerate(keys):
            var_name = f"{name}{key_format % key}"
            continuous_vars[var_name] = self.continuous_var(lb[i], ub[i], var_name)
    ...
    def integer_var_dict(
        ...
        for i, key in enumerate(keys):
            var_name = f"{name}{key_format % key}"
            integer_vars[var_name] = self.integer_var(lb[i], ub[i], var_name)
        return integer_vars
    ...
    def minimize(self, expr: Expression) -> CplexEnum:
        ...
        Returns:
            HybridSolverStatus: The solution status after optimization.
        """
        self.set_objective(HybridSolverOptSenses.MINIMIZE, expr)
        return self.optimize()
    ...
    def maximize(self, expr: Expression) -> CplexEnum:
        ...
        Returns:
            HybridSolverStatus: The solution status after optimization.
        """
        self.set_objective(HybridSolverOptSenses.MAXIMIZE, expr)
        return self.optimize()
