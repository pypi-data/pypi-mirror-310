...
class Model(BaseModel):
    ...
    def __init__(
        self,
        name: str = "Model",
        sense: HybridSolverOptSenses | PythonMipEnum | None = None,
        ...
        if sense is not None:
            if sense not in [
                ...
                HybridSolverOptSenses.MAXIMIZE,
                PythonMipEnum.MINIMIZE,
                PythonMipEnum.MAXIMIZE,
            ]:
                error_message = "Unsupported sense."
                raise ModelError(error_message)
            self.set_sense(sense)
        if solver_name is not None:
            msg = "The solver to use is the HybridSolver."
            warnings.warn(msg, stacklevel=2)
        self._type = "PythonMipModel"
    ...
    def sense(self) -> PythonMipEnum:
        ...
        """
        return hybridsolver_sense_to_python_mip[self.sense]
    ...
    @objective.setter
    def objective(self, obj_info: Expression | tuple[Expression, HybridSolverOptSenses | PythonMipEnum]) -> None:
        """Sets the objective function of the model.

        Args:
            obj_info (Expression | Tuple[Expression, HybridSolverOptSenses]): The objective function to be set.
        """
    ...
    def add_var(
        ...
        if column is not None:
            ...
            warnings.warn(msg, stacklevel=2)
        return self.add_variable(lb, ub, name, obj, var_type)
