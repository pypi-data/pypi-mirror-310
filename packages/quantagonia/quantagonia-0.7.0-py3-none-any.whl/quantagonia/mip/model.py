...
from quantagonia.mip.wrappers.wrapper_enums import GRB, CplexEnum, PythonMipEnum
...
class Model:
    ...
    @sense.setter
    def sense(self, sense: HybridSolverOptSenses | GRB | CplexEnum | PythonMipEnum | str) -> None:
    ...
    @property
    def solve_status(self) -> HybridSolverStatus:
        """The solution status of the model.
    ...
    @property
    def status(self) -> HybridSolverStatus:
        """The solution status of the model.
    ...
    def n_cont_vars(self) -> int:
        ...
        """
        return self.n_variables - self.n_bin_vars - self.n_int_vars
    ...
    def add_variable(
        ...
        name: str = "",
        coeff: float = 0,
        var_type: VarType | GRB | CplexEnum | PythonMipEnum = VarType.CONTINUOUS,
    ) -> Variable:
        ...
        """
        if var_type in [VarType.CONTINUOUS, GRB.CONTINUOUS, CplexEnum.CONTINUOUS, PythonMipEnum.CONTINUOUS]:
            vtype = "C"
        elif var_type in [VarType.BINARY, GRB.BINARY, CplexEnum.BINARY, PythonMipEnum.BINARY]:
            vtype = "B"
        elif var_type in [VarType.INTEGER, GRB.INTEGER, CplexEnum.INTEGER, PythonMipEnum.INTEGER]:
            vtype = "I"
        else:
    ...
    def set_objective(
        self,
        sense: HybridSolverOptSenses | GRB | CplexEnum | PythonMipEnum | str = HybridSolverOptSenses.MINIMIZE,
        ...
        if expr is None:
            ...
            raise ModelError(error_message)
        if sense in [HybridSolverOptSenses.MINIMIZE, GRB.MINIMIZE, CplexEnum.Minimize, PythonMipEnum.MINIMIZE, "min"]:
            ref_sense = "minimize"
        elif sense in [HybridSolverOptSenses.MAXIMIZE, GRB.MAXIMIZE, CplexEnum.Maximize, PythonMipEnum.MAXIMIZE, "max"]:
            ref_sense = "maximize"
        else:
            error_message = "Unsupported sense."
            raise ModelError(error_message)
    ...
    def set_sense(self, sense: HybridSolverOptSenses | GRB | CplexEnum | PythonMipEnum | str) -> None:
        """Sets the optimization sense of the model.
