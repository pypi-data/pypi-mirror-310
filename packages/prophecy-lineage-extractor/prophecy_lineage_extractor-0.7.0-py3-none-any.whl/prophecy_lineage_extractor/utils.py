...
def append_to_csv(df, csv_path):
    ...
    if df.empty:
        logging.warning("Received an empty DataFrame, nothing to write.")
        return
...
def convert_csv_to_excel(csv_path: Path, output_path: Path):
    ...
    def _concatenate_transformations(series):
        return "\n\n".join(series)  # Concatenate with ' | ' as the delimiter

    df = df.groupby(["database", "table", "column_name"], as_index=False).agg(
        {"upstream_transformation": _concatenate_transformations}
    )
    ...
    column_widths = {
        "A": 30,  # Width for 'database'
        "B": 50,  # Width for 'table'
        "C": 30,  # Width for 'column_name'
        "D": 80,  # Width for 'upstream_transformation'
    }
