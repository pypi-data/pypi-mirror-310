...
def selectComponent(componentType):
    ...
    while True:
        ...
        if 1 <= choice <= pageSize and (startIdx + choice - 1) < totalParts:
            ...
            if not isCompatible:
                displayIncompatibilityMessages(comp.messages, componentType, userPC)
...
def displayIncompatibilityMessages(messages, componentType, userPC):
    """Display any incompatibility messages."""
    # Filter out empty messages
    filteredMessages = {key: value for key, value in messages.items() if value}
    ...
    if filteredMessages:
        ...
        if suggestAlternatives.lower() == "y":
            suggestComponent(userPC, componentType, messages)


def suggestComponent(userPC, componentType, messages):
    clearScreen()
        ...
        while True:
            ...
            if not suggestedParts:
                ...
                if componentType not in differentMessage:
                    ...
                    for component, componentMessages in messages.items():
            ...
            if componentType == "ssd" and been != 1:
                ssdType = (
                    "M.2"
                    if any(
                        "M.2" in interface
