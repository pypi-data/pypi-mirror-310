def suggestCompatibleCases(userBuild, caseComp):
    ...
    useCasePercentages = {
        "work": 0.05,  # 5% of budget
        "gaming": 0.05,  # 5% of budget
    }
    casePercentage = useCasePercentages.get(userBuild.useCase, 0.05)

    # Step 3: Calculate the case budget (directly, since no case exists in userBuild)
    caseBudget = budget * casePercentage

    # Step 4: Filter compatible cases
    compatibleCases = getCompatibleCases(allCases, userBuild)

    # Step 5: Handle out-of-budget or insufficient case budget scenarios
    if userBuild.totalPrice > budget or caseBudget == 0:
