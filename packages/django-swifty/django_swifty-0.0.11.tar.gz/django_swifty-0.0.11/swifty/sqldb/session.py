...
def get_session() -> Session:
    ...
    session.configure(bind=create_engine(db_url=settings.SWIFTY_DB_URL, echo=True))  # type: ignore
    return session()
...
class SwiftyDBSessionMixin(SwiftyLoggerMixin):
    ...
    db_url: str = settings.SWIFTY_DB_URL
    cache_pattern: Dict[str, Union[str, int, List[str]]] = (
        settings.SWIFTY_DB_CACHE_PATTERN or DB_CACHE_PATTERN
    )
