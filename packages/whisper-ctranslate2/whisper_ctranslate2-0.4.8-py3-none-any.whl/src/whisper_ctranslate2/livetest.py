...
class Live:
    ...
    def _is_there_voice2(self, indata, frames):
    ...
    def read_wav_file(self, filepath):
        data, samplerate = sf.read(filepath)
        if samplerate != SampleRate:
            raise ValueError(f"Expected sample rate {SampleRate}, but got {samplerate}")
        return data

    def process_wav_file(self, filepath):
        data = self.read_wav_file(filepath)
        frames_per_block = int(SampleRate * BlockSize / 1000)
        for start in range(0, len(data), frames_per_block):
            block = data[start : start + frames_per_block]
            if len(block) < frames_per_block:
                block = np.pad(
                    block, ((0, frames_per_block - len(block)), (0, 0)), "constant"
                )
            self.callback(block, frames_per_block, None, None)
            self.process()
        self.running = False
        self.process()  # Process remaining buffers

    # Example usage


if __name__ == "__main__":
    #    options = TranscriptionOptions()  # Define your transcription options
    live_transcriber = Live(
    ...
    live_transcriber.process_wav_file("dosparlants.wav")
