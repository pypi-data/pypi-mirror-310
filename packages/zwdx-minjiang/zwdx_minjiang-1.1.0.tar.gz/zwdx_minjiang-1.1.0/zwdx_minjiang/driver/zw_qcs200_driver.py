...
class fpgadev(object):
    ...
    def __init__(self):
        ...
        self.offset = []
        self.offset.append(930)
        self.offset.append(1020)
        self.offset.append(1023)
        self.offset.append(1023)
        self.offset.append(1030)
        self.offset.append(1014)
        ...
        for i in range(8):
            self.dac_offset(i+14, self.offset[i])
...
#                        self.ad_ch.append(['slot'+str(BoardSlot[i]),ch_i,'8ad8da'])
                    if BoardSlot[i] == 1:
                        for ch_i in range(6):
                            self.da_ch[ch_i+6] = ['slot'+str(BoardSlot[i]),ch_i,'8ad8da']
                        for ch_i in range(2):
                            self.ad_ch[ch_i+2] = ['slot'+str(BoardSlot[i]),ch_i,'8ad8da']
                    else:
                        for ch_i in range(6):
                            self.da_ch[ch_i] = ['slot'+str(BoardSlot[i]),ch_i,'8ad8da']
                        for ch_i in range(2):
                            self.ad_ch[ch_i] = ['slot'+str(BoardSlot[i]),ch_i,'8ad8da']
...
#                        self.ad_ch.append(['slot'+str(BoardSlot[i]),ch_i,'8ad8da'])
                    if BoardSlot[i] == 2:
                        self.da_ch[14] = ['slot'+str(BoardSlot[i]),6,'8da']
                        self.da_ch[15] = ['slot'+str(BoardSlot[i]),4,'8da']
                        self.da_ch[16] = ['slot'+str(BoardSlot[i]),2,'8da']
                        self.da_ch[17] = ['slot'+str(BoardSlot[i]),0,'8da']
                        self.da_ch[18] = ['slot'+str(BoardSlot[i]),3,'8da']
                        self.da_ch[19] = ['slot'+str(BoardSlot[i]),1,'8da']
                        self.da_ch[20] = ['slot'+str(BoardSlot[i]),5,'8da']
                        self.da_ch[21] = ['slot'+str(BoardSlot[i]),7,'8da']
                    elif BoardSlot[i] == 1:
                        for ch_i in range(8):
                            self.da_ch[ch_i+6] = ['slot'+str(BoardSlot[i]),ch_i,'8ad8da']
                    else:
...
#        global sampling_g
    ...
    def rd_adc_data_ctrl(self,adc_chennel_num,trigger_delay,times,save_len):
        ...
        self.writeReg(0x4000 + 0x400*ch_num+3,np.uint32(times),slot)
...
#        self.adc_modle_reset(slot_dac,1)
        ...
        for i in range(len(mul_f)):
            ...
            mul_start_phase.append(np.uint32((2*np.pi - mul_f[i][0])/2/np.pi*dds_phase_amp))
            mul_f_phase.append(np.uint32((adc_fs - (mul_f[i][1]%adc_fs))/adc_fs*dds_phase_amp))
            
        
        self.writeReg(0x6000 + 0x400*ch_num+0,1,slot)
        
        self.writeReg(0x6000 + 0x400*ch_num+1,np.uint32(trigger_delay),slot)
        self.writeReg(0x6000 + 0x400*ch_num+2,np.uint32(times),slot)
        ...
        for i in range(len(mul_f)):
            self.writeReg(0x6000 + 0x400*ch_num+2*i+4,mul_start_phase[i]+2**31,slot)
            self.writeReg(0x6000 + 0x400*ch_num+2*i+5,mul_f_phase[i],slot)
...
#                print(slot,ch_num)
    ...
    def rd_adc_mul_data(self,adc_chennel_num,modle_num,times):
        ...
        baseaddr = 0x500000000 + 0x8000000*ch_num + 0x400000*modle_num
...
#            adc_data_o = np.reshape(adc_data,(2,-1),order='F')
    ...
    def set_ch_offset(self,chennel_num,offset):
        if chennel_num > 21 or chennel_num < 14:
            print('非Z-PULSE通道：',chennel_num)
            return
        self.dac_offset(chennel_num,np.round(self.dc1v * offset + self.offset[chennel_num-14]))
        
        return
    
    def rfdac_SetNyquistZone(self,NyquistZone,ch):
        
        slot = self.da_ch[ch][0]
        print(slot)
        cmd_reg = 1
    ...
    def rfdac_sampling(self,sampling,ch):
