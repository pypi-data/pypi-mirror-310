...
@track_changes
def remove(
    df: Union[List, dict, tuple, np.ndarray, str],
    operation: str,
    columns: Optional[Union[str, List[str]]] = None,
    keep: Optional[str] = 'first',
    consider_all: bool = True,
    inplace: bool = False
) -> Optional[Union[List, dict, tuple, str]]:
    """
    Remove duplicates or columns from a list, dict, or other data structures based on the specified operation.

    Parameters:
    - df (Union[list, dict, tuple, np.ndarray, str]): Input data.
    - operation (str): Type of removal operation. Options are:
      - 'duplicates': Remove duplicate rows.
      - 'columns': Remove specified columns (if applicable).
    - columns (Optional[Union[str, List[str]]]): Column(s) to consider for the operation.
      - For 'duplicates': Columns to check for duplicates.
      - For 'columns': Column(s) to be removed (for dict-like structures).
    - keep (Optional[str]): Determines which duplicates to keep. Options are:
      ...
      Default is 'first'.
    - consider_all (bool): Whether to consider all columns in the data structure if duplicates are found in the specified columns.
    ...
    Returns:
    - Optional[Union[list, dict, tuple, str]]: Updated data or None if inplace=True.
    ...
    if operation == 'duplicates':
        ...
        if columns is not None:
            ...
            if isinstance(df, dict):
                df = [list(df.values())]  # Convert dict to list of values to mimic rows
            elif isinstance(df, (list, tuple)):
                df = [list(row) for row in df]  # Flatten rows into a list of lists
            missing_columns = [col for col in columns if col not in range(len(df[0]))]  # column indices
            if missing_columns:
                raise ValueError(f"Columns {missing_columns} not found in the data.")

        # Remove duplicates
        if keep == 'none':
            df_cleaned = [row for idx, row in enumerate(df) if row not in df[:idx]]
        else:
            seen = set()
            df_cleaned = []
            for row in df:
                row_tuple = tuple(row)  # Use tuple to handle rows
                if row_tuple not in seen:
                    seen.add(row_tuple)
                    df_cleaned.append(row)

    # Handle 'columns' operation (only for dict-like structures)
    elif operation == 'columns' and isinstance(df, dict):
        if isinstance(columns, str):
            columns = [columns]
        ...
        missing_columns = [col for col in columns if col not in df]
        if missing_columns:
            raise ValueError(f"Columns {missing_columns} not found in dict.")
        
        df_cleaned = {key: value for key, value in df.items() if key not in columns}
    ...
    if inplace:
        if isinstance(df, dict):
            ...
            raise ValueError("Inplace modification is not supported for string data.")
        return None
    ...
    if isinstance(df, list):
        ...
        return tuple(map(tuple, df_cleaned))
    elif isinstance(df, dict):
        return df_cleaned
    elif isinstance(df, str):
        return json.dumps(df_cleaned)
    else:
