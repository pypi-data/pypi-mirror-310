...
def load_gastric_cancer_data(
    ...
    if same_source:
        try:
            ...
            normal_file = f'{data_dir}normal_data_source1.csv'
            exp1 = pd.read_csv(normal_file, index_col=0)
    else:
        try:
            with resources.open_text('ot_grn.data', 'normal_data_source2.csv') as f:
                exp1 = pd.read_csv(f, index_col=0)
        except:
            normal_file = f'{data_dir}normal_data_source2.csv'
            exp1 = pd.read_csv(normal_file, index_col=0)        
    ...
    try:
        ...
        tumor_file = f'{data_dir}tumor_data_source1.csv'
        exp2 = pd.read_csv(tumor_file, index_col=0)
