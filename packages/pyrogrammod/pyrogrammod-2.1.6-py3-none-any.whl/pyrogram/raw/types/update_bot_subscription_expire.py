...
class UpdateBotSubscriptionExpire(TLObject):  # type: ignore
    ...
    __slots__: List[str] = ["user_id", "payload", "invoice_slug", "until_date", "qts"]

    ID = 0x2d13c6ee
    QUALNAME = "types.UpdateBotSubscriptionExpire"

    def __init__(self, *, user_id: int, payload: str, invoice_slug: str, until_date: int, qts: int) -> None:
    ...
    def read(b: BytesIO, *args: Any) -> "UpdateBotSubscriptionExpire":
        ...
        invoice_slug = String.read(b)
        
        until_date = Int.read(b)
        
        qts = Int.read(b)
        
        return UpdateBotSubscriptionExpire(user_id=user_id, payload=payload, invoice_slug=invoice_slug, until_date=until_date, qts=qts)
