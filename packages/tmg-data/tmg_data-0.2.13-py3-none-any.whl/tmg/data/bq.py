...
class Client:
    ...
    def upload_table(self, file_path, table, write_preference, separator=',', auto_detect=True, skip_leading_rows=True,
                     schema=(), partition_date=None, partition_field=None, partition_type=None, max_bad_records=0, file_format='csv'):
        ...
        Args:
            ...
            partition_type (str, Optional): The partition type. The default is DAY. The possible values are DAY, HOUR, MONTH, YEAR.
            max_bad_records (int, Optional): The maximum number of invalid rows. Defaults to :data:`0`.
            file_format (str, Optional): The file format. Defaults to :data:`csv`. The possible values are csv, parquet, pq.
        ...
        if file_format == 'csv':
            ...
            raise ValueError(f"Unsupported file format: {file_format}. Please use 'csv' or 'parquet'.")
        ...
        if partition_date:
            job_config.time_partitioning = bigquery.TimePartitioning(
                type_=getattr(bigquery.TimePartitioningType, partition_type.upper() if partition_type else "", "DAY"))
            table_id += '${}'.format(partition_date)
        elif partition_field:
            job_config.time_partitioning = bigquery.TimePartitioning(
                type_=getattr(bigquery.TimePartitioningType, partition_type.upper() if partition_type else "", "DAY"), field=partition_field)
    ...
    def run_query(self, query=None, query_file_name=None, params=(), destination=None, write_preference='empty',
                  partition_date=None, partition_field=None, partition_type=None):
        ...
        Args:
            ...
            partition_date (str, Optional): The ingestion date for partitioned destination table. For example: ``20210101``. The partition field name will be __PARTITIONTIME
            partition_field (str, Optional): The field on which the destination table is partitioned. The field must be a top-level TIMESTAMP or DATE field. If this field is used [partition_date] must not be specified.
            partition_type (str, Optional): The partition type. The default is DAY. The possible values are DAY, HOUR, MONTH, YEAR.
        ...
        if destination:
            if partition_date:
                ...
                job_config.time_partitioning = bigquery.TimePartitioning(
                    type_=getattr(bigquery.TimePartitioningType, partition_type.upper() if partition_type else "", "DAY"), field=partition_field)
            elif partition_field and partition_date:
                raise Exception("if [partition_field] is used then a [partition_date] must not be specified.")
