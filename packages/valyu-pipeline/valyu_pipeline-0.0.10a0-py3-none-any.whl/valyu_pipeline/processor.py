...
class PDFProcessor:
    ...
    def _poll_job_status(self, job_id: str, total_pdfs: int) -> Dict[str, Dict[str, Any]]:
        ...
        # Create progress bars for PDFs with notebook-friendly settings
        pdf_ocr_pbar = tqdm(total=total_pdfs, desc="OCR (PDFs)", 
                           position=0, leave=True, mininterval=1.0)
        pdf_img_pbar = tqdm(total=total_pdfs, desc="Image Extraction (PDFs)", 
                           position=1, leave=True, mininterval=1.0)
        ...
        while True:
                    ...
                    if total_pages > 0:  # Only initialize if we have a real page count
                        if page_ocr_pbar is None:
                            page_ocr_pbar = tqdm(total=total_pages, desc="OCR (pages)", 
                                               position=2, leave=True, mininterval=1.0)
                            page_img_pbar = tqdm(total=total_pages, desc="Image Extraction (pages)", 
                                               position=3, leave=True, mininterval=1.0)
                        else:
            ...
            if is_notebook:
                display(pdf_ocr_pbar)
                display(pdf_img_pbar)
                if page_ocr_pbar:
                    display(page_ocr_pbar)
                if page_img_pbar:
                    display(page_img_pbar)
    ...
    def _upload_pdfs(self, upload_urls: UploadUrls, folder_path: str, file_paths: List[str]) -> None:
        """
        ...
        def upload_file(file_path: str) -> None:
            with open(f"{folder_path}/{file_path}", 'rb') as file:
                response = requests.put(upload_urls.presigned_urls[file_path], data=file)
                response.raise_for_status()  # Raise an error for bad responses
