...
| id     | name | Limitation |
|--------|------|------------|
| 1.1.2  | code_tracing | Tracing code changes back to a task, requires visibility into crosslinked issues from the relevant merge request. Currently this feature is not available, [however it has been requested](https://gitlab.com/gitlab-org/gitlab/-/issues/461536). This function simply checks if _issues_ are utilised in the project and `FAIL`'s if not. |
