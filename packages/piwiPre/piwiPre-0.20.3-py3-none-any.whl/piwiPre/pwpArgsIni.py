...
class PwpArgsIni(argparse.ArgumentParser):
    ...
    def parse_args_and_ini(self, program: str, ini_to_parse: str, arguments, with_config=True,):
        ...
        old_dir = os.getcwd()
        new_dir = self.get_val_from_cmdline('--base', real_args)
        ...
        if new_dir:
            ...
            new_dir = PwpConfig.guess_latest_base(self.get_val_from_cmdline('--home', real_args))
        ...
        new_dir = self.get_val_from_cmdline('--home', real_args)
        if new_dir:
            if not os.path.isdir(new_dir):
    ...
    def parse_for_dir(self, path, home: str, base: str, language, real_args):
        ...
        if path == home:
            return manage_cmdline(config)

        target = os.path.abspath(base + '/' + ini_to_parse)
