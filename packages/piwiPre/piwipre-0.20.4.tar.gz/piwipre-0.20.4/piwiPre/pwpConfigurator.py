...
class PwpEditorUI(PwpGui):
    ...
    def __gui_change_parameters(self):
        ...
        row += 1
        label3 = GuiLabel(self, self.parameters_menu, column=0, row=row,
                          text="cmd line",
                          fr_text="ligne de commande",
                          width="", bold=True)
        self.parameters_menu.add_item(label3)
...
class PwpConfigurator:
    ...
    def set_dir_and_config(self, path, config):
        if path == "CMD LINE":
            ...
            LOGGER.msg(f"target           : 'cmd line items'")
            path = self.base
            # TODO: MAYBE we should take the highest item in the config hierarchy,
            #       so that the merge with cmdline can be seen
            #       but this can be hard to explain to the user
        else:
...
"""
        ...
        if self.dir_to_configure == self.base:
            # The following piece of code is buggy and hard to understand the rationale
            # in any case, we have to wait for the final value of home and base,
            # which may be modified by manage_cmdline_specials
            # so config_delta.save_base_history() would be wrong
            #
            # if 'home' in config_delta:
            #     config_delta.save_base_history()
            # else:
