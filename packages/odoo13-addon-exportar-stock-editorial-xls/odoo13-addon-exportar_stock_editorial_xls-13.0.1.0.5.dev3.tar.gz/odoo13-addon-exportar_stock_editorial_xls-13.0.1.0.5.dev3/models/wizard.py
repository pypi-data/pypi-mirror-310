    ...
    def get_lines(self, data, warehouse):
        ...
        for obj in categ_products:
            ...
            books_delivered_authorship = obj.get_product_quantity_in_location(
                self.env.company.location_authors_id.id
            )
            books_for_promotion = obj.get_product_quantity_in_location(
                self.env.company.location_promotion_id.id
            )

            destroyed = obj.get_product_quantity_in_location(
                [location.id for location in self.env['stock.location'].search([('scrap_location', '=', True)])]
            )
