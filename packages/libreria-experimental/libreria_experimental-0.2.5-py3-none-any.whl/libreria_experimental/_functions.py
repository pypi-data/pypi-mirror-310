...
def ajuste_linear(x,y,errorx=0,errory=0,xlabel='',ylabel='',titulo=''):
    ...
    r21='r^2='+str(r2)
    return f1,m1,b1,r21
def ajuste_magico(x,y,funcion,parametros,xerror=0,yerror=0,xlabel='',ylabel='',titulo=''):
    ...
    for i in x[1:]:
        max_x=max(max_x,i)        
    x_ajuste=np.linspace(min_x,10,max_x)
    plt.figure()
    plt.plot(x,y,'o',label='Datos')
...
def properrores(listav,f,errores,valores1):
    ecuacion=0
    errorindirecto1=[]
    valores=valores1.T
