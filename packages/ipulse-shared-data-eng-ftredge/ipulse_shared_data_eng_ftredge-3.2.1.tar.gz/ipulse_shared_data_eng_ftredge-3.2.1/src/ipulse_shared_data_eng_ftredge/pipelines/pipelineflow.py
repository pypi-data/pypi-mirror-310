...
class PipelineFlow:
    ...
    #             task_or_group.complete()  # Automatically compute the group's completion status
    #             header = f"{' ' * indent}** {task_or_group.name}  **"
    #             inner_flow = "\n".join(_generate_flow(t, indent + 2) for t in task_or_group.tasks.values())
    #             return f"{header}\n{inner_flow}"

    #     return "\n".join(_generate_flow(step) for step in self.steps.values()) + "\n"
    

    # def get_pipeline_flow(self) -> str:
    #     """
    #     Generate a string representation of the pipeline flow.
    #     :return: String representing the pipeline flow.
    #     """
    ...
    #         elif isinstance(task_or_group, PipelineLoopGroup):
    #             iteration_status = (
    #                 f"Iterations: {task_or_group.current_iteration}/{task_or_group.iteration_end}"
    #                 if task_or_group.iteration_end is not None
    #                 else f"Current Iteration: {task_or_group.current_iteration} (Total unknown)"
    #             )
