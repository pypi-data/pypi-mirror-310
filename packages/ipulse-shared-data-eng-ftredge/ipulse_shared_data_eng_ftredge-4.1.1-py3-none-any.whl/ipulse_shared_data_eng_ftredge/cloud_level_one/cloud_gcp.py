...
from .helper import handle_operation_exception
...
def get_secret_from_gcp_secret_manager_extended(
    ...
    result = {"data":None,
               "status":{
                    "action_type": DataActionType.READ_FROM_SECRET_MANAGER.value,
                    "execution_state": "NOT_STARTED",
                    "exception_or_error_during_operation": "",
                    "details": {
                        "secret_id": secret_id,
    ...
    try:
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_SECRET_FROM_SECRET_MANAGER_FAILED,
                                   operation_name="get_secret_from_gcp_secret_manager_extended", pipelinemon=pipelinemon,
...
def read_file_from_gcs_extended(storage_client:GCSClient, bucket_name:str, file_path:str,
    ...
    result = {
        "data": None,
        "status": {
            "action_type": DataActionType.READ_FROM_FILE.value,
            "execution_state": "NOT_STARTED",
            "exception_or_error_during_operation": None,
            "details": {
    ...
    try:
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_FILE_FROM_CLOUD_STORAGE_FAILED,
                                   operation_name="read_file_from_gcs_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
    return result
...
def write_file_to_gcs_extended(storage_client: GCSClient,
    ...
    try:
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_WRITE_IN_CLOUD_STORAGE_FAILED,
                                   operation_name="write_file_to_gcs_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def _get_bigquery_job_status(job: Union[bigquery.LoadJob, bigquery.QueryJob, bigquery.CopyJob, bigquery.ExtractJob],max_job_errors_to_log:int=7) -> Dict[str, Any]:
...
def _summarize_job_errors(job: Union[bigquery.LoadJob, bigquery.QueryJob, bigquery.CopyJob, bigquery.ExtractJob], max_errors_to_log: int = 7) -> str:
...
def _determine_column_type(value: Any) -> str:
    ...
    try:
     ...
     # Check if the TABLE exists before attempting to delete it
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_CREATE_CLOUD_DB_TABLE_FAILED,
                                   operation_name="create_bigquery_table_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
    return result
...
def write_insert_batch_into_bigquery_extended(project_id: str,
    ...
    try:
        ...
        load_job =bigquery_client.load_table_from_json(data, data_table_full_path, job_config=job_config,project=project_id)
        load_job.result()  # Wait for job completion
        if pipelinemon:
            pipelinemon.add_system_impacted(f"bigquery_load: {data_table_full_path}")
        result["status"]["details"].update(_get_bigquery_job_status(job=load_job))
        result["status"]["exception_or_error_during_operation"]= _summarize_job_errors(job=load_job, max_errors_to_log=max_job_errors_to_log)
        # Check job status
        if load_job.state == "DONE" and load_job.errors is None:
            result["status"]["execution_state"] += ">INSERT_JOB_SUCCESS"
            msg=f"Successful load_table_from_json {load_job.job_id} for table {data_table_full_path}."
            log_info(msg=msg, logger=logger, print_out=print_out)
            log_debug(msg=result["status"], logger=logger, print_out=print_out) 
            if pipelinemon:
            ...
            error_message = f"ERRORED Bigquery load_table_from_json {load_job.job_id} for table {data_table_full_path}. Job Results: {result["status"]}."
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_WRITE_IN_CLOUD_DB_FAILED,
                                   operation_name="write_insert_batch_into_bigquery_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def write_merge_batch_into_bigquery_extended(
    ...
    try:
        ...
        create_temp_table_from_json_result=write_create_bigquery_table_from_json_extended(bigquery_client=bigquery_client,
                                                                      ...
                                                                      logger=logger,
                                                                      print_out=print_out,)
        result["status"]["create_temp_table_from_json_status"]=create_temp_table_from_json_result["status"]
        if create_temp_table_from_json_result["status"]["exception_or_error_during_operation"]:
            error_already_logged=True
            result["status"]["execution_state"] += ">TEMP_TABLE_CREATION_FAILED"
            raise Exception(create_temp_table_from_json_result["status"]["exception_or_error_during_operation"])
        ############################################################
         # 2. Perform the Merge:
         ############################################################
        ...
        merge_temp_table_into_target_table_result=write_merge_two_bigquery_tables_extended(bigquery_client=bigquery_client,
                                                                                        project_id=project_id,
                                                                                        target_table_full_path=data_table_full_path,
        ...
        result["status"]["merge_temp_table_into_target_table_status"]=merge_temp_table_into_target_table_result["status"]
        if merge_temp_table_into_target_table_result["status"]["exception_or_error_during_operation"]:
            error_already_logged=True
            result["status"]["execution_state"] += ">MERGE_FAILED"
            raise Exception(merge_temp_table_into_target_table_result["status"]["exception_or_error_during_operation"])
        ...
        if not error_already_logged:
            handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_MERGE_FAILED,
                                    operation_name="write_merge_batch_into_bigquery_extended", pipelinemon=pipelinemon,
                                    logger=logger, print_out=print_out, raise_e=raise_e)       
    ...
    ############################################################   
        ...
        if temp_table_name:
            try:
                ...
                result["status"]["execution_state"] += "_TEMP_TABLE_DELETED"
...
def write_create_bigquery_table_from_json_extended(
    ...
    if isinstance(data, dict):
        data = [data]
    temp_str="TEMP_" if temp else ""
    result = {
        "status": {
            "action_type": DataActionType.CREATE_TABLE_FROM_JSON.value,
            "execution_state": "NOT_STARTED",
            "exception_or_error_during_operation": "",
            "details": {}
        }
    ...
    try:
        ...
        result["status"]["execution_state"] = temp_str+"TABLE_LOAD_STARTED"
        ...
        result["status"]["exception_or_error_during_operation"]= _summarize_job_errors(job=load_job, max_errors_to_log=max_job_errors_to_log)
        if load_job.state == "DONE" and load_job.errors is None:
            result["status"]["execution_state"] = temp_str+"TABLE_CREATE_AND_LOAD_SUCCESS"
            ...
            result["status"]["execution_state"] = temp_str+"TABLE_CREATE_AND_LOAD_WITH_ERRORS"
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_CREATE_AND_LOAD_CLOUD_DB_TABLE_FAILED,
                                   operation_name="write_create_bigquery_table_from_json_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def write_merge_two_bigquery_tables_extended(target_table_full_path: str,
    ...
    try:
        ...
        result["status"]["details"].update(_get_bigquery_job_status(job=query_merge_job))
        result["status"]["exception_or_error_during_operation"]=_summarize_job_errors(job=query_merge_job, max_errors_to_log=max_job_errors_to_log)
        # Check job status
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_MERGE_FAILED,
                                   operation_name="write_merge_two_bigquery_tables_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
    return result
...
def write_query_sql_bigquery_table(project_id: str,
    ...
    try:
        ...
        result["status"]["details"].update(_get_bigquery_job_status(job=query_write_job))
        result["status"]["exception_or_error_during_operation"]= _summarize_job_errors(job=query_write_job, max_errors_to_log=max_job_errors_to_log)
        # Check job status
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_ACTION_WRITE_IN_CLOUD_DB_FAILED,
                                   operation_name="write_query_sql_bigquery_table", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
...
def read_query_sql_bigquery_table_extended( project_id: str,
    ...
    try:
        ...
        result["status"]["details"].update(_get_bigquery_job_status(job=query_read_job))
        result["status"]["exception_or_error_during_operation"]=_summarize_job_errors(job=query_read_job, max_errors_to_log=max_job_errors_to_log)
        ...
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_DB_FAILED,
                                   operation_name="read_query_sql_bigquery_table_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)

    return result


def read_query_condition_for_dates_bigquery_extended(
    project_id: str,
    data_table_full_path: str,
    data: Union[Dict[str, Any], List[Dict[str, Any]]],
    date_column: str,
    ...
    try:
        ...
        column_type = _determine_column_type(first_value)
...
#                                    bigquery.ScalarQueryParameter("records_oldest_date", "DATE", sourced_records_oldest_date))   
        ...
        result["status"]["execution_state"] += ">QUERY_READ_JOB_STARTED"
        query_read_job = bigquery_client.query(query, job_config=job_config, project=project_id)
        query_response = query_read_job.result() # Wait for the job to complete    
        result["status"]["details"].update(_get_bigquery_job_status(job=query_read_job))
        result["status"]["exception_or_error_during_operation"] = _summarize_job_errors(job=query_read_job, max_errors_to_log=max_job_errors_to_log)
        if query_read_job.state == 'DONE' and query_read_job.errors is None:
            succss_msg=f"QUERY_READ_JOB_SUCCESS. Fetched {len(query_response)} records."
        ...
        if column_type == "DATE":
            existing_values = {row[date_column].strftime('%Y-%m-%d') for row in query_response}
        else:
            existing_values = {row[date_column] for row in query_response}

        result["data"] = existing_values
        return result

    except Exception as e:
        handle_operation_exception(e=e, result=result, log_level=LogLevel.ERROR_READ_DB_FAILED,
                                   operation_name="read_query_condition_for_dates_bigquery_extended", pipelinemon=pipelinemon,
                                   logger=logger, print_out=print_out, raise_e=raise_e)
