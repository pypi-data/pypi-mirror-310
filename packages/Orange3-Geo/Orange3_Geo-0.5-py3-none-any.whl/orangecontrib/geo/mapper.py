...
def __load():
    ...
    if not files:
        raise RuntimeError(
            ('Missing GeoJSON files. ' + ('In development environments, merge in the "json"' + 'branch. See CONTRIBUTING.md')))
    ...
    for filename in files:
        ...
        for feature in collection['features']:
            ...
            if (np.isnan(p.get(_tr.m[2, 'latitude'], np.nan)) or
                np.isnan(p.get(_tr.m[3, 'longitude'], np.nan))):
                try:
                ...
                p.update({_tr.m[4, "latitude"]: r.y, _tr.m[5, "longitude"]: r.x})
...
def get_bounding_rect(region_ids):
    ...
    if not region_ids:
        return None
    coords = itemgetter(_tr.m[6, 'longitude'], _tr.m[7, 'latitude'])
