...
class OWGeoTransform(OWWidget):
    name = _tr.m[85, "Geo Transform"]
    description = _tr.m[86, "Transform geographic coordinates from one system to another."]
    icon = "icons/GeoTransform.svg"
    priority = 320
    keywords = _tr.m[87, "geo transform, transform, geo"]

    class Inputs:
        data = Input(_tr.m[88, "Data"], Table)

    class Outputs:
        data = Output(_tr.m[89, "Data"], Table)
    ...
    class Error(OWWidget.Error):
        no_lat_lon_vars = Msg(_tr.m[90, "Data has no latitude and longitude variables."])
    ...
    def __init__(self):
        ...
        layout = QFormLayout()
        gui.widgetBox(self.controlArea, box=_tr.m[91, "Coordinates:"], orientation=layout)
        ...
        layout.addRow(_tr.m[92, "Latitude:"], gui.comboBox(None, self, "attr_lat", **args))
        layout.addRow(_tr.m[93, "Longitude:"], gui.comboBox(None, self, "attr_lon", **args))
        layout.addWidget(
            gui.checkBox(
                None, self, "replace_original", _tr.m[94, "Replace original coordinates"],
                tooltip=(_tr.m[95, "If unchecked, the original coordinates are retained "] + _tr.m[96, "and new coordinates are added as separate variables."])))

        layout = QFormLayout()
        gui.widgetBox(self.controlArea, _tr.m[97, "Transformation:"], orientation=layout)
        args["model"] = PyListModelTooltip(self.EPSG_CODES, list(self.EPSG_CODES))
        layout.addRow(_tr.m[98, "From:"], gui.comboBox(None, self, "from_idx", **args))
        layout.addRow(_tr.m[99, "To:"], gui.comboBox(None, self, "to_idx", **args))

        self.commit_button = gui.button(self.controlArea, self, _tr.m[100, "&Commit"],
                                        self.apply)
    ...
    def _transformed_domain(self):
        ...
        if self.replace_original:
            ...
            if not (names[0].lower().startswith(LATITUDE_NAMES) and
                ...
                names = (_tr.m[101, "latitude"], _tr.m[102, "longitude"])
    ...
    def send_report(self):
        ...
        self.report_items(
            "",
            [(_tr.m[103, "Original system"], data.from_trans),
             (_tr.m[104, "Conversion to"], data.to_trans),
             (_tr.m[105, "Coordinate variables"], f"{data.coord_names[0]} / {data.coord_names[1]}"),
             (_tr.m[106, "Output coordinates"],
              data.transf_names
