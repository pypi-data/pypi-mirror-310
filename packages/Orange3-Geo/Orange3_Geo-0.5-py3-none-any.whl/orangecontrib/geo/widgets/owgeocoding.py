...
class OWGeocoding(widget.OWWidget):
    name = _tr.m[47, "Geocoding"]
    description = (_tr.m[48, 'Encode region names into geographical coordinates, or '] + _tr.m[49, 'reverse-geocode latitude and longitude pairs into regions.'])
    icon = "icons/Geocoding.svg"
    priority = 40
    keywords = _tr.m[50, "geocoding, geo, coding"]

    class Inputs:
        data = Input(_tr.m[51, "Data"], Table, default=True)

    class Outputs:
        coded_data = Output(_tr.m[52, "Coded Data"], Table, default=True)

    settings_version = 2
    settingsHandler = settings.DomainContextHandler()

    ID_TYPE = OrderedDict((
        (_tr.m[53, 'Country name'], ToLatLon.from_cc_name),
        (_tr.m[54, 'ISO 3166-1 alpha-2 country code'], ToLatLon.from_cc2),
        (_tr.m[55, 'ISO 3166-1 alpha-3 country code'], ToLatLon.from_cc3),
        # ('ISO 3166-1 numeric-3 country code', ),  # Who uses this?
        # ('ISO 3166-2 subdivision code', ),  # XXX: where to get these?
        (_tr.m[56, 'Region name'], ToLatLon.from_region),
        (_tr.m[57, 'Major city (US)'], ToLatLon.from_city_us),
        (_tr.m[58, 'Major city (Europe)'], ToLatLon.from_city_eu),
        (_tr.m[59, 'Major city (World)'], ToLatLon.from_city_world),
        (_tr.m[60, 'FIPS code'], ToLatLon.from_fips),
        (_tr.m[61, 'HASC code'], ToLatLon.from_hasc),
        (_tr.m[62, 'US state (name or abbr.)'], ToLatLon.from_us_state),
    ))
    ...
    def __init__(self):
        ...
        gui.appendRadioButton(
            modes, _tr.m[63, '&Encode region names into geographical coordinates:'], insertInto=top)
        ...
        gui.comboBox(
            box, self, 'str_attr', label=_tr.m[64, 'Region identifier:'],
        ...
        self.str_type_combo = gui.comboBox(
            box, self, None, label=_tr.m[65, 'Identifier type:'], orientation=Qt.Horizontal,
        ...
        gui.appendRadioButton(
            modes, _tr.m[66, '&Decode latitude and longitude into regions:'], insertInto=top)
        box = gui.indentedBox(top)
        model = DomainModel(parent=self, valid_types=ContinuousVariable)
        self.domainmodels.append(model)
        combo = gui.comboBox(
            box, self, 'lat_attr', label=_tr.m[67, 'Latitude:'], orientation=Qt.Horizontal,
            callback=self.commit.deferred, sendSelectedValue=True, model=model)
        combo = gui.comboBox(
            box, self, 'lon_attr', label=_tr.m[68, 'Longitude:'], orientation=Qt.Horizontal,
            callback=self.commit.deferred, sendSelectedValue=True, model=model)
        gui.comboBox(
            box, self, 'admin', label=_tr.m[69, 'Administrative level:'], orientation=Qt.Horizontal,
            callback=self.commit.deferred,
            items=(_tr.m[70, 'Country'],
                   _tr.m[71, '1st-level subdivision (state, region, province, municipality, ...)'],
                   _tr.m[72, '2nd-level subdivisions (1st-level & US counties)']),)
        ...
        gui.checkBox(
            ...
            label=_tr.m[73, 'E&xtend coded data with additional region properties'],
            callback=self.commit.deferred,
            toolTip=(_tr.m[74, 'Extend coded data with region properties, such as'] + (_tr.m[75, 'ISO codes, continent, subregion, region type, '] + _tr.m[76, 'economy type, FIPS/HASC codes, region capital etc. as available.'])))

        gui.auto_commit(self.controlArea, self, 'autocommit', _tr.m[77, '&Apply'])
        ...
        model.setHorizontalHeaderLabels([_tr.m[78, 'Unmatched Identifier'], _tr.m[79, 'Custom Replacement']])
        box = gui.vBox(self.mainArea)
        self.info_str = ' /'
        gui.label(box, self, _tr.m[80, 'Unmatched identifiers: %(info_str)s'])
    ...
    def encode(self):
        ...
        return self._to_addendum(latlon, [_tr.m[81, 'latitude'], _tr.m[82, 'longitude']])
    ...
    def _to_addendum(self, df, keep):
        ...
        for col in addendum:
            ...
            if col in (_tr.m[83, 'latitude'], _tr.m[84, 'longitude']):
