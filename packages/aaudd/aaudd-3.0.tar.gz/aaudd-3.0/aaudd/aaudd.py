...
# Определяем путь к папке, где установлен скрипт
script_dir = Path(__file__).parent

TOKEN_FILE = os.path.join(script_dir, "api_token.txt")
LANGUAGE_FILE = os.path.join(script_dir, "language.txt")
...
def main():
    ...
    while True:
        ...
        if save_option == 'y':
            ...
            with open(os.path.join(script_dir, f"{file_name}.txt"), 'w') as file:
                file.write(output)
            if language == 'english':
