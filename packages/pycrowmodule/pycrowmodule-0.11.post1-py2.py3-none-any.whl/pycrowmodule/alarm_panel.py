...
class CrowAlarmPanel():
    ...
    def start(self):
        ...
        if self._connectionType == 'ip':
            logging.info(str.format("Connecting to Crow IP / Serial Module using host: {0}, port: {1}", self._host, self._port))
        elif self._connectionType == 'serial':
            logging.info(str.format("Connecting to Crow IP / Serial Module using serial port: {0}", self._port))
        self._client = CrowModuleClient(self, self._eventLoop)
        self._client.start()
