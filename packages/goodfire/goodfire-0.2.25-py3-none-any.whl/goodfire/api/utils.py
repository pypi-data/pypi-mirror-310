...
class HTTPWrapper:
    def __init__(self, inital_backoff_time: float = 1.75, max_retries: int = 5):
...
class AsyncHTTPWrapper:
    def __init__(self, initial_backoff_time: float = 1.75, max_retries: int = 5):
    ...
    async def get(
        ...
        async with httpx.AsyncClient() as client:
            ...
            try:
                ...
                if _attempt_num < self.max_retries:
                    self._rate_limit_warning()
                    await asyncio.sleep(self.initial_backoff_time**_attempt_num)
