...
class Token:
    ...
    def inspect(self, k: int = 5):
        ...
        features = [self._context._features[uuid] for uuid in uuids]
...
class ContextInspector:
    def __init__(
        ...
        features: list[Feature] = []
        for chunk_start in range(0, len(self._feature_ids), 100):
            features += self._client.list(
                list(self._feature_ids)[chunk_start : chunk_start + 100]
            )
    ...
    def top(self, k: int = 5):
        ...
        features = [self._features[feat_id] for feat_id, _ in sorted_feature_ids[:k]]
    ...
    def matrix(self) -> tuple[NDArray[np.float64], dict[int, Feature]]:
        feature_lookup: dict[int, Feature] = {}

        token_vectors: list[np.ndarray[int, np.float64]] = []
        for token in self.tokens:
            token_vector, token_feature_lookup = token.inspect().vector()
            feature_lookup.update(token_feature_lookup)
            token_vectors.append(token_vector)

        return np.ndarray(token_vectors), feature_lookup
