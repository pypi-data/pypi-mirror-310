...
class DataUtils:
    ...
    def transform_numeric_columns(df, fill_value=0, transform_func=None, meta_type='int64'):
        ...
        if float_columns:
            ...
            for col in float_columns:
                df[col] = df[col].fillna(fill_value).astype(meta_type).apply(
                    transform_func_float, meta=(col, meta_type)
                )
        return df
    ...
    def merge_lookup_data(classname, df, **kwargs):
        ...
        if source_col not in df.columns:
            DataUtils.logger.info(f'{source_col} not in dataframe columns')
            return df
        ids = list(df[source_col].dropna().unique())
        if not ids:
            DataUtils.logger.info(f'No IDs found in the source:{source_col} column')
            return df
