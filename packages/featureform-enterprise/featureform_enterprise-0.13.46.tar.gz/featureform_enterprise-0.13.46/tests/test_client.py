...
def test_kafka_config_serde():
    # Create an original KafkaConfig object
    original_kafka_config = KafkaConfig(
        bootstrap_servers=["broker1:9092", "broker2:9092"], use_msk_iam_auth=False
    )

    # Serialize the object
    serialized_data = original_kafka_config.serialize()

    # Deserialize the data back into a new object
    deserialized_kafka_config = KafkaConfig.deserialize(serialized_data)

    # Ensure the original and deserialized objects are the same
    assert (
        original_kafka_config.bootstrap_servers
        == deserialized_kafka_config.bootstrap_servers
    )
    assert (
        original_kafka_config.use_msk_iam_auth
        == deserialized_kafka_config.use_msk_iam_auth
    )
