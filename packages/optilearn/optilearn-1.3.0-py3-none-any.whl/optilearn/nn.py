...
class Sequential:
  ...
  def ins(self,return_params=False):
    ...
    weights1 = list(map(lambda x: np.random.normal(loc=0.0 ,scale = np.sqrt(2 / weights[x].shape[0]),size=weights[x].shape),range(len(weights))))
    biases1 = list(map(lambda x: np.zeros((biases[x].shape[0],)),range(len(biases))))
  ...
  def forword(self,input_data,return_all_outputs=False):
    import numpy as np
