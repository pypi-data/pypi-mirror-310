...
async def expected_all(fixtures: Mapping[CollectionKey | str, List[DesktopObjectDict]],
    ...
    for desktop_object_dict in desktop_object_dicts:
        ...
        items.append(item)
    if desktop_object is None or len(desktop_object_dicts) > 1:
        empty_obj = type(desktop_object)()
        empty_obj.owner = context.sub
        attr_perms = {attr: empty_obj.custom_attribute_permissions(attr, context.sub) for attr in empty_obj.get_attributes()}
    else:
        attr_perms = await desktop_object.get_all_attribute_permissions(context)
...
def _template_data_generator(sub: str, action: dict[str, Any], desktop_object: DesktopObject, attr_perms: dict[str, list[Permission]], len_fixtures=1):
...
def _new_template_data_item(i: dict[str, Any], value: Any, sub: str, perms: list[Permission]) -> dict[str, Any]:
    """
...
def _set_collection_template(action, collection_doc, desktop_object, len_desktop_objects, rel, sub, attr_perms):
    """
