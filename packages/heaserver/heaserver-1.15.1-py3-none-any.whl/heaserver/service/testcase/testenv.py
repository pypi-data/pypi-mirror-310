...
from abc import ABC
from ..util import retry, raise_multiple
import logging
...
def app_context(db_manager_cls: Type[MicroserviceDatabaseManager],
    ...
    with TestEnvExitStack() as context_manager, closing(db_manager_cls()) as external_db_, \
        ...
        with ThreadPoolExecutor() as pool:
            ...
            futures: list[Future] = []
            futures.append(pool.submit(external_db_.insert_all, external_desktop_objects, content))
            for b_ in bridge_dbs:
                futures.append(pool.submit(b_.insert_all, bridge_desktop_objects, content)) # Use exception groups when we start using Python 3.11
