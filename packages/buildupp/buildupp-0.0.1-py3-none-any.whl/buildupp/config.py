...
class Config:
    ...
    def load_systems_measured(self):
        ...
        spec = importlib.util.spec_from_file_location("systems_measured", systems_measured_file)
        ...
        for section in config.sections():
            ...
            for key in config[section]:
                try:
                    ...
                    compound_data[key] = float(config[section][key]) if '.' in config[section][key] else int(config[section][key])
                except ValueError:
    ...
    def load_powder_properties(self):
        """
        Load powder properties from the physical properties INI file.
        Returns a dictionary of powder properties, where each powder is a key.
        """

        properties_file = self.config_dir + '/physical_properties.ini'
        ...
        powder_properties = {}
        for section in config.sections():
            powder_properties[section] = {}
            for key, value in config.items(section):
                try:
                    # Try to convert to float if possible
                    powder_properties[section][key] = float(value)
                except ValueError:
                    # If it's not a number, store as a string
                    powder_properties[section][key] = value
            
        return powder_properties
    ...
    def load_user_functions(self):
        ...
        spec = importlib.util.spec_from_file_location("user_functions", user_functions_file)
    ...
    def get_psd_data(self, material: str):
        ...
        psd_filename = self.powder_properties[material].get('psd_file')
        reader_function_name = self.powder_properties[material].get('psd_reader_function')
        
        if not psd_filename or not reader_function_name:
            raise ValueError(f"Missing PSD path or reader function for material {material}.")

        # Dynamically resolve the correct path to the PSD file based on the user's data root path
        psd_path = os.path.join(self.data_root_directory, psd_filename)
        ...
        if not reader_function:
            raise AttributeError(f"Reader function {reader_function_name} not found in user_functions.py.")

        psd_df = reader_function(psd_path)

        if not isinstance(psd_df, pd.DataFrame):
            raise TypeError(f"Reader function {reader_function_name} did not return a pandas DataFrame.")
