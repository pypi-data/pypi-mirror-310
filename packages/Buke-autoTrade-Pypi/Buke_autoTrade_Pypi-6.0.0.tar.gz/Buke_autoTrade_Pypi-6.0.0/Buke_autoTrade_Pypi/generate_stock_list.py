...
def get_day_price(dir, end_date):
    ...
    day_price = pd.read_csv(f"{dir}/data_file/수급데이터_2022_to_{end_date.year}.csv")
...
def generate_stock_list(dir, day):
    ...
    df.to_csv(f"{dir}/data_file/코스피_종목리스트_2022_to_{day.year}.csv")

    df = pd.DataFrame(kosdaq_date_groupby_df)
    df.to_csv(f"{dir}/data_file/코스닥_종목리스트_2022_to_{day.year}.csv")
