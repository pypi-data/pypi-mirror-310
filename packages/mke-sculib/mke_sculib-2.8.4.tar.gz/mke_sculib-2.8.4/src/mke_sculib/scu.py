...
def send_mattermost(url, whoami, subject, t=''):
    ...
    try:
        ...
        if not t:
            t = 'no_time_given'
        s = f'```[{t}] {whoami}@{send_mattermost.ip} : {subject}```'
...
def _cb_log_send_mattermost(name, msg, color, t, *args, mattermost_url = None, **kwargs):
    ...
    if not mattermost_url is None:
        ...
        threading.Thread(target = send_mattermost, args=(mattermost_url, name, emoji+msg, t)).start()
...
# ██ ██   ████    ██    ███████ ██   ██ ██   ████ ██   ██ ███████     ██       ██████  ██   ████ ███████ 
    ...
    def log(self, msg, *args, t = None, name=None, color=colors.BLACK, **kwargs):
        ...
        if not t and self.use_socket and self.data_streamer and self.data_streamer.is_alive(): # not costly to get current time!
...
#  ██████  ██████  ██   ████   ████   ███████ ██   ████ ██ ███████ ██   ████  ██████ ███████ 
    ...
    def status(self, as_log=True, oneliner=True):
        ...
        if as_log:
            color = colors.FAIL if oneliner and 'FAIL' in s else colors.BOLD
            if not oneliner:
                s = '\n' + s
