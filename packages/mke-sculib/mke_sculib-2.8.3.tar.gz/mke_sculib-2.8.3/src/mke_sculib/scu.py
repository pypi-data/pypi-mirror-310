...
class TrackHandler(object):
    def __init__(self, scu, t, az, el, activate_logging, config_name, wait_start=True, verb=False, wait_for_end_pos=True, wait_after_stopping_logger_sec=2):
        self.scu = scu
    ...
    def __enter__(self):
        ...
        scu = self.scu
        log('Entering a tracking table t_internal = {} AZEL_now: {:5.3f}°, {:5.3f}° ...'.format(scu.t_internal.iso, *scu.azel), color=colors.OKBLUE)

        t, az, el, activate_logging, config_name = self.args

        scu.stop_program_track(not self.verb)
        if self.wait_after_stopping_logger_sec:
        ...
        log(f'Entering a tracking table..DONE', color=colors.OKGREEN)
        return self
    
    def __exit__(self, *args):
        log(f'Exiting a tracking table..', color=colors.OKBLUE)
        scu = self.scu
        t, az, el, activate_logging, config_name = self.args
        t_start, t_end = Time(t[0], format='mjd'), Time(t[-1], format='mjd')
        ...
        log(f'Exiting a tracking table..DONE', color=colors.OKGREEN)
...
# ██ ██   ████    ██    ███████ ██   ██ ██   ████ ██   ██ ███████     ██       ██████  ██   ████ ███████ 
    ...
    def activate_logging_mattermost(self, whoami=None, url_qry = 'http://10.98.76.45:8990/logurl'):
        if whoami is None:
            whoami = self.antenna_id if self.antenna_id else self.ip
        return activate_logging_mattermost(whoami, url_qry)
        

    def deactivate_logging_mattermost():
        return deactivate_logging_mattermost()
...
#  ██████ ██      ██ ██████      ██████  ██ ███████ ██   ██ 
    ...
    def _tt_test_and_clean(self, t, az, el, verb=True):
        ...
        if verb:
            log('t_internal = {} AZEL_now: {:5.3f}°, {:5.3f}°'.format(self.t_internal.iso, *self.azel))
            log('Running Table ({}, {:5.3f}, {:5.3f}) --> ({}, {:5.3f}, {:5.3f}) T={:.2f} N={}, dt_min={:.12f}'.format(Time(t[0], format='mjd').iso, az[0], el[0], Time(t[-1], format='mjd').iso, az[-1], el[-1],  np.ptp(t_astro.unix), len(az), dt), color=colors.BOLD)
...
#  ███ ███  ██   ██ ██    ██        ██       ██████  ██   ████ ███████ 
    ...
    def wait_until(self, T, no_stout=False):
        ...
        if T < tnow:
            log('T={} is {} in the past! returning without waiting!'.format(T.iso, (T-tnow).to(u.s).value), color=colors.WARNING)
...
# ███████  ██████  ██████      ███████  ██████   ██████   ██████  ███████ ██   ██     ██   ██ ██      ██ 
    ...
    def logger_stop(self, verb=True):
        ...
        if verb:
            r = self.session_last()
            uuid = r.get('uuid')
            log(f'logger stop...success (last_session={r})')
...
# ███████ ███████ ███████ ███████ ██  ██████  ██   ████     ██   ██ ██      ██ 
    ...
    def sorted_sessions(self, isDescending = 'True', startValue = '1', endValue = '25', sortBy = 'startTime', filterType='indexSpan'):
        log(f'getting sortedSessions with {startValue=}, {endValue=}, {filterType=}')
