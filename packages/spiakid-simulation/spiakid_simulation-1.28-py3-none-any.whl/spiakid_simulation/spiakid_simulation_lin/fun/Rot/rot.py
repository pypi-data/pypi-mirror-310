...
def Rotation(rotation, altguide, azguide, latitude, posx, posy, exptime, pxnbr):
  ...
  if rotation == True:
    ...
    rot,evalt, alt_az_t, ra_dec_t, ang = EarthRotation(lat_tel=latitude,coo_guide=guide,coo_star=[posx, posy],time = exptime,size=pxnbr)
  else:
    ...
    ra_dec_t = [0,0,0]
    ang = 0
  return(rot, evalt, alt_az_t, ra_dec_t, ang)