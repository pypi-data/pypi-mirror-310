...
cdef void convert_to_stdvec(object stri, vector[string] * outvector):
    cdef:
        Py_ssize_t i
    if isinstance(stri, str) and os.path.exists(stri):
        with open(stri, mode='rb') as f:
            stri=f.read().splitlines()
    if isinstance(stri, (str, bytes)):
        stri = [stri]
    outvector.resize(len(stri))
    outvector.clear()
    for i in range(len(stri)):
        if isinstance(stri[i],bytes):
            outvector.emplace_back(<string>stri[i])
        else:
            outvector.emplace_back(<string>(stri[i].encode('utf-8', 'backslashreplace')))
...
cdef class PyStringMatcher:
    ...
    def __cinit__(self):
        ...
        self.stri1list=new vector[string]()
        self.stri2list=new vector[string]()

    def __init__(self, object stri1, object stri2):
        convert_to_stdvec(stri1,self.stri1list)
        convert_to_stdvec(stri2,self.stri2list)
        self.sm._load_vecs_for_cython(self.stri1list,self.stri2list)
    ...
    def __str__(self) -> str:
        ...
        self.sm._str__for_cython();
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\nCython data:")
        print("\n------------------------------Vector 1")
        _repr__for_cython(self.stri1list)
        print("\n------------------------------Vector 2")
        _repr__for_cython(self.stri2list)
        print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX\n")
        return ""
