...
def run_sql_script(
    ...
    None
    '''
    sql_files: List[os.PathLike[str]] = [os.path.join('sql', f)
                                        for f in os.listdir('sql')
                                        if os.path.isfile(os.path.join('sql', f))
                                        and f.endswith('.sql') and 'schema' in f.lower()]
    sql_files.sort()
    ...
    try:
        ...
        logger.error(f'Error occurred in executing SQL file - {filename} - {repr(e)} - {traceback.print_exc()}')
        raise
...
def load_mysql(
    ...
    4.1. Table: country  <- df_country
    4.2. Table: location <- df_location
    ...
    Args:
        ...
        df_dict (Dict[str, pd.DataFrame]): A dictionary which holds the created dataframes that maps the tables in schema `location_db`
    ...
    schema: str = env_vars['EMP_SCHEMA_LOCATION']
