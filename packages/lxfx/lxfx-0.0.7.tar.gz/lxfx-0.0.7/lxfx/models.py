...
class ConcatenationAttention(nn.Module):
    ...
    def forward(self, encoder_input:torch.Tensor = None, decoder_h_input:torch.Tensor = None):
        ...
        return context_vector, attention_weights # size = (batch_size, encoder_latent_dim)
...
class FxFCEncoder(nn.Module):
    ...
    def forward(self, input_data):
        ...
        if self.is_attentive:
            input_encoded = self.encoder.fc(input_encoded)
            return input_encoded[:, -1, :] , input_encoded
        else:
            return self.encoder.fc(input_encoded[:, -1, :])
...
class FxFCDecoder(nn.Module):
    def __init__(self, latent_dim,
        ...
        if self.is_attentive:
            self.attention = ConcatenationAttention(self.latent_dim, self.decoder_hidden_size)
        
        self.decoder_in_features = self.latent_dim+self.out_units[-1] if self.is_attentive else self.latent_dim
    ...
    def forward(self, encoded_input, encoder_outputs = None):
        outputs = []
        decoder_input = encoded_input
        # initialize the hidden states 
        # h_t, c_t = (self.tensor_initializer())
        # Note: encoded_input.size(0) is the batch_size
        prev_state = None
        if self.initialize_weights:
            if self.initializer_method == "xavier":
                ...
                h_t, c_t = (torch.zeros(self.num_layers, encoded_input.size(0), self.units[-1]).to(encoded_input.device),
                            torch.zeros(self.num_layers, encoded_input.size(0), self.units[-1]).to(encoded_input.device))
                prev_state = (h_t, c_t)
            else:
                prev_state = None
        else:
        ...
        for t in range(self.future_pred_length):
            ...
            if encoder_outputs is not None:
                ...
                h_t = h_t.squeeze(0) # size (batch_size, decoder_hidden_size) i.e remove the batch dim

                context_vector, attention_weights = self.attention(encoder_outputs, h_t)
                decoder_input = torch.cat((decoder_input, context_vector.unsqueeze(1)), dim=2)
...
class FxFCAutoEncoder(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        preds = self.decoder(encoded_vector, encoded_outputs)
        return preds
