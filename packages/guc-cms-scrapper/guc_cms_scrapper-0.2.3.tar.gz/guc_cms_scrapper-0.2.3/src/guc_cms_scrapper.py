...
class GucCmsScrapper:
    def __init__(self, username: str, password: str, base_url: str = "https://cms.guc.edu.eg"):
        self.authenticated_session = GucCmsScrapper.get_authenticated_session(username, password, base_url)
        self.base_url = base_url

    def get_authenticated_session(username: str, password: str, base_url: str = "https://cms.guc.edu.eg") -> requests.Session:
        """
    ...
    def get_courses(self) -> list[CourseMetadata]:
        ...
        url = self.base_url + "/apps/student/HomePageStn.aspx"
    ...
    def get_course_data(self, course_id: str, semester: str) -> CourseData:
        ...
        url = f"{self.base_url}/apps/student/CourseViewStn.aspx?id={course_id}&sid={semester}"
        response = self.authenticated_session.get(url)
        soup = BeautifulSoup(response.content, "html.parser")
        ...
        for weekSoup in soup.select(".weeksdata"):
            ...
            for itemSoup in weekSoup.select(".card-body"):
                ...
                url = self.base_url + "/" + itemSoup.select_one("a")["href"]
