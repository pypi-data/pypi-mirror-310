...
class aef_gw:
    ...
    def start(self):
        ...
        if self.northbound_info["southbound"]["authentication_method"] == "HTTP Basic Authentication":
            self.__generate_northbound_api()
        if self.northbound_info["southbound"]["authentication_method"] == "JWT Bearer Token":
    ...
    def create_route_handler(self, route, params, decode_token, method):
        ...
        async def route_handler(
            ...
            try:
                ...
                jwt_token = {self.southbound_info["southbound"]["credentials"].get("jwt")}
                if not jwt_token:
