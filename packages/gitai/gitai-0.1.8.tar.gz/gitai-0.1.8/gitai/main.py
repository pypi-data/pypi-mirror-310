...
DESCRIBE_TEMPLATE = """
You are a software developer. You have made some changes to the codebase. Please review the provided files and generate a description of the contents/purpose of the folder. (Before running any tools, make sure to ask the user for permission to run them):
"""

DESCRIBE_TEMPLATE_SHORT = """
You are a software developer. You have made some changes to the codebase. Please review the provided files and generate a description of the contents/purpose of the folder. (Keep it short and concise):
"""
...
def get_filtered_file_paths():
    ...
    try:
        is_git_repo = subprocess.run(['git', 'rev-parse', '--is-inside-work-tree'], capture_output=True, text=True).stdout.strip()
        if is_git_repo != 'true':
            cmd = 'find . -type f'
        else:
...
def get_detailed_diff(cached=True, max_tokens=100000):
...
def get_detailed_diff_of_files(files, max_tokens=100000):
...
def main(mode, llm, short, max_tokens):
    try:
        ...
        if mode == 'readme':
            ...
            try:
                ...
                detailed_diff = get_detailed_diff(cached, max_tokens=max_tokens)
                ...
                if mode == 'pr':
                    ...
                    detailed_diff = get_detailed_diff_of_files(top_selected_diff_files, max_tokens=max_tokens)
