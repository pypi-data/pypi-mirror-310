...
class LLMTask:
    ...
    def process(self, text: str, extra_instructions: str = None):
        """Default process function for calling the model to respond"""
    ...
    def invoke(self):
        """Overridable function for calling and handling the task"""
        return self.process("")


def single_instruction(instruction: str, text: str):
    """
    Performs a single response from the model using the 'instructions' as the system prompt and the 'text' as the user input
    """
    instance.conversation = Conversation()
    instance.system(instruction)
    instance.user(text)
    instance.chat()
    return instance.response
...
def make_title(text: str, target_word_count: int = 6, instructions: str = None):
    ...
    system_prompt = f"""Respond to the user by generating a concise title, ensuring it is no longer than {
        target_word_count} words, with an emphasis on keeping it around {int(target_word_count * 0.5)} words whenever possible; focus on crafting a strong, clear, and human-like identifier that could naturally serve as a headline or label, and do not include any formatting, quotation marks, or special symbols in the response.. You are expected to fully avoid responding with any text that references our task. DO NOT write any text that states that this is a title response"""
    ...
    return single_instruction(instruction=system_prompt, text=text)
...
def make_summary(text: str, target_word_count: int = 30, instructions: str = None):
    ...
    system_prompt = f"""Respond to the user by creating a concise summary that captures the essence of the input text, ensuring it is no longer than {
        target_word_count} words, with an emphasis on keeping it around {int(target_word_count * 0.5)} words whenever possible; the summary must always be shorter than the original input text and should clearly and effectively convey the main idea without including any formatting, quotation marks, or special symbols in the response. You are expected to fully avoid responding with any text that references our task. DO NOT write any text that states that this is a summary response"""
...
You are expected to treat any response from the user as the input text and not a conversation. Avoid any conversation."""
    ...
    response = single_instruction(instruction=system_prompt, text=text)

    # Extract and parse tags from AI's response
    extracted_tags = extract_list_items_from_text(response)
