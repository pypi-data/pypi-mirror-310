...
# Get file path `blast_xml_results.xml`
BLAST_XML_FILE = os.getenv("BLAST_XML_FILE") or Path(
    "~/.g2blast/blast_xml_results.xml"
).expanduser()
if not BLAST_XML_FILE.parent.exists():
    BLAST_XML_FILE.parent.mkdir(parents=True, exist_ok=True)
...
def analyze_blast_xml(
    ...
    sns.barplot(data=top_hits, x="bit_score", y="title", ax=ax4)
...
def main() -> None:
    fig, df = analyze_blast_xml(
        BLAST_XML_FILE,
        exclude=["M. tuberculosis", "Mycobacteroides tuberculosis"]
    )
