...
def get_code(files: List[str]) -> List[str]:
    """Reads the code from the given files with encoding detection.
    ...
    """
    contents = []
    # Common encodings to try before using chardet
    common_encodings = ['utf-8', 'ascii', 'iso-8859-1', 'utf-16', 'windows-1252']
    ...
    for file in files:
        try:
            ...
            for encoding in common_encodings:
                try:
                    contents.append(Path(file).read_text(encoding=encoding))
                    break
                except UnicodeDecodeError:
                    continue
            else:
                # Only use chardet if common encodings fail
                ...
                try:
                    ...
                    contents.append(Path(file).read_text(encoding='latin-1'))
