...
async def get_next(it: AsyncGenerator[T, U], resevoir=[], hint=None) -> Optional[T]:
    if resevoir:
...
async def inner_merge_join_o2o(
    ...
    while a is not None and b is not None:
        ...
        if ret == -1:
            a = await get_next(left, hint=b)
        elif ret == 1:
            b = await get_next(right, hint=a)
        elif ret == 0:
            yield a, b
            a, b = await asyncio.gather(get_next(left, hint=b), get_next(right, hint=a))
        else:
            raise Exception
...
async def inner_merge_join_o2m(
    cmp: ComparisonFunc[T],
    left: AsyncGenerator[T, U],
    right: AsyncGenerator[T, U],
) -> AsyncGenerator[Tuple[T, T], U]:
    """
    One to many join.
    """
    a, b = await asyncio.gather(get_next(left), get_next(right))
...
async def list(source: Union[AsyncGenerator[T, None], AsyncIterator[T]]) -> List[T]:
