...
class MSD80(ECU):
    ...
    def set_block(self, pydiabas: PyDIABAS, values: list[str]) -> Result | None:
        ...
        if result["JOB_STATUS"] == "ERROR_TABLE":
            raise BlockCreateError("Only values from MESSWERTETAB where NAME is '-' are supported, except as very last item in value list")
    ...
    def read_block(self, pydiabas) -> Result | None:
        ...
        if result["JOB_STATUS"] == "ERROR_ECU_CONDITIONS_NOT_CORRECT_OR_REQUEST_SEQUENCE_ERROR":
