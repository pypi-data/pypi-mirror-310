...
def create_task(promptx, token_aws, content_id, jwt_token, enhance, model):
    ...
    payload = {
        "input": {
            "prompts": [
                {"type": "text", "content": f"{promptx}"},
                {"type": "image", "content": f"ssupload:?id={content_id}", "src_img": f"ssupload:?id={content_id}"}
            ],
            "enhance": enhance
        },
        "type": "img2video",
...
def gen_image_to_video(prompt, enhance, model):
    ...
    #parser.add_argument('--model', type=str, required=True, help='Modelo a usar (vidu-1 o vidu-high-performance)')
    ...
    if file_id and jwt_token:
        try:
            ...
            task_id = create_task(prompt, token_swt, file_id, jwt_token, enhance, model)
