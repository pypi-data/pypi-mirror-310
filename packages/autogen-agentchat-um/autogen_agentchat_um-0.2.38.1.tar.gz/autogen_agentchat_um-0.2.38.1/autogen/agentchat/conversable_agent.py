...
class ConversableAgent(LLMAgent):
    ...
    def __init__(
        ...
        # print(f"str(type(self)) = {str(type(self))}")
        if "<class 'core.autogen.agentchat.caller_agent.CallerAgent'>" != str(type(self)):
            #print(f"not CallerAgent in init : {str(type(self))}, self: {self}")
            self._validate_llm_config(llm_config)
        # else:
        #     print(f"is CallerAgent: {str(type(self))}")
        #     # 设置 llm_config 、 client
    ...
    def update_tool_signature(self, tool_sig: Union[str, Dict], is_remove: None):
        ...
        ## zxc callerAgent 直接跳过
        if "<class 'core.autogen.agentchat.caller_agent.CallerAgent'>" != str(type(self)):
            #print(f"not CallerAgent in update_tool_signature : {str(type(self))}, self: {self}")
            self.client = OpenAIWrapper(**self.llm_config)
        else:
            self.client = OpenAIWrapper(**self.llm_config)
            #print(f"CallerAgent in update_tool_signature : {str(type(self))}, self.llm_config: {self.llm_config}")
...
def format_history(messages: List[dict]):
    # # 需要将tool的内容全部去掉，否则选择speaker时会报错 https://github.com/microsoft/autogen/issues/3799
