...
class ProdDataManager:
    ...
    def createMDSubStructure(self, metadata, basepath, program_category, output_type):
        """create meta data structure"""
        ...
        for key, value in collections.OrderedDict(
            ...
            path = os.path.join(basepath, self._formatPath(value))
    ...
    def createMDSubDirectory(self, metadata, base_path, program_category, trans_id):
        """create meta data structure"""
        ...
        # Create subdir according to the output_type (Data, Log, Model) and set MD
        transformation_path = path
        for output_type in ["Data", "Log", "Model"]:
            path = os.path.join(transformation_path, output_type)
            res = self.fc.createDirectory(path)
            if not res["OK"]:
                return res
