...
def create_output_dir(workflow_config, workflow_element):
    ...
    for process_program in ["tel_sim_prog", "analysis_prog"]:
        if process_program in output_metadata:
            ...
            res = prod_dm.createMDTopDirectory(meta_data_json, base_path, program_category)
            path = res["Value"]
...
def create_output_subdir(prod_name, trans_list, trans_client, workflow_element_dict):
    ...
    for trans in trans_list:
        ...
        step_name = trans_name.split("_")[1] + "_" + trans_name.split("_")[2]
    ...
    for workflow_element, output_dir_dict in workflow_element_dict.items():
        ...
        base_path = workflow_element.job.base_path
        print(base_path)

        for program_category in ["tel_sim", "analysis_prog"]:
            if program_category in output_metadata:
                res = prod_dm.createMDSubDirectory(meta_data_json, path, program_category, trans_id)
                print(res)
...
def build_workflow(workflow_config, prod_sys_client, prod_name, mode):
    ...
    for workflow_step in workflow_config["ProdSteps"]:
        ...
        step_name = "Step" + str(workflow_step["ID"]) + "_" + workflow_step["job_config"]["type"]
        output_dir_dict = {}
        output_dir_dict[step_name] = path
        workflow_element_dict[workflow_element] = output_dir_dict
        print(output_dir_dict)
...
def main():
    ...
    workflow_element_dict = build_workflow(workflow_config, prod_sys_client, prod_name, mode)
    ...
    if mode.lower() == "ps":
        ...
        if res["OK"]:
            ...
            create_output_subdir(prod_name, trans_list, trans_client, workflow_element_dict)
        else:
            DIRAC.gLogger.error(res["Message"])
            DIRAC.exit(-1)
