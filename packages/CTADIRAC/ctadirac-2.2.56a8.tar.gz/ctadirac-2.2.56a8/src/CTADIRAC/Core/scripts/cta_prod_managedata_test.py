...
def main():
    ...
    # Create MD structure
    if update_ts:
        result = prod_dm.getTransformationPath(
            metadata, base_path, program_category)
        if result["OK"]:
            path = result["Value"]
        #result = prod_dm.createMDSubStructure(
        ...
        #        if not res["OK"]:
        #            DIRAC.gLogger.warn(
        #                f"Failed to set file status to PROBLEMATIC {lfn}"
        #            )
        #    return result
    # This will be removed later. Just kept it here to make sure MC simulation keep running
    else:
        # Create MD structure
