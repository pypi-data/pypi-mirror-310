...
class JinaReaderTools(Toolkit):
    def __init__(self, api_key: Optional[str] = getenv("JINA_API_KEY"), base_url: str = "https://r.jina.ai/",
                 search_url: str = "https://s.jina.ai/", max_content_length: int = 0, timeout: Optional[int] = None,
                 read_url: bool = True, search_query: bool = False, ):
        super().__init__(name="jina_reader_tools")

        self.config: JinaReaderToolsConfig = JinaReaderToolsConfig(api_key=api_key, base_url=base_url,
    ...
    def read_url(self, url: str) -> str:
        ...
        logging.info(f"Reading URL: {full_url}")
        try:
            response = requests.get(full_url, headers=self._get_headers(), timeout=self.config.timeout)
    ...
    def search_query(self, query: str) -> str:
        ...
        logging.info(f"Performing search: {full_url}")
        try:
            response = requests.get(full_url, headers=self._get_headers(), timeout=self.config.timeout)
            response.raise_for_status()
    ...
    def _get_headers(self) -> Dict[str, str]:
        headers = {"Accept": "application/json", "X-With-Links-Summary": "true", "X-With-Images-Summary": "true", }
