...
from duowen_agent.utils.json_schema import get_json_schema
...
class Function(BaseModel):
    ...
    def from_callable(cls, c: Callable) -> "Function":
        ...
        try:
            # logging.info(f"Getting type hints for {c}")
            type_hints = get_type_hints(c)
            # logging.info(f"Type hints for {c}: {type_hints}")
            # logging.info(f"Getting JSON schema for {type_hints}")
            parameters = get_json_schema(
                type_hints)  # logging.info(f"JSON schema for {c}: {parameters}")  # logging.debug(f"Type hints for {c.__name__}: {type_hints}")
        except Exception as e:
            logging.warning(f"Could not parse args for {c.__name__}: {e}")

        return cls(name=c.__name__, description=getdoc(c), parameters=parameters, entrypoint=validate_call(c), )
    ...
    def get_definition_for_prompt(self) -> Optional[str]:
        ...
        function_info = {"name": self.name, "description": self.description,
            "arguments": self.parameters.get("properties", {}), "returns": returns, }
...
class FunctionCall(BaseModel):
    ...
    def execute(self) -> bool:
        ...
        logging.debug(f"Running: {self.get_call_str()}")
        ...
        if self.arguments is None:
            try:
                ...
                logging.warning(f"Could not run function {self.get_call_str()}")
                logging.exception(e)
                self.error = str(e)
                return False
