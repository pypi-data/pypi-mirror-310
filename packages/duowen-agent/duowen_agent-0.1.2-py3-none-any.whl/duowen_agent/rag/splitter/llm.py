...
class Sections:
    ...
    def __init__(self, llm_instance: OpenAIChat, document: str, max_characters: int = 40000):
...
[important] Respond with JSON code only without any explanations and comments. -- just the JSON code.'''
    ...
    def run(self, **kwargs):
        max_iterations = 2 * (len(self.document) // self.max_characters + 1)
        document_lines = self.get_document_lines(self.document)
        start_line = 0
        all_sections = []
...
[important] Output all in Chinese.
    ...
    def chunk(self, document: str, chunk_size: int = 600, split_max_characters: int = 40000, document_title: str = None,
        ...
        sections = Sections(llm_instance=self.llm_instance, document=document,
                            max_characters=split_max_characters).run()
