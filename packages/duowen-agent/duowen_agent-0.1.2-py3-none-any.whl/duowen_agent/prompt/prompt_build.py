...
class GeneralPromptBuilder:
    ...
    def __init__(self, instruction: Union[StringTemplate, str], step: Optional[Union[StringTemplate, str]] = None,
        ...
        self.sample = sample
        self.note = note
        if user_input and "user_input" in user_input.variables:
    ...
    def load(cls, name: str = None) -> 'GeneralPromptBuilder':
        ...
        if os.path.exists(f"{practice_dir}{name}.yaml"):
            with open(practice_dir + name + '.yaml', 'r') as f:
                data = yaml.load(f, Loader=yaml.FullLoader)
                return cls.load_dict(data)
        else:
            raise FileNotFoundError(f"GeneralPromptBuilder load [{name}] not found")
