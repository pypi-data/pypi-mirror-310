...
def test_load_gen(source, year, table_type, nrows, agency):
	if check_for_dataset(source, table_type):
		...
		offset = 0
		k = 0
		# df_iter = pd.DataFrame(columns=df.columns)
		for t in src.load_iter(table_type, year, nbatch=nrows, force=True, agency=agency):
			...
			# df_iter = pd.concat([df_iter, df2], ignore_index=True)
			offset+=len(t.table)
			k+=1
			if k>=max_iter:
				break

		# assert df_iter.shape == df.shape
		# if not df_iter.equals(df):
		# 	# Try to match rows
		# 	unmatched = pd.Series(True, index=df_iter.index)
		# 	for k in df.index:
		# 		df_cur = df_iter[unmatched].copy()
		# 		for c in df.columns:
		# 			if pd.isnull(df.loc[k,c]):
		# 				m = df_cur[c].isnull()
		# 			else:
		# 				m = df_cur[c]==df.loc[k,c]

		# 			df_cur = df_cur[m]
		# 			if len(df_cur)==0:
		# 				raise ValueError(f"No matches found for index {k}")
		# 			elif len(df_cur)==1:
		# 				break

		# 		for j in df_cur.index:
		# 			if df_cur.loc[j].equals(df.loc[k]):
		# 				unmatched[j] = False
		# 				break
		# 		else:
		# 			raise ValueError(f"No match found for row {k}")
