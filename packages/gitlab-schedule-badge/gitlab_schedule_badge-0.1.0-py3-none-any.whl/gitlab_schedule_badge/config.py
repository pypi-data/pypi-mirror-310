...
from gitlab_schedule_badge.defs import CgfKey
...
class Config:
        ...
        try:
            ...
            with path.open('rb') as f:
                ...
                for instance_cfg in instances_cfg:
                    instance_token = instance_cfg.get(CgfKey.TOKEN, None)
                    instance_ns = instance_cfg.get(CgfKey.NAMESPACE, '')
                    instance_url = instance_cfg.get(CgfKey.URL, '')
                    ...
                    if instance_url and instance_token:
                        ...
                        instance_cfg[CgfKey.FQPN] = instance_ref
    ...
    def cache_seconds(self) -> int | None:
        ...
        """
        raw_value = self._config.get(CgfKey.CACHE_SECONDS, None)
    ...
    def timeout(self) -> int | None:
        ...
        Returns:
            the timeout value
        """
        raw_value = self._config.get(CgfKey.TIMEOUT, None)
        if raw_value is None:
            return None
    ...
    def validate(self) -> bool:
        ...
        if self._instances:
            for entry in self._instances:
                ...
                aeo = instance_cfg.get(CgfKey.API_ENDPOINT_OVERRIDE)
                if aeo and purl:
                    try:
                        urljoin(entry, aeo)
                    except TypeError:
                ...
                cert = instance_cfg.get(CgfKey.CA_CERTIFICATE)
                if cert and not cert.exists():
                    err(f'(cfg) ca-certificate missing ({entry}): {cert}')
                    rv = False

                # validate session cookies
                cookies = instance_cfg.get(CgfKey.SESSION_COOKIES)
                if cookies:
                ...
                headers = instance_cfg.get(CgfKey.SESSION_HEADERS)
                if headers:
                ...
                # validate token
                if not instance_cfg[CgfKey.TOKEN].startswith('glpat-'):
    ...
    def _finalize(self) -> None:
        ...
        def kentry_convert(cfg: dict, key: CgfKey) -> None:
        ...
        for instance_cfg in self._instances.values():
            ...
            cert = instance_cfg.get(CgfKey.CA_CERTIFICATE)
            if cert:
                instance_cfg[CgfKey.CA_CERTIFICATE] = Path(cert)

            # ensure url and api-endpoint override ends with a forward flash
            instance_url = instance_cfg.get(CgfKey.URL)
            if not instance_url.endswith('/'):
                instance_cfg[CgfKey.URL] = f'{instance_url}/'

            aeo = instance_cfg.get(CgfKey.API_ENDPOINT_OVERRIDE)
            if isinstance(aeo, str) and not aeo.endswith('/'):
                instance_cfg[CgfKey.API_ENDPOINT_OVERRIDE] = f'{aeo}/'

            # ensure session cookies/headers are made into simple dictionaries
            kentry_convert(instance_cfg, CgfKey.SESSION_COOKIES)
            kentry_convert(instance_cfg, CgfKey.SESSION_HEADERS)
