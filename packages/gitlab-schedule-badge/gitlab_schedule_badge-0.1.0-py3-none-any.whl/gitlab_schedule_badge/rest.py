...
class Rest:
    def __init__(self, cfg: Config, instance: dict) -> None:
        ...
        """
        api_endpoint_override = instance.get(CgfKey.API_ENDPOINT_OVERRIDE)
        ca_certificate = instance.get(CgfKey.CA_CERTIFICATE)
        ignore_tls = instance.get(CgfKey.IGNORE_TLS)
        instance_url = instance.get(CgfKey.URL)
        namespace = instance.get(CgfKey.NAMESPACE)
        session_cookies = instance.get(CgfKey.SESSION_COOKIES)
        session_headers = instance.get(CgfKey.SESSION_HEADERS)
        token = instance.get(CgfKey.TOKEN)
