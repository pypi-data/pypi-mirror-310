...
def build_httpd(state: State) -> Flask:
    ...
    def badge(bid: str | None = None, text: str | None = None) -> \
        ...
        try:
            status, web_url = pending_result.result()
        ...
        banner_text = text if text else 'Build'
    ...
    def find_project() -> \
        ...
        try:
            for entry in pipeline_schedules:
                status, web_url = query_pipeline_schedule(api, entry)
                entry.status = status
                entry.url = web_url
        except QueryError as exc:
            return jsonify({'message': str(exc)}), exc.return_code
        ...
        hosted_url = request.base_url[:-len(request.path)]
        ...
        for entry in pipeline_schedules:
            ...
            pipeline_schedules_entries.append({
                'badge': badge_id,
                'badge_url': f'{hosted_url}/badge/{badge_id}',
