...
class ganada:
	...
	def search_nth_word_after_del_x07(self, input_no, input_text=""):
		...
		result = self.jf.search_nth_result_with_re_sql("\S+", input_no, all_text)
		# 현재 커서의 위치를 읽어온다
		return result
	...
	def search_nth_word_including_list_format(self, input_no, input_text=""):
		...
		temp = self.jf.search_nth_result_with_re_sql("\S+", input_no, changed_all_text)
		...
		for no in range(len(listformat_no_list) + 1):
			count_format = 0
			temp = self.jf.search_nth_result_with_re_sql("\S+", input_no - no, changed_all_text)
			self.select_xy(temp[0][1], temp[0][2])
			cur_para_no = self.get_para_no_at_selection_start()
			...
			if input_no - no + count_format == word_no_as_screen:
				result = self.jf.search_nth_result_with_re_sql("\S+", input_no - no, changed_all_text)
				return result
