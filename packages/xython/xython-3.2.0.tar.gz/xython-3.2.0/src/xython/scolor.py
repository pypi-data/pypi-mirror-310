...
class scolor:
	...
	def change_excel56enum_to_color_name(self, input_excel56enum):
		result = self.change_excel56enum_to_color_kor(input_excel56enum)
		return result

	def change_excel56enum_to_color_kor(self, input_excel56enum):
		"""
		엑셀 56색의 번호 -> 한글 색이름

		:param input_excel56enum:
		:return:
		"""
		result = self.vars["excel56_vs_color_kor"][int(input_excel56enum)]
		return result

	def change_excel56enum_to_rgb(self, i_no):
		"""
		엑셀 56색의 번호 -> rgb 리스트
		...
		:return:
		"""
		result = self.vars["rgb_56_for_excel"][int(i_no)]
		return result

	def move_hsl_to_bright_up(self, hsl, strong_level=50):
		"""
	...
	def move_hsl_to_dark_up(self, hsl, strong_level=0.5):
		"""
	...
	def move_hsl_to_gray_up(self, hsl, strong_level=0.5):
		"""
		입력받은 hsl값을 어두운 쪽으로 이동시키는것
	...
	def change_hsl_to_high_bright(self, hsl):
		"""
		입력된 hsl값 -> 고명도로 바꾸는 것
		"""
		hsl[2] = 80
	...
	def change_hsl_to_high_color(self, hsl):
		"""
		입력된 hsl값 -> 고채도로 바꾸는 것
		"""
		hsl[1] = 80
	...
	def change_scolor_to_pastel_style_by_1to3(self, i_scolor, level1to3=2):
		"""
		...
		hsl = self.change_scolor_to_hsl(i_scolor)
		h, s, l = hsl
	...
	def change_hsl_to_pastel_style(self, hsl, strong_level=0.5):
		"""
	...
	def change_hsl_to_vivid_level1(self, hsl, strong_level=0.5):
		"""
		level1 : 0~1사이의값
		입력받은 hsl값을 어두운 쪽으로 이동시키는것
		bright = [100,100], sharp = [50,100], graish = [100,0], dark = [0,0], black = [50, 0]
	...
	def get_n_rgb_set_by_hsl_to_10_rgb_set_step_by_s(self, hsl, step=10):
		"""
	...
	def change_hsl_to_2near_bo_rgb(self, hsl, h_step=36):
		...
		"""
		result =[]
		hsl_set = self.change_hsl_to_2near_bo_hsl(hsl, h_step=36)
		for one_hsl in hsl_set:
			result.append(self.change_hsl_to_rgb(one_hsl))
		return result

	def change_hsl_to_2near_rgb_step_by_h(self, hsl, h_step=36):
		"""
	...
	def get_4_rgb_set_for_input_hsl_step_by_90_h(self, hsl):
		"""
	...
	def change_rgb_to_close_excel56enum(self, input_rgb):
		"""
	...
	def change_rgbint_to_color_name(self, rgbint):
		...
		try:
			...
			color_index = self.change_rgb_to_close_excel56enum(rgblist)
			colorname = self.change_excel56enum_to_color_name(color_index)
		except:
			colorname = None
		return colorname
	...
	def change_scolor_to_anycolor(self, scolor_i):
		result ={}
		rgbint = self.change_scolor_to_rgbint(scolor_i)
		rgb= self.change_scolor_to_rgb(scolor_i)
		hsl= self._change_scolor_to_hsl(scolor_i)
		hex = self.change_scolor_to_hex(scolor_i)
		excel56 = self.change_rgb_to_close_excel56enum(rgb)
	...
	def change_scolor_to_close_excel56enum(self, input_scolor):
		"""
		...
		result = self.change_rgb_to_close_excel56enum(rgb_value)
		return result
	...
	def change_scolor_to_vivid_level1(self, input_scolor, step1=.3):
		"""
		** 추후 사용하지 말아주세요
		control의 의미는 입력의 자료형태를 그대로 유지하면서 미세 조정을 하는것인데, 이것은 다른 의미이므로 사용을 저제 하여 주시기 바랍니다
		...
		"""
		hsl = self._change_scolor_to_hsl(input_scolor)
		result = self.change_hsl_to_vivid_level1(hsl, step1)
		return result
	...
	def change_hsl_by_0to100_for_sl(self, hsl, step_no):
		"""
		hsl값을 명도를 조정하는 방법
		+，-로 조정을 하는것이다
	...
	def change_hsl_by_plusminus100_for_sl(self, hsl, s_step="++", l_step="++"):
		"""
	...
	def control_hsl_to_bright_high(self, hsl):
		"""
		입력된 hsl값 -> 중명도로 바꾸는 것
		"""
		hsl[2] = 90
	...
	def control_hsl_to_bright_low(self, hsl):
		"""
	...
	def control_hsl_to_bright_middle(self, hsl):
		"""
		입력된 hsl값 -> 중명도로 바꾸는 것
		"""
		hsl[2] = 50
	...
	def change_rgb_by_plusminus100_for_sl(self, input_rgb, s_step="++", l_step="++"):
		"""
	...
	def control_scolor_to_bright_style_as_level1(self, input_scolor, step1=.3):
		hsl = self._change_scolor_to_hsl(input_scolor)
	...
	def control_scolor_to_dark_style_as_level1(self, input_scolor, step1=.3):
		hsl = self._change_scolor_to_hsl(input_scolor)
		if step1 < 1: step1 = step1 * 100
	...
	def data_for_excel_rgb_46(self):
		"""
		엑셀 기본 46 rgb 값
		"""
	...
	def data_for_excel_rgb_56(self):
		"""
		엑셀 기본 56 rgb 값
		"""
	...
	def get_8_rgb_set_as_contrast_for_backgound_n_text(self):
		result = [ [[239, 68, 68],[255,255,255]],
					[[250, 163, 27],[0,0,0]],
					[[255, 240, 0],[0,0,0]],
					[[130, 195, 65],[0,0,0]],
	...
	def get_color_name_list_for_cool(self):
		"""
		차가운 색깔의 이름들
		"""
		result = ["파랑", "초록", "보라"]
	...
	def get_n_rgb_set_from_0_step_by_h(self, input_no=36):
		"""
	...
	def get_n_hsl_set_from_input_hsl_step_by_h(self, hsl, input_nth=10):
		result = []
	...
	def get_n_hsl_set_from_input_hsl_step_by_l(self, hsl, input_nth=10):
		result = []
	...
	def get_n_hsl_set_from_input_hsl_step_by_s(self, hsl, input_nth=10):
		result = []
		step = int(100/input_nth)
		for no in range(1, 101, step):
			temp = [hsl[0], no, hsl[2]]
	...
	def get_color_name_set_for_worm_style(self):
		"""
		따뜻한 색깔의 이름들
	...
	def get_n_rgb_set_between_scolor1_to_scolor2(self, scolor_1, scolor_2, step=10):
		"""
