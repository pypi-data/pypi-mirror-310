...
class anydb:
	...
	listdb_2d : series형태로된 리스트의 묶음, 일반 list_2d와는 가로세로가 반대로 되어있다
	"""
	...
	def append_df1_to_df2(self, df_i_1, df_i_2):
		"""
		...
		:return:
		"""
		result = pandas.concat([df_i_1, df_i_2])
		return result
	...
	def change_listdb_to_df(self, col_list="", listdb_2d=""):
		"""
		리스트 자료를 dataframe로 만드는것

		:param col_list: 제목리스트
		:param listdb_2d: 2차원 값리스트형
		:return: dataframe로 바꾼것
		"""
		checked_list_2d = self.util.change_list_1d_to_list_2d(listdb_2d)
		# 컬럼의 이름이 없거나하면 기본적인 이름을 만드는 것이다
		checked_col_list = self.check_input_data(col_list, listdb_2d)
		df_i = pandas.DataFrame(data=checked_list_2d, columns=checked_col_list)
		return df_i

	def change_listdb_to_df_rev1(self, listdb_2d, y_title_list="", x_title_list=""):
		"""
		...
		dic_l1d = self.change_listdb_to_dic_list_style(listdb_2d, y_title_list)
		if x_title_list == "":
	...
	def change_listdb_to_dic_list_style(self, listdb_2d, col_list=""):
		"""
		...
		temp = []
		result = {}
		if type(listdb_2d) == type([]):
			listdb_2d = self.util.check_list_2d(listdb_2d) #2차원이 아닐때 2차원으로 만들러 주는것
			listdb_2d = self.util.change_list_2d_to_same_max_len(listdb_2d) # 길이가 다를때 제일 긴것으로 똑같이 만들어 주는것
			#print("====> ", list_2d)
			# 별도로 column의 제목이 없다면, 1번부터 시작하는 번호를 넣어준다
			if col_list == "":
				for index, l1d in  enumerate(listdb_2d):
					temp.append(index+1)
				col_list = temp

			for index, l1d in enumerate(listdb_2d):
				result[col_list[index]] = listdb_2d[index]
		else:
			result = listdb_2d
		return result

	def change_df_to_dic(self, df_i, style="split"):
		"""
		...
		if not style in ["split", "list", 'series', 'records', 'index']:
			checked_style = "split"
		result = df_i.to_dict(checked_style)
		return result

	def change_df_to_list(self, df_i):
		"""
		df자료를 커럼과 값을 기준으로 나누어서 결과를 리스트로 돌려주는 것이다

		:param df_i: dataframe객체
		:return: [[컬럼이름1, 컬럼이름2,,,,], [자료1], [자료2]....]
		"""
		col_list = df_i.columns.values.tolist()
		value_list = df_i.values.tolist()
	...
	def change_list_2d_to_listdb(self, list_2d):
		"""
		...
		listdb_2d = self.util.change_xylist_to_yxlist(list_2d)
		return listdb_2d

	def change_list_2d_to_listdb_n_title(self, list_2d, first_is_title = True):
		"""
	...
	def change_sqlite_table_name(self, db_name, table_name_old, table_name_new):
		"""
		현재 db에서 테이블 이름 변경
	...
	def change_sqlite_table_to_df(self, db_name, table_name ):
		...
		df_i = pandas.DataFrame.from_records(data=sql_result.fetchall(), columns=cols)
		return df_i
	...
	def check_x_index_in_df(self, df_i, input_index):
		title_list = df_i.index
		result = None
	...
	def check_x_index_in_df_old(self, df_i, input_index):
		"""
	...
	def check_y_index_in_df(self, df_i, input_index):
		"""
		index가 기본 index인 0부터 시작하는 것이 아닌 어떤 특정한 제목이 들어가 있는경우는
	...
	def delete_all_empty_y_in_df(self, df_i):
		"""
		dataframe의 빈열을 삭제
		제목이 있는 경우에만 해야 문제가 없을것이다
		"""
		nan_value = float("NaN")
		df_i.replace(0, nan_value, inplace=True)
		df_i.replace("", nan_value, inplace=True)
		df_i.dropna(how="all", axis=1, inplace=True)
		return df_i

	def delete_empty_y_in_df(self, df_i):
		result = self.delete_all_empty_y_in_df(df_i)
		return result
	...
	def delete_empty_y_in_sqlite_table(self, db_name, table_name):
		...
		"""
		self.check_db_for_sqlite(db_name)
		y_name_all = self.read_all_ytitle_list_in_sqlite_table(table_name, db_name)
	...
	def delete_y_in_sqlite_table_by_ytitle_list(self, db_name, table_name, y_name_list):
		"""
		컬럼 삭제
	...
	def get_all_ytitle_in_sqlite(self, db_name, table_name):
		"""
		해당하는 테이의 컬럼구조를 갖고온다
		입력형태 : 테이블이름
		출력형태 : 컬럼이름들
	...
	def get_property_for_y_all_in_table_for_sqlite(self, db_name, table_name):
		"""
	...
	def get_ytitle_list_from_no1_to_no2_in_table_for_sqlite(self, db_name, table_name, offset=0, row_count=100):
		"""
	...
	def insert_y_in_listdb(self, input_listdb, input_y_name, input_yline_data):
		"""
	...
	def insert_y_in_sqlite_memory_db_by_ytitle_list(self, table_name, col_data_list_s):
		"""
		...
		all_exist_y_name = self.read_all_ytitle_list_in_sqlite_table(table_name)
	...
	def insert_y_in_sqlite_table_by_ytitle_list(self, db_name, table_name, col_data_list_s):
		"""
		(여러줄) 새로운 새로 컬럼을 만든다
	...
	def insert_ytitle_in_df(self, df_i, input_data):
		"""
		여러가지 형식으로 값을 넣어도 컬럼을 추가하는 방법입니다
		df_i.rename(columns={0: 'TEST', 1: 'ODI', 2: 'T20'}, inplace=True)
		...
		df_i.rename(columns=checked_changed_data, inplace=True)
		return df_i

	def insert_yy_in_sqlite_table(self, db_name, table_name, col_data_list_s):
		"""
		(여러줄) 새로운 새로 컬럼을 만든다
		col_data_list_s : [["이름1","int"],["이름2","text"]]
		["이름2",""] => ["이름2","text"]
	...
	def make_sql_text_for_insert_by_y_names(self, table_name, col_list):
		"""
		...
		:return:
		"""
		sql_columns = self.util.change_list_1d_to_one_text_by_chain_word(col_list, ", ")
		sql_values = "?," * len(col_list)
		result = "insert into %s (%s) values (%s)" % (table_name, sql_columns, sql_values[:-1])
		return result

	def make_sql_text_for_new_column_by_column_name_list(self, table_name, col_list):
		result = self.make_sql_text_for_insert_by_y_names(table_name, col_list)
		return result

	def make_sql_text_from_dic_data(self, table_name, input_dic):
		"""
	...
	def make_table_for_sqlite_memory_db(self, table_name):
		"""
	...
	def make_table_in_db_for_sqlite(self, db_name, table_name):
		"""
	...
	def read_all_table_name_in_sqlite_db(self, db_name=""):
		"""
	...
	def read_all_ytitle_list_in_sqlite_memory_db(self, table_name):
		"""
	...
	def read_all_ytitle_list_in_sqlite_table(self, table_name):
		"""
	...
	def read_value_in_df_by_name(self, df_i, x, y):
		"""
		...
		exec("self.result = df_i.loc[{}, {}]".format(temp[0], temp[1]))
		return self.result

	def read_value_in_df_by_no(self, df_i, x, y):
		"""
		...
		exec("self.result = df_i.iloc[{}, {}]".format(x_list, y_list))
		return self.result

	def read_value_in_df_by_xx(self, df_i, x):
		"""
		x의 라인들을 읽어온다
		...
		x_list = self.check_x_index_in_df(df_i, x)
		exec("self.result = df_i.loc[{}, {}]".format(x_list, ":"))
		return self.result

	def read_value_in_df_by_xy(self, df_i, xy=[0, 0]):
		"""
		(dataframe의 1개의 값 읽어오기)
		...
		:return:
		"""
		result = df_i.iat[int(xy[0]), int(xy[1])]
		return result

	def read_value_in_df_by_xyxy(self, df_i, xyxy):
		"""
		...
		x_list = self.check_x_index_in_df(df_i, x)
		y_list = self.check_y_index_in_df(df_i, y)
		#print(x_list, y_list)
		exec("self.result = df_i.loc[{}, {}]".format(x_list, y_list))
		return self.result

	def read_value_in_df_by_yy(self, df_i, y):
		"""
		y의 라인들을 읽어온다
		...
		"""
		y_list = self.check_y_index_in_df(df_i, y)
		exec("self.result = df_i.loc[{}, {}]".format(":", y_list))
		return self.result
	...
	def read_value_in_sqlite_memory_db_with_ytitle_by_xy(self, table_name, x_no, y_no):
		"""
	...
	def read_value_in_sqlite_memory_db_with_ytitle_by_xy_except_none_data(self, table_name, x_no, y_no):
		"""
	...
	def read_value_in_sqlite_table_by_ytitle_list(self, db_name, y_title_s="", condition="all"):
		"""
		...
		if y_title_s == "":
			...
			sql_columns = self.util.change_list_1d_to_one_text_by_chain_word(y_title_s, ", ")
		if condition == "all":
			lim_no = 100
		else:
			lim_no = condition
	...
	def read_value_in_table_as_dic_style_for_sqlite(self, table_name):
		"""
	...
	def read_value_in_table_as_dic_style_for_sqlite_except_none(self, table_name):
		"""
	...
	def read_xtitle_in_df_by_index(self, df_i, x_no=""):
		"""
	...
	def read_xxyy_line_in_df(self, df_i, x, y=""):
		"""
		숫자번호로 pandas의 dataframe의 일부를 불러오는 것
		단, 모든것을 문자로 넣어주어야 한다
		x=["1:2", "1~2"] ===> 1, 2열
	...
	def read_ytitle_in_df_by_index(self, df_i, y_no=""):
		"""
	...
	def write_df_to_excel(self, df_i, xy=[1, 1]):
		"""
	...
	def write_df_to_sqlite(self, db_name, table_name, df_i):
		"""
		...
		"""
		self.check_db_for_sqlite(db_name)
		df_i.to_sql(table_name, self.con)
	...
	def write_dic_to_sqlite(self, db_name, table_name, input_dic):
		...
		for one_col in list(input_dic[0].keys()):
			if not one_col in self.read_all_ytitle_list_in_sqlite_table(table_name, db_name):
				self.insert_yy_in_sqlite_table(table_name, [one_col], db_name)

		sql = self.make_sql_text_for_insert_by_y_names(table_name, list(input_dic[0].keys()))
		value_list = []
	...
	def write_list_1d_to_sqlite(self, db_name, table_name, y_title_s, input_list_1d):
		...
		"""
		self.check_db_for_sqlite(db_name)
		sql = self.make_sql_text_for_insert_by_y_names(table_name, y_title_s)
	...
	def write_value_in_sqlite(self, table_name, y_title_s, input_2=""):
		...
		:return:
		"""
		list_1d_dic = self.change_anydata_to_dic(y_title_s, input_2)
		sql_columns = ""
		sql_values = ""
	...
	def split_list_2d_as_data_xtitle_ytitle(self, list_2d, xtitle_len, ytitle_len):
		...
		for _ in range(total_y_len - ytitle_len):
			ytitle_l2d.append([])

		for l1d in list_2d[ytitle_len:]:
		...
		if ytitle_len:
			for index, l1d in enumerate(list_2d):
				if index < ytitle_len:
					for index2, one in enumerate(l1d[xtitle_len:]):
						ytitle_l2d[index2].append(one)
	...
	def make_2d_sample_data_in_excel(self):
		...
		excel.write_list_1d_from_cell("", [1, 2], y_title_list)
		for no1 in range(1, x_len + 1):
			for no2 in range(1, y_len + 1):
				excel.write_value_in_cell("", [no1 + 1, no2 + 1], no1 * 10 + no2)

		x_title_list = []
		for index in range(1, x_len + 1):
			x_title_list.append("줄_" + str(index))
		excel.write_list_1d_from_cell_as_yline("", [2, 1], x_title_list)

	def make_table_with_y_for_sqlite(self, db_name, table_name, column_data_list):
		"""
		...
		"""
		self.check_db_for_sqlite(db_name)
		sql_2 = self.make_sql_for_new_table_with_title_list_only(table_name, column_data_list)
		self.cursor.execute(sql_2)
		return sql_2

	def make_sql_for_new_table_with_title_list_only(self, table_name, column_data_list):
		"""
	...
	def write_list_2d_to_new_sqlite_table_by_data_only(self, db_name, table_name, list_2d):
		...
		sql_text = self.make_sql_for_new_table_with_title_list_only(table_name, title_list)
	...
	def make_listdb_2d_with_data_type(self, line_len, y_len, title_list=[], type_list=[]):
		"""
		list_2d = [
	...
	def make_listdb_2d_as_simple(self, line_len, y_len):
		"""
		단순하게 아래와 같은 형태로 만든다
	...
	def delete_y_line_in_listdb_2d_by_no(self, l2d_i, input_no):
		for index in range(len(l2d_i)):
			del l2d_i[index][input_no]
		return l2d_i

	def insert_y_line_in_listdb_2d_by_no(self, l2d_i, input_no, title_i="", type_i=""):
		if not type_i:
			type_i = "any"
	...
	def add_new_line_in_listdb_2d(self, l2d_i, line_data=[]):
		temp = []
		new_no = l2d_i[-1][0] + 1
	...
	def delete_x_line_in_listdb_2d_by_no(self, l2d_i, input_no):
		del l2d_i[input_no]
		return l2d_i

	def change_value_for_listdb_2d_at_xy(self, l2d_i, x, y, value):
		l2d_i[x][y] = value
		return l2d_i
