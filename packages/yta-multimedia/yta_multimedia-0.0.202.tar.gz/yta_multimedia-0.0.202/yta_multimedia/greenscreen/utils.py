...
def insert_video_into_moving_greescreen(greenscreen_google_drive_url: str, video: Union[VideoFileClip, CompositeVideoClip, ImageClip, str], output_filename: Union[str, None] = None):
    ...
    for index, frame in enumerate(frames):
        ...
        green_screen_clip = ImageClip(VideoFrameExtractor.get_frame_by_number(moving_gs, index), duration = imageclip.duration).fx(vfx.mask_color, color = frame_details['rgb_color'], thr = 100, s = 5)
    ...
    final = concatenate_videos(final_clips).set_audio(video.audio)
