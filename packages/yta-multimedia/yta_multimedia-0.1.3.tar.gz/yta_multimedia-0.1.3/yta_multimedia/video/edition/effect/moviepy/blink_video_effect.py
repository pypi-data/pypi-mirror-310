...
class BlinkVideoEffect(VideoEffect):
    ...
    def apply(clip: Union[str, VideoFileClip, VideoClip, CompositeVideoClip, ImageClip, ColorClip], color = None):
        ...
        clip = concatenate_videos([
            FadeOutVideoEffect.apply(clip.with_subclip(0, half_duration), duration = half_duration, color = color),
            FadeInVideoEffect.apply(clip.with_subclip(half_duration, clip.duration), duration = half_duration, color = color)
        ])
