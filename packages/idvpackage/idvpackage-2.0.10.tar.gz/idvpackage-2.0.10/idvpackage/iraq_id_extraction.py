...
def extract_name_fields_from_raw(text):
    try:
        ...
        try: 
            ...
            try:
                gender = translator.translate(gender_ar)
            except:
...
def iraq_front_id_extraction(client, image_data, front_id_text, front_id_text_description, front_translated_data, image_format):
    ...
    if gender:
        try:
            front_data['gender_ar'] = translator.translate(gender)
        except:
...
#         issuing_authority = ''
    ...
    if issuing_authority:
        try:
            issuing_authority_en = translator.translate(issuing_authority).upper()
        except:
            issuing_authority_en = GoogleTranslator('ar', 'en').translate(issuing_authority)

    if place_of_birth:
        try:
            place_of_birth_en = translator.translate(place_of_birth).upper()
