...
class ConnectDB:
    ...
    def __init__(self, database=None, SQLALCHEMY_DATABASE_URI=None):
        ...
        初始化数据库链接
            ...
            :param SQLALCHEMY_DATABASE_URI: 数据库链接参数
        """
        if SQLALCHEMY_DATABASE_URI and database:
            ...
            if data_url is None:
                ...
                data_url = input(f"请参考以下数据库链接示例填写您的数据库链接:\n{mysql_example}\n{postgresql_example}\n请输入您的数据库链接(环境变量SQLALCHEMY_DATABASE_URI的值):\n")
            ...
            raise ValueError("数据库链接参数SQLALCHEMY_DATABASE_URI和database不能为空")
        ...
        if self.db_url.startswith('mysql+pymysql:'):
            ...
            raise ValueError("数据库链接参数SQLALCHEMY_DATABASE_URI格式不正确")
