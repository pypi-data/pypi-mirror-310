...
class NetworkMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        # Keep existing frame metrics and add sequence tracking
        self.frame_intervals = deque(maxlen=30)
        self.frame_sizes = deque(maxlen=30)
        self.total_frames = 0
        self.frames_lost = 0  # Add this for counting lost frames
        self.last_frame_time = None
        self.last_sequence_number = -1  # Add this for sequence tracking
        self.frame_timestamps = deque(maxlen=30)
        
        # Keep all other existing initializations
        self._interfaces = {}
        self._prev_stats = {}
        ...
        with self.metrics_lock:
            ...
            if isinstance(data, bytes) and data.startswith(b'FRAME:'):
                try:
                    ...
                    header_end = data.index(b':', 6)  # Skip initial "FRAME:"
                    sequence_str = data[6:header_end].decode('utf-8')
                    current_sequence = int(sequence_str)
                    
                    # Calculate frames lost based on sequence gap
                ...
                # Keep existing frame processing
                self.total_frames += 1
                self.frame_sizes.append(size)
                self.frame_timestamps.append(timestamp)
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        # Keep all existing metric calculations
        with self.metrics_lock:
            ...
            metrics["frame_metrics"] = {
                ...
                "total_frames": self.total_frames,
                "frames_lost": self.frames_lost  # Add frames lost count
            }
            
            metrics["frames"] = {              
                "total": self.total_frames,
                "received": self.total_frames,
                "lost": self.frames_lost  # Add frames lost count
            }
