...
class SensorMetricsCalculator:
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            # Calculate throughput
            throughput = 0
            if self.total_bytes > 0 and self.last_packet_time is not None:
                actual_time = self.last_packet_time - self.start_time
                if actual_time > 0:
                    throughput = (self.total_bytes * 8) / (actual_time * 1_000_000)
