...
class NetworkMetricsCalculator:
    ...
    def record_data_sent(self, data: Any, timestamp: float) -> None:
        ...
        with self.metrics_lock:
            if isinstance(data, bytes):
                # Try to extract frame info from header
                try:
                    parts = data.split(b'|', 1)
                    if len(parts) == 2:
                        header, frame_data = parts
                        frame_num, total_frames = map(int, header.decode().split(','))
                        size = len(frame_data)
                        
                        self.total_frames += 1
                        if self.last_frame_number > 0 and frame_num > self.last_frame_number + 1:
                            frames_missed = frame_num - self.last_frame_number - 1
                            self.total_frames_lost += frames_missed
            ...
            # Rest of the original metrics recording
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            # Calculate frame loss percentage based on sequence gaps
            frame_loss_percent = (self.total_frames_lost / self.total_frames * 100) if self.total_frames > 0 else 0
            ...
            metrics["frame_metrics"] = {
                "frame_rate": {
                    ...
                    "frames_received": self.total_frames - self.total_frames_lost
                },
                "frame_size": {
                ...
                "latency": {
                    "min_ms": min(self.frame_intervals) * 1000 if self.frame_intervals else 0,
                    "max_ms": max(self.frame_intervals) * 1000 if self.frame_intervals else 0,
                    "avg_ms": statistics.mean(self.frame_intervals) * 1000 if self.frame_intervals else 0,
                    "jitter_ms": self._calculate_jitter(self.frame_timestamps)
                }
            }
            
            # Additional frame stats directly in metrics root
            metrics["frames"] = {
                "total": self.total_frames,
                "received": self.total_frames - self.total_frames_lost,
                "lost": self.total_frames_lost,
                "loss_percent": frame_loss_percent
            }
