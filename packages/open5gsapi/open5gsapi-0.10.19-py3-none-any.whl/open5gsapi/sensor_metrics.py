...
class SensorMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        self.send_timestamps = deque(maxlen=window_size)
        self.receive_timestamps = deque(maxlen=window_size)
    ...
    def record_data_received(self, data: Any, timestamp: float, content_type: str) -> None:
        with self.metrics_lock:
            ...
            # Ensure timestamps are recorded in the same order as sent
            if self.send_timestamps:
                self.receive_timestamps.append(timestamp)
            else:
                logger.warning("Received data without corresponding sent data. Skipping timestamp recording.")
                return
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            # Calculate throughput
            throughput_bps = (self.total_bytes * 8) / max(elapsed, 1)
            
            # Calculate latency statistics
            latencies = []
            for i in range(min(len(self.send_timestamps), len(self.receive_timestamps))):
                send_time = self.send_timestamps[i]
                receive_time = self.receive_timestamps[i]
                latency = receive_time - send_time
                if latency >= 0:
                    latencies.append(latency * 1000)
            ...
            jitter = 0
            if len(latencies) >= 2:
                delays = [latencies[i] - latencies[i-1] for i in range(1, len(latencies))]
                if delays:
                    avg_delay = sum(delays) / len(delays)
                    jitter = sum(abs(d - avg_delay) for d in delays) / len(delays)
