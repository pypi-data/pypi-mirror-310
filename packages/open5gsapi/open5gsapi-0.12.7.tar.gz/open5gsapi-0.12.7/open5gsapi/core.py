...
class Open5GS:
    ...
    def send_data(self, endpoint: str, data: Any, port_offset: int = 0) -> Dict[str, Any]:
        try:
            ...
            # Add send timestamp to data if it's sensor data
            if isinstance(data, (dict, list)) and isinstance(data, dict) and "sensor_id" in data:
                data_to_send = data.copy()  # Don't modify original data
        ...
        try:
            ...
            if 'application/json' in content_type:
                try:
                    ...
                    # Extract sensor data and handle UPF format
                    sensor_data = None
                    ...
                    if sensor_data and '_send_timestamp' in sensor_data:
                        # Calculate actual latency using embedded send timestamp
