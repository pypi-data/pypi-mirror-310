...
class NetworkMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        # Add these new attributes for latency and jitter
        ...
        # Interface monitoring (existing)
        self._interfaces = {}
        self._prev_stats = {}
        self._last_update = monotonic()
        self._update_interfaces()
        
        # Start monitoring thread (existing)
        ...
        with self.metrics_lock:
            try:
                ...
                if isinstance(data, dict) and '_send_timestamp' in data:
                    send_time = data['_send_timestamp']
                    frame_data = data['data'] if 'data' in data else data
                    size = len(frame_data)
                ...
                if isinstance(data, dict) and 'data' in data and isinstance(data['data'], bytes):
                    frame_data = data['data']
                else:
                    frame_data = data
                    
                if isinstance(frame_data, bytes) and frame_data.startswith(b'FRAME:'):
                    try:
                        header_end = frame_data.index(b':', 6)
                        sequence_str = frame_data[6:header_end].decode('utf-8')
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            metrics["frame_metrics"] = {
                ...
                "total_frames": self.total_frames,
                "frames_lost": self.total_frames_lost  # Use total_frames_lost for consistency
            }
