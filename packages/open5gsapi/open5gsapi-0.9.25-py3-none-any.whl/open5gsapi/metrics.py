...
class NetworkMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        # Add these to existing initializations
        self.frames_sent = 0
    ...
    def record_data_sent(self, data: Any, timestamp: float) -> None:
        ...
        with self.metrics_lock:
            # Handle frame metadata if present
            if isinstance(data, dict) and 'frame_number' in data:
                self.frames_sent += 1
                self.expected_frames = data['total_frames']
            ...
            if isinstance(data, dict) and 'frame_number' in data:
                ...
                if self.last_frame_number > 0:
                    # Calculate actual frame gap
                    frame_gap = current_frame_number - self.last_frame_number - 1
                    if frame_gap > 0:
                        self.frame_gaps.append(frame_gap)
                        self.total_frames_lost += frame_gap
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            if self.frames_sent > 0:
                frame_loss_percent = (self.total_frames_lost / self.frames_sent) * 100
            else:
            ...
            metrics = {
                ...
                "latency": {
                    ...
                    "avg_ms": (sum(self.frame_intervals) / len(self.frame_intervals) * 1000) if self.frame_intervals else 0,
