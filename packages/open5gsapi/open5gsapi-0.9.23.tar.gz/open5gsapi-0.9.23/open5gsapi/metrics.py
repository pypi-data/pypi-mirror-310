...
class NetworkMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        self.target_fps = 30  # Default, can be updated
    ...
    def set_target_fps(self, fps: float):
        """Update target FPS for the current stream"""
        self.target_fps = fps
        
    def start_stream(self):
        """Reset stream-specific metrics and start timing"""
        with self.metrics_lock:
            self.frames_sent = 0
            self.frames_received = 0
            self.stream_start_time = monotonic()
            self.last_frame_time = None
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        ...
        with self.metrics_lock:
            ...
            stream_elapsed = (current_time - self.stream_start_time) if self.stream_start_time else 0
            
            # Calculate expected frames based on elapsed time and target FPS
            expected_frames = int(stream_elapsed * self.target_fps) if stream_elapsed > 0 else 0
            
            # Calculate frame loss
            frames_lost = max(0, expected_frames - self.frames_received) if expected_frames > 0 else 0
            frame_loss_percent = (frames_lost / expected_frames * 100) if expected_frames > 0 else 0
