...
class SensorMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        # Track send/receive timestamps for latency calculation
        self.packet_pairs = {}  # Dictionary to track sent packets and their timestamps
    ...
    def record_data_sent(self, data: Any, timestamp: float) -> None:
        with self.metrics_lock:
            if isinstance(data, dict) and "sensor_id" in data:
                ...
                size = len(json.dumps(data).encode())  # Get actual packet size in bytes
                
                # Store send timestamp and size for this packet
                packet_key = f"{sensor_id}_{sequence}"
                self.packet_pairs[packet_key] = {
                    'send_time': timestamp,
                    'size': size
                }
                
                # Initialize or update sensor tracking
                if sensor_id not in self.readings_by_sensor:
                    self.readings_by_sensor[sensor_id] = {
                        "total_readings": 0,
                        "last_sequence": -1,
    ...
    def record_data_received(self, data: Any, timestamp: float, content_type: str) -> None:
        with self.metrics_lock:
            if isinstance(data, dict) and "sensor_id" in data:
                ...
                # If we have the corresponding send timestamp
                if packet_key in self.packet_pairs:
                    send_info = self.packet_pairs[packet_key]
                    ...
                    # Calculate throughput (Mbps) for this packet
                    if latency > 0:
                        throughput = (size * 8) / (latency / 1000) / 1_000_000  # Convert to Mbps
                        self.throughputs.append(throughput)
                    
                    # Clean up tracked packet
                    del self.packet_pairs[packet_key]
                    
                    # Track packet size
                    self.packet_sizes.append(size)
                
                # Update intervals for reading rate calculation
                if self.last_reading_time is not None:
                    interval = timestamp - self.last_reading_time
                    if interval > 0:
                        self.sensor_intervals.append(interval)
                ...
                # Update sensor-specific tracking
                if sensor_id in self.readings_by_sensor:
                    sensor_data = self.readings_by_sensor[sensor_id]
                    if sensor_data["last_sequence"] >= 0:
                        expected_sequence = sensor_data["last_sequence"] + 1
                        if sequence > expected_sequence:
                            sensor_data["readings_lost"] += sequence - expected_sequence
                    sensor_data["last_sequence"] = sequence
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            latency_stats = {
                ...
                "jitter_ms": self._calculate_jitter(self.readings_timestamps)
            }
            ...
            for sensor_id, sensor_data in self.readings_by_sensor.items():
                ...
                if readings:
                    metrics["sensor_metrics"]["by_sensor"][sensor_id] = {
                        ...
                        "min_value": min(readings, key=lambda x: self._get_value(x["value"])),
                        "max_value": max(readings, key=lambda x: self._get_value(x["value"]))
                    }

            # Add interface latency measurements
            self._update_interfaces()
            for iface, data in self._interfaces.items():
                if iface.startswith('uesimtun'):
                    latency = self._measure_interface_latency(iface)
    ...
    def _calculate_jitter(self, timestamps: deque) -> float:
        ...
        prev_timestamp = timestamps[0]
        for timestamp in list(timestamps)[1:]:
            delay = abs(timestamp - prev_timestamp) * 1000  # Convert to ms
            delays.append(delay)
            prev_timestamp = timestamp
        return statistics.stdev(delays) if len(delays) > 1 else 0.0

    def _get_value(self, data: Dict) -> float:
        # Helper method to extract numeric value from sensor data
        for key, value in data.items():
            if isinstance(value, (int, float)) and key not in ['sequence', 'timestamp', 'port']:
                return value
        return 0.0
    ...
    def _monitor_network(self):
        while True:
            try:
                self._update_interfaces()
                # Clean up old packet pairs (older than 30 seconds)
                current_time = monotonic()
                with self.metrics_lock:
                    self.packet_pairs = {
                        k: v for k, v in self.packet_pairs.items()
                        if current_time - v['send_time'] < 30
                    }
                sleep(1)
            except Exception as e:
                logger.error(f"Error in network monitor: {e}")
