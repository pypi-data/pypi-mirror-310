...
class Open5GS:
    ...
    def send_data(self, endpoint: str, data: Any, port_offset: int = 0) -> Dict[str, Any]:
        try:
            ...
            # Get packet_id for matching send/receive pairs
            ...
            if isinstance(data, bytes):
                ...
                # Sensor data - Add packet_id to track send/receive pairs
                data_with_id = data.copy()
                data_with_id['_packet_id'] = packet_id
                self.sensor_metrics.record_data_sent(data_with_id, current_time)
                data = data_with_id  # Use modified data for sending
            else:
                ...
                try:
                    ...
                    if isinstance(data, dict):
                        if 'sensor_id' in data:
                            ...
                            # Add source_ip to sensor data for metrics
                            sensor_data['source_ip'] = data.get('source_ip')
                    
                    if sensor_data:
                        # Add receive timestamp to sensor data for latency calculation
                        sensor_data['_receive_timestamp'] = current_time
