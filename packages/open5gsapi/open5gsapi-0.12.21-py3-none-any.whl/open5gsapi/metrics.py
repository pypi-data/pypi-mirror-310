...
class NetworkMetricsCalculator:
    def __init__(self, window_size: int = 100):
        ...
        self.frame_intervals = deque(maxlen=25)
        self.frame_sizes = deque(maxlen=25)
        self.total_frames = 0
        ...
        self.frame_timestamps = deque(maxlen=25)
            ...
            try:
                ...
                if isinstance(data, bytes) and data.startswith(b'FRAME:'):
                    try:
                        ...
                        # Calculate jitter for video/frame data
                        if len(self.packet_timestamps) > 1:
                            prev_timestamp = self.packet_timestamps[-1]
                            delay = abs(timestamp - prev_timestamp)
                            self.jitter += (delay - self.jitter) / 16.0
                            self.jitters.append(self.jitter)
                        else:
                            self.jitters.append(0)  # Initialize jitter to 0 for first frame
                ...
                # Calculate latency and jitter for sensor data
                if isinstance(data, dict) and '_send_timestamp' in data:
                    ...
                    if 0 < latency < 1000:  # Sanity check
                        ...
                        if len(self.latencies) > 1:
                            ...
                            self.jitters.append(0)  # Initialize jitter to 0 for first packet
