...
class SensorMetricsCalculator:
    ...
    def record_data_sent(self, data: Any, timestamp: float) -> None:
        with self.metrics_lock:
            ...
            if isinstance(data, dict) and "sensor_id" in data:
                ...
                reading_data = {
                    'timestamp': send_time,  # Use monotonic time
                    'value': data,
                    'size': size,
    ...
    def record_data_received(self, data: Any, timestamp: float, content_type: str) -> None:
        with self.metrics_lock:
            ...
            receive_time = time.monotonic()  # Record exact receive time
            ...
            if isinstance(data, dict) and "sensor_id" in data:
                ...
                # Find matching sent packet by sequence number
                for sent_reading in reversed(list(sensor_info["values"])):
                    if sent_reading.get('sequence') == sequence:
                        # Calculate true network latency
                        latency = (receive_time - sent_reading['timestamp']) * 1000  # Convert to ms
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            # Collect all latencies and calculate aggregate metrics
            for sensor_id, sensor_data in self.readings_by_sensor.items():
                ...
                # Sum up total bytes
                for reading in sensor_data["values"]:
                    total_bytes += reading.get('size', 0)

            # Calculate metrics
            metrics = {
                "throughput": {
                    "total_bps": (total_bytes * 8) / elapsed,  # bits per second
                    "bytes_transmitted": total_bytes,
                    "avg_packet_size": total_bytes / max(len(total_latencies), 1)
                },
                "latency": {
                    "min_ms": min(total_latencies) if total_latencies else 0,
                    "max_ms": max(total_latencies) if total_latencies else 0,
                    "avg_ms": sum(total_latencies) / len(total_latencies) if total_latencies else 0,
                    "jitter_ms": 0
                },
                "sensor_metrics": {
                    "reading_rate": {
            ...
            # Calculate jitter if we have latencies
            if len(total_latencies) >= 2:
                latency_diffs = [abs(total_latencies[i] - total_latencies[i-1]) 
                            for i in range(1, len(total_latencies))]
                metrics["latency"]["jitter_ms"] = sum(latency_diffs) / len(latency_diffs)
            ...
            for sensor_id, sensor_data in self.readings_by_sensor.items():
                ...
                metrics["sensor_metrics"]["by_sensor"][sensor_id] = {
                    "total_readings": sensor_data["total_readings"],
                    "readings_lost": sensor_data["readings_lost"],
                    "latest_value": sensor_data["values"][-1]["value"] if sensor_data["values"] else None,
                    "min_value": min(sensor_data["values"], key=lambda x: x['timestamp'])["value"] if sensor_data["values"] else None,
                    "max_value": max(sensor_data["values"], key=lambda x: x['timestamp'])["value"] if sensor_data["values"] else None,
                    "network_metrics": {
                        "latency_ms": {
                        ...
                        "packet_sizes": {
                            "min": min((r.get('size', 0) for r in sensor_data["values"]), default=0),
                            "max": max((r.get('size', 0) for r in sensor_data["values"]), default=0),
                            "avg": sum((r.get('size', 0) for r in sensor_data["values"])) / 
                                len(sensor_data["values"]) if sensor_data["values"] else 0
                        }
                    }
                }
