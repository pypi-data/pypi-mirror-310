...
class SensorMetricsCalculator:
    ...
    def record_data_sent(self, data: Any, timestamp: float) -> None:
        with self.metrics_lock:
            size = len(str(data).encode('utf-8')) if isinstance(data, (dict, list)) else len(data)  # Get actual byte size
            self.total_bytes += size
            self.total_packets += 1
            self.packet_sizes.append(size)
            self.packet_timestamps.append(timestamp)
        ...
        with self.metrics_lock:
            ...
            for sensor_id, sensor_data in self.readings_by_sensor.items():
                ...
                for i in range(1, len(readings)):
                    if ('value' in readings[i] and 'value' in readings[i-1] and 
                        'timestamp' in readings[i]['value'] and 'timestamp' in readings[i-1]['value']):
                        # Calculate time difference between consecutive packets
                        latency = (readings[i]['value']['timestamp'] - readings[i-1]['value']['timestamp']) * 1000  # ms
                        if latency > 0:  # Only consider valid latencies
                            latencies.append(latency)
                            packet_sizes.append(readings[i].get('size', 0))

            # Calculate throughput in bps, not Mbps
            total_bits = self.total_bytes * 8
            throughput_bps = total_bits / elapsed if elapsed > 0 else 0

            # Calculate jitter only if we have latencies
            jitter = 0
            if len(latencies) >= 2:
            ...
            metrics = {
                "throughput": {
                    "total_bps": throughput_bps,
                    "bytes_transmitted": self.total_bytes,
            ...
            for sensor_id, sensor_data in self.readings_by_sensor.items():
                ...
                if readings:
                    ...
                    for i in range(1, len(readings)):
                        if ('value' in readings[i] and 'value' in readings[i-1] and 
                            'timestamp' in readings[i]['value'] and 'timestamp' in readings[i-1]['value']):
                            latency = (readings[i]['value']['timestamp'] - readings[i-1]['value']['timestamp']) * 1000
                            if latency > 0:
                                sensor_latencies.append(latency)
                                sensor_sizes.append(readings[i].get('size', 0))
                    ...
                    metrics["sensor_metrics"]["by_sensor"][sensor_id] = {
                        ...
                        "network_metrics": {
                            "latency_ms": {
                                "min": min(sensor_latencies) if sensor_latencies else 0,
                                "max": max(sensor_latencies) if sensor_latencies else 0,
                                "avg": sum(sensor_latencies) / len(sensor_latencies) if sensor_latencies else 0
                            },
                            "packet_sizes": {
                                "min": min(sensor_sizes) if sensor_sizes else 0,
                                "max": max(sensor_sizes) if sensor_sizes else 0,
                                "avg": sum(sensor_sizes) / len(sensor_sizes) if sensor_sizes else 0
                            }
                        }
                    }
