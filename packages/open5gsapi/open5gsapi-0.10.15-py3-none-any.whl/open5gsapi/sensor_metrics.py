...
#     def record_data_sent(self, data: Any, timestamp: float) -> None:
#         with self.metrics_lock:
#             size = len(str(data).encode('utf-8')) if isinstance(data, (dict, list)) else len(data)
#             self.total_bytes += size
#             self.total_packets += 1
...
#                 # Calculate latency if we have the matching sent packet
#                 if key in self.packet_pairs:
#                     sent_data = self.packet_pairs[key]
#                     latency = (receive_time - sent_data['send_time']) * 1000  # Convert to ms
...
#                             sensor_info["readings_lost"] += sequence - expected_sequence
#                     sensor_info["last_sequence"] = sequence
                    
#                     # Clean up used packet pair
#                     del self.packet_pairs[key]
                
#                 # Update intervals for rate calculation
#                 if self.last_reading_time is not None:
#                     interval = timestamp - self.last_reading_time
#                     if interval > 0:
#                         self.sensor_intervals.append(interval)
...
#             # Calculate metrics from stored sensor data
#             for sensor_id, sensor_data in self.readings_by_sensor.items():
#                 sensor_latencies = [r.get('latency', 0) for r in sensor_data['values'] if 'latency' in r]
#                 all_latencies.extend(sensor_latencies)
                
#                 # Calculate total bytes and get timestamps for rate calculation
#                 for reading in sensor_data['values']:
#                     if 'size' in reading and 'timestamp' in reading:
#                         total_bytes += reading['size']
#                         recent_timestamps.append(reading['timestamp'])
            
#             # Calculate actual throughput based on recent readings
#             recent_timestamps = sorted(recent_timestamps)[-10:]  # Last 10 readings
#             if len(recent_timestamps) >= 2:
#                 time_window = recent_timestamps[-1] - recent_timestamps[0]
#                 current_bps = (total_bytes * 8) / time_window if time_window > 0 else 0
#             else:
#                 current_bps = 0

#             metrics = {
#                 "throughput": {
#                     "total_bps": current_bps,
#                     "bytes_transmitted": total_bytes,
#                     "avg_packet_size": total_bytes / max(len(all_latencies), 1)
#                 },
#                 "latency": {
...
#                 jitter = sum(abs(all_latencies[i] - all_latencies[i-1]) 
#                            for i in range(1, len(all_latencies))) / (len(all_latencies) - 1)
...
#                         "total_readings": sensor_data["total_readings"],
#                         "readings_lost": sensor_data["readings_lost"],
#                         "latest_value": sensor_data["values"][-1]["value"],
#                         "min_value": min(sensor_data["values"], key=lambda x: x["timestamp"])["value"],
#                         "max_value": max(sensor_data["values"], key=lambda x: x["timestamp"])["value"],
#                         "network_metrics": {
#                             "latency_ms": {
#                                 "min": min((x['latency'] for x in sensor_data["values"] if 'latency' in x), default=0),
#                                 "max": max((x['latency'] for x in sensor_data["values"] if 'latency' in x), default=0),
#                                 "avg": sum((x['latency'] for x in sensor_data["values"] if 'latency' in x)) / 
#                                       len([x for x in sensor_data["values"] if 'latency' in x]) if any('latency' in x for x in sensor_data["values"]) else 0
#                             },
#                             "packet_sizes": {
#                                 "min": min((x.get('size', 0) for x in sensor_data["values"])),
#                                 "max": max((x.get('size', 0) for x in sensor_data["values"])),
#                                 "avg": sum((x.get('size', 0) for x in sensor_data["values"])) / len(sensor_data["values"])
#                             }
#                         }
#                     }
