...
class SensorMetricsCalculator:
    ...
    def calculate_metrics(self) -> Dict[str, Any]:
        with self.metrics_lock:
            ...
            # Calculate raw data throughput
            total_bytes = self.total_bytes
            throughput_mbps = (total_bytes * 8) / (1_000_000 * max(elapsed, 1))  # Prevent division by zero
            
            # Calculate latency statistics
            latencies = []
            for metrics in self.readings_by_type.values():
                readings = list(metrics['readings'])
                for reading in readings:
                    if 'timestamp' in reading:
                        # Convert timestamp to monotonic time relative to start_time
                        receive_time = reading['timestamp']
                        if 'value' in reading and 'timestamp' in reading['value']:
                            send_time = reading['value']['timestamp']
                            latency = receive_time - send_time
                            if latency >= 0:  # Ignore negative latencies
                                latencies.append(latency * 1000)  # Convert to ms
            
            # Calculate jitter properly
            jitter = 0
            ...
            metrics = {
                "throughput": {
                    ...
                    "bytes_per_second": total_bytes / max(elapsed, 1)
                },
                "latency": {
                ...
                "sensor_metrics": {
                    "reading_rate": {
                        ...
                        "reading_interval_ms": statistics.mean(self.sensor_intervals) * 1000 if self.sensor_intervals else 0,
                        "total_readings": sum(metrics['total'] for metrics in self.readings_by_type.values()),
                        "readings_lost": sum(metrics['lost'] for metrics in self.readings_by_type.values())
                    },
                    "by_sensor": {}
                }
            }
            
            # Calculate per-sensor type metrics
            for sensor_type, sensor_data in self.readings_by_type.items():
                readings = list(sensor_data['readings'])
                if readings:
                    latest_values = [r['value'] for r in readings]
                    readings_per_second = len(readings) / max(elapsed, 1)
                    
                    metrics["sensor_metrics"]["by_sensor"][sensor_type] = {
                        "total_readings": sensor_data['total'],
                        "readings_lost": sensor_data['lost'],
                        "current_rate": readings_per_second,
                        "latest_value": latest_values[-1],
                        "min_value": min(latest_values, key=lambda x: x.get('timestamp', 0)),
                        "max_value": max(latest_values, key=lambda x: x.get('timestamp', 0))
                    }
