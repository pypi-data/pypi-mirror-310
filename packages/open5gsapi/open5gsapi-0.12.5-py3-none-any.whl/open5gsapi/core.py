...
class Open5GS:
    ...
    def send_data(self, endpoint: str, data: Any, port_offset: int = 0) -> Dict[str, Any]:
        try:
            ...
            # Record send timestamp BEFORE sending
            send_time = time.monotonic()
            
            if isinstance(data, bytes):
                # Video frame data
                self.metrics.record_data_sent(data, send_time)
            elif isinstance(data, (dict, list)) and isinstance(data, dict) and "sensor_id" in data:
                # For sensor data, add timestamp to the data
                data['_send_timestamp'] = send_time  # Add timestamp to data
                self.sensor_metrics.record_data_sent(data, send_time)
            else:
                # Default to video metrics for other data types
                self.metrics.record_data_sent(data, send_time)
    ...
    def receive_data(self, endpoint: str) -> Any:
        try:
            ...
            # Record receive timestamp immediately after getting response
            ...
            if 'application/json' in content_type:
                try:
                    ...
                    # Handle UPF response format where sensor data might be nested
                    sensor_data = None
                    ...
                    if sensor_data:
                        # Check if this data has a send timestamp
                        send_time = sensor_data.get('_send_timestamp')
                        if send_time:
                            # Calculate latency using timestamps
                            self.sensor_metrics.record_data_received(
                                sensor_data,
                                receive_time,
                                content_type
                            )
                        else:
                            logger.warning("Received sensor data without send timestamp")
                    else:
