...
class PacketMetrics:
    ...
    packet_size: int
    packet_type: str  # 'sensor' or 'streamer'
    source_ip: str
    sequence_number: int
...
class MetricsCollector:
    def __init__(self, window_size: int = 100):
        ...
        self.packets: deque[PacketMetrics] = deque(maxlen=window_size)
        self.frames: deque[FrameMetrics] = deque(maxlen=window_size)
        self.sequence_counter = 0
        self.frame_counter = 0
        self.lock = Lock()
        
    def record_packet(self, timestamp_sent: float, packet_size: int, 
                     packet_type: str, source_ip: str) -> int:
        with self.lock:
            self.sequence_counter += 1
            return self.sequence_counter
            
    def update_packet(self, sequence_number: int, timestamp_received: float):
        with self.lock:
            metrics = PacketMetrics(
                timestamp_sent=time.time(),
                timestamp_received=timestamp_received,
                packet_size=0,  # Will be updated when data is actually sent
                packet_type="",  # Will be updated when data is actually sent
                source_ip="",   # Will be updated when data is actually sent
                sequence_number=sequence_number
            )
            self.packets.append(metrics)
            
    def record_frame(self, timestamp_sent: float, frame_size: int, 
                    total_chunks: int, source_ip: str) -> int:
        with self.lock:
            self.frame_counter += 1
            metrics = FrameMetrics(
                timestamp_sent=timestamp_sent,
                timestamp_received=0,  # Will be updated when frame is complete
                packet_size=frame_size,
                packet_type='streamer',  # Updated to match API endpoint name
                source_ip=source_ip,
    ...
    def calculate_metrics(self) -> Dict[str, Dict[str, float]]:
        with self.lock:
            metrics: Dict[str, Dict[str, float]] = {
                ...
                'streamer': {  # Updated to match API endpoint name
                    'latency': 0.0,
                    'throughput': 0.0,
                    'jitter': 0.0,
                    'packet_loss': 0.0,
            ...
            sensor_packets = [p for p in self.packets if p.packet_type == 'sensor']
            if sensor_packets:
                latencies = [p.timestamp_received - p.timestamp_sent for p in sensor_packets]
                sizes = [p.packet_size for p in sensor_packets]
                
                metrics['sensor']['latency'] = np.mean(latencies) * 1000  # Convert to ms
                metrics['sensor']['jitter'] = np.std(latencies) * 1000 if len(latencies) > 1 else 0
                metrics['sensor']['throughput'] = np.sum(sizes) / (max(latencies) if latencies else 1) / 1024  # KB/s
                metrics['sensor']['packet_loss'] = (
                    1 - len(sensor_packets) / self.sequence_counter
                ) * 100 if self.sequence_counter > 0 else 0
            
            # Calculate streamer metrics
            if self.frames:
                frame_latencies = [f.timestamp_received - f.timestamp_sent for f in self.frames 
                                 if f.timestamp_received > 0]
                complete_frames = [f for f in self.frames if f.received_chunks == f.total_chunks]
                
                if frame_latencies:
                    metrics['streamer']['latency'] = np.mean(frame_latencies) * 1000
                    metrics['streamer']['jitter'] = np.std(frame_latencies) * 1000 if len(frame_latencies) > 1 else 0
                    
                    # Calculate FPS
                    if len(complete_frames) >= 2:
                        time_diff = complete_frames[-1].timestamp_received - complete_frames[0].timestamp_received
                        metrics['streamer']['fps'] = len(complete_frames) / time_diff if time_diff > 0 else 0
                    
                    # Calculate frames lost
                    metrics['streamer']['frames_lost'] = (
                        1 - len(complete_frames) / self.frame_counter
                    ) * 100 if self.frame_counter > 0 else 0
                    
                    # Calculate streaming throughput
                    total_size = sum(f.packet_size for f in complete_frames)
                    total_time = max(frame_latencies) if frame_latencies else 1
                    metrics['streamer']['throughput'] = total_size / total_time / 1024  # KB/s
...
class Open5GS:
    ...
    def send_data(self, endpoint: str, data: Any, port_offset: int = 0) -> Dict[str, Any]:
        try:
            ...
            timestamp_sent = time.time()
            packet_size = len(data) if isinstance(data, (bytes, str)) else len(str(data))
            ...
            # Record metrics
            sequence_number = self.metrics_collector.record_packet(
                timestamp_sent=timestamp_sent,
                packet_size=packet_size,
                packet_type='sensor' if isinstance(data, (dict, list)) else 'streamer',
                source_ip=url.split('/')[2]
            )
            
            self.metrics_collector.update_packet(
                sequence_number=sequence_number,
    ...
    def receive_data(self, endpoint: str) -> Any:
        try:
            ...
            if 'application/json' in content_type:
                data = response.json()
                if 'receive/sensor' in endpoint and isinstance(data, dict):
                    self.metrics_collector.update_packet(
                ...
                if 'receive/streamer' in endpoint:
                    self.metrics_collector.record_frame(
                        timestamp_sent=data.get('timestamp', time.time()),
                        frame_size=len(data),
                        total_chunks=1,
                        source_ip=endpoint.split('/')[2]
                    )
                return data
            else:
                return response.json()  # Default to JSON
    ...
    def get_metrics(self) -> Dict[str, Dict[str, float]]:
        """
        Get the current network performance metrics.
        
        Returns:
            Dict containing metrics for both sensor and streaming data:
            {
                'sensor': {
                    'latency': float (ms),
                    'throughput': float (KB/s),
                    'jitter': float (ms),
                    'packet_loss': float (%)
                },
                'streamer': {
