...
class NXtomo(NXobject):
    ...
    def node_is_nxtomo(node: h5py.Group) -> bool:
        ...
        if "NX_class" in node.attrs or "NXclass" in node.attrs:
            _logger.info(f"{node.name} is recognized as a nx class.")
        else:
            _logger.info(f"{node.name} isn't recognized as a nx class.")
            return False
        if "definition" in node.attrs and node.attrs["definition"].lower() == "nxtomo":
            _logger.info(f"{node.name} is recognized as an NXtomo class.")
            return True
        elif (
            "instrument" in node
