...
def sign(input_file, output_file, api_token, tenant_id, pki, user_id=None, email=None, environment='stg', pdf_password=''):
    """Sign document"""
    if environment == 'dev':
    ...
    res, msg = sign_pplg(input_file, output_file, api_token, tenant_id, pki, user_id, email, environment, pdf_password)
    ...
    if res:
        ...
        if msg == 'password_required':
            click.echo(f'''
                    Status: failure. \n 
                    PDF file's password is not correct. Please provide correct password.''', 
                err=True
            )
        else:
