...
class PdfSigner:
    ...
    async def async_digest_doc_for_post_signing(
        ...
        *,
        ...
        """
        click.echo(f"==> Step 3.9.1: {datetime.now()}")
        ...
        click.echo(f"==> Step 3.9.2: {datetime.now()}")
            
        validation_info = await signing_session.perform_presign_validation(
            pdf_out
        )
        
        click.echo(f"==> Step 3.9.3: {datetime.now()}")
        if bytes_reserved is None:
            estimation = signing_session.estimate_signature_container_size(
                validation_info=validation_info,
                tight=self.signature_meta.tight_size_estimates,
        ...
        click.echo(f"==> Step 3.9.4: {datetime.now()}")
        ...
        click.echo(f"==> Step 3.9.5: {datetime.now()}")
