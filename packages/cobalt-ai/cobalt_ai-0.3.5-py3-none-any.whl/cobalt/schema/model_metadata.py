...
class ModelMetadata:
    """Encapsulates various metadata about a model.

    Attributes:
        outcome_columns: List of strings indicating the outcome columns.
        prediction_columns: List of strings indicating the prediction columns.
        task: ModelTask denoting the task the model performs.
    ...
    @classmethod
    def from_dict(cls, data) -> ModelMetadata:
        data["task"] = ModelTask(data["task"])
        evaluation_metrics = data.get("evaluation_metrics")
        if evaluation_metrics:
            del data["evaluation_metrics"]
        instance = cls(**data)
    ...
    def performance_metrics(self) -> Dict[str, EvaluationMetric]:
        """Return the relevant performance metrics for this model's task.
        ...
        if self.outcome_column and self.prediction_column:
            if self.task == ModelTask.classification:
                elements = [
                    ClassificationInaccuracy(
                        self.prediction_column, self.outcome_column
                    ),
                    ClassificationAccuracy(self.prediction_column, self.outcome_column),
                ]

            elif self.task == ModelTask.regression and self:
                elements = [
                    AbsoluteError(self.prediction_column, self.outcome_column),
                    SquaredError(self.prediction_column, self.outcome_column),
                ]

        metrics = {element.get_key(): element for element in elements}
        return {**metrics, **self.evaluation_metrics}
    ...
    def get_confusion_matrix(
        ...
        if self.task != ModelTask.classification:
            raise ValueError(
                "Model Overviews are only available for classification models "
                "(model task set to 'classification')."
            )
    ...
    def __repr__(self):
        return (
            f"ModelMetadata(name={self.name}, "
            f"task={self.task}, "
