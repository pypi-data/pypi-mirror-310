...
class DatasetBase(ABC):
    ...
    def _write_image_columns_to_table(
        ...
        image_host_paths: Optional[Dict[str, str]] = None,
    ):
        ...
        for media_column in media_columns:
            ...
            if not media_column.is_remote:
                image_host_url = image_host_paths[media_column.host_directory]
    ...
    def create_rich_media_table(
        ...
        if media_columns:
            self._write_image_columns_to_table(
                media_columns, table_df, image_host_paths
            )
...
class CobaltDataset(DatasetBase):
    ...
    ## PERFORMANCE METRICS
    def _column_name_for_model_name(self, index: int, key: str) -> str:
        """Returns model name."""
        model = self.models[index]
        return f"{model.name}_{key}"
    ...
    def compute_model_performance_metrics(self):
        ...
        for i, model in enumerate(models):
            ...
            cols = []
            for _, metric in performance_metrics.items():
                stats = metric.calculate(self)

                for key in stats:
                    new_column_name = self._column_name_for_model_name(i, key)
                    self.set_column(new_column_name, stats[key])
                    cols.append(new_column_name)

            model.error_columns = cols
