...
class Workspace:
    ...
    def __init__(
        ...
        if media_columns:
            for mc in media_columns:
                if not mc.is_remote:
                    self.state.server_repo.add_server(media_columns[0].host_directory)
        self.state.server_repo.run_servers()
    ...
    def view_table(
        ...
        server_paths = {
            ...
            for server in self.state.server_repo.get_servers()
        }
        table_df = subset.create_rich_media_table(image_host_paths=server_paths)

        image_columns = [
            img_col.autoname_media_visualization_column()
            for img_col in subset.metadata.media_columns
        ]
