...
class UI:
    def __init__(
        ...
        self._override_container_styles()
    ...
    def _create_table(self, data: CobaltDataset):
        ...
        server_paths = {
            server.image_folder: server.img_host_url
            for server in self.ws.state.server_repo.get_servers()
        }
        table_df = data.create_rich_media_table(image_host_paths=server_paths)

        image_columns = [
            img_col.autoname_media_visualization_column()
            for img_col in data.metadata.media_columns
        ]
    ...
    def _prepare_visualization(self):
        ...
        self.user_groups = UserGroups(
            self.dataselector, self.visualization, self.ws.state, num_of_failure_groups
        )
        ...
        self.groups_container = v.Flex(
            children=[self.user_groups, self.failure_groups],
    ...
    def _override_container_styles(self):
        css = """
        <style>
            #notebook-container {
                width: 85% !important;
