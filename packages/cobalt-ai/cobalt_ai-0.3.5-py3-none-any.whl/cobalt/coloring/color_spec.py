...
class NumericColoringSpec(GraphColoringSpec):
    ...
    def construct_legend(
        ...
        def make_widget():
            ...
            legend = v.Col(
                class_="pa-3 pb-0 pt-7 ma-0",
                children=[
                    v.Row(
                        class_="pa-0 d-flex flex-nowrap",
                        no_gutters=True,
                        children=[gradient_bar, bounds],
                        style_="gap: 12px; line-height: 15px;",
                    ),
                    wrap_with_tooltip(truncate_str(name), f"Colored by: {name}"),
                ],
...
class CategoricalColoringSpec(GraphColoringSpec):
    ...
    def construct_legend(
        ...
        def make_widget():
            ...
            legend = v.Col(
                class_="pb-0 pt-7 ma-0 overflow-y-auto",
                children=[
                    v.Col(
...
def wrap_with_tooltip(value: str, tooltip_label: Optional[str] = None) -> v.Tooltip:
        ...
        v_slots=[
            {
                ...
                "children": v.Row(
                    class_="font-weight-medium ma-0 pt-2 pb-3",
                    justify="center",
                    children=[disp_value],
                    style_="position: sticky; bottom: 0;",
                    v_on="tooltipData.on",
                ),
            },
        ],
