...
class FeatureGenerator:
    def __init__(self, feature_mode: str, truth_parameters: dict[str, int] | None = None) -> None:
        ...
        self._total_truth_parameters = sum(self._truth_parameters.values())
    ...
    def execute_all(
        self, xf: np.ndarray, truth_in: dict[str, np.ndarray] | None = None
        ...
        if truth_in is not None:
            ...
            for name in truth_in:
                ...
                if truth_in[name].shape[1] != self._truth_parameters[name]:
                    raise ValueError(f"truth_in, '{name}', must have {self._truth_parameters[name]} parameters")

            flat_truth = np.concatenate(tuple(truth_in[name] for name in truth_in), axis=1)
        else:
            flat_truth = None
        ...
        for input_frame in range(input_frames):
            if flat_truth is not None:
                self.execute(xf[input_frame], flat_truth[input_frame])
            else:
        ...
        truth_out: dict[str, np.ndarray] = {}
        start_index = 0
        for name in self._truth_parameters:
            truth_out[name] = truth[:, :, start_index : start_index + self._truth_parameters[name]]
            start_index += self._truth_parameters[name]
