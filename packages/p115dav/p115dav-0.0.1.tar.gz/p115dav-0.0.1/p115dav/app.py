...
LIMIT 1;""", (share_code, id))
                ...
                if remains:
                    ...
                    for a in children:
                        try:
                            ...
                            mtime_groups[a["mtime"]] = {a["id"]: a}
                    his_it = iter(mtime_groups.items())
                    children = []
    ...
    _get_list_g = cycle([True, False]).__next__
        ...
        async with ID_TO_LIST_LOCK.setdefault(cid, Lock()):
            ...
            if not will_full_update:
                ...
                if isnan(ttl) or isinf(ttl) or  ttl < 0:
                    ...
                    updated_at = await to_thread(query, "SELECT updated_at FROM list WHERE id=?", id)
            ...
            count = 0
            payload = {
                "asc": 1, "cid": cid, "cur": 1, "custom_order": 1, "fc_mix": 0, "limit": 1_200, 
                "natsort": 1, "o": "file_name", "offset": offset, "show_dir": 1, 
            }
            resp = await client.fs_files_aps(payload, base_url=_get_list_g(), async_=True)
            payload["limit"] = page_size
            while True:
                check_response(resp)
                if cid and int(resp["path"][-1]["cid"]) != cid:
