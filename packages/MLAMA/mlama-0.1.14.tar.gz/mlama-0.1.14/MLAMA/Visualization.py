...
#as observed contains only test_y, so this works here with the prediction only
  ...
  for WAVE in  WAVES:
    ...
    for prediction_length in predictions:
      for delay in delay_list:
        ...
        real_STAT = metrics_df.loc[(metrics_df['wave'] == waveID) &  (metrics_df['prediction_length'] == prediction_length) &  (metrics_df['delay'] == delay)]['Observed_fit_train_pred_test_STAT'].values[0]#metric_row['Observed_fit_train_pred_test_STAT'].values[0]
        real_ML = metrics_df.loc[(metrics_df['wave'] == waveID) &  (metrics_df['prediction_length'] == prediction_length) &  (metrics_df['delay'] == delay)]['Observed_fit_train_pred_test_ML'].values[0]#metric_row['Observed_fit_train_pred_test_ML'].values[0]
        print(real_STAT.shape, rf_data.shape)
        #print(real_ML.shape)
        week_values_STAT = metrics_df.loc[(metrics_df['wave'] == waveID) &  (metrics_df['prediction_length'] == prediction_length) &  (metrics_df['delay'] == delay)]['weekSTAT'].values[0]#metric_row['weekSTAT'].values[0]
        week_values_ML = metrics_df.loc[(metrics_df['wave'] == waveID) &  (metrics_df['prediction_length'] == prediction_length) &  (metrics_df['delay'] == delay)]['weekML'].values[0]#metric_row['weekML'].values[0]
