...
class DataPrepTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        if self.drop_missing:
            def drop_missing(df: pd.DataFrame, target_sr: pd.Series = None):
                ...
                if target_sr is not None:
                    ...
                    # clean_target_sr.reset_index(drop=True)
                return clean_df, clean_target_sr
            new_X, y = drop_missing(X, target_sr=y)
        if self.date_cols is not None:
            for col in self.date_cols:
                new_X[col] = pd.to_datetime(new_X[col])
        ...
        if self.clip_data:
            rel_cols = self.clip_data.get('rel_cols')
            filterby = self.clip_data.get('filterby')
            pos_thres = self.clip_data.get('pos_thres')
        ...
        try:
            ...
            pass
        if self.correlated_cols is not None or not (not self.correlated_cols):
            new_X.drop(columns=self.correlated_cols, inplace=True)
