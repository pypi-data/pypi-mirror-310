...
class SelectiveColumnTransformer(ColumnTransformer):
    """
    Applies transformers to columns of an array or pandas DataFrame by selectively scaling only columns specified.
    """
    def __init__(self, transformers: list=None, **kwargs):
        """
        Initialize the selective scaler.
        :param transformers: List of (name, transformer, columns) tuples specifying any transformer objects to be
        applied to subsets of the data, in addition tothose that may already be configured in the app-config.properties.
        :type transformers: list
        :param kwargs: see all other parameters from scikit-learn ColumnTransformer
        :type kwargs:
        """
        ...
        if config_transformers is not None:
            for item in config_transformers.values():
                ...
                self.transformers.append((name, tf, cols))
        super().__init__(transformers=self.transformers, remainder='passthrough', force_int_remainder_cols=False, **kwargs)
        self.transformers_ = self.transformers.copy()

    def fit(self, X, y=None, **params):
        LOGGER.debug('SelectiveColumnTransformer: Fitting dataset, shape = {}, {}', X.shape,
    ...
    def transform(self, X, **params):
        LOGGER.debug('SelectiveColumnTransformer: Transforming dataset, shape = {}', X.shape)
        self.feature_cols = list(X.columns)
        transformed_X = super().transform(X, **params)
        new_X = pd.DataFrame(transformed_X, columns=self.feature_cols)
        return new_X

    def fit_transform(self, X, y=None, **params):
        LOGGER.debug('SelectiveColumnTransformer: Fit-Transforming dataset, shape = {}, {}', X.shape,
                     y.shape if y is not None else None)
        self.feature_cols = list(X.columns)
        transformed_X = super().fit_transform(X, y, **params)
        new_X = pd.DataFrame(transformed_X, columns=self.feature_cols)
        return new_X
