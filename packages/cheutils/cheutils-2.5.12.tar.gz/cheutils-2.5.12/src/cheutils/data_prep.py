...
class SelectiveColumnTransformer(ColumnTransformer):
    ...
    """
    def __init__(self, default_transformers: list=None, **kwargs):
        """
        Initialize the selective scaler.
        :param default_transformers: List of (name, transformer, columns) tuples specifying the transformer objects to be applied to subsets of the data.
        :type transformers: list
        :param kwargs: see all other parameters from scikit-learn ColumnTransformer
        :type kwargs:
        """
        self.transformers = default_transformers if default_transformers is not None else []
        config_transformers = APP_PROPS.get_list('model.selectivescaler.transformers')
        if config_transformers is not None:
            for item in config_transformers:
                name = item.get('name')
                tf = item.get('transformer')
