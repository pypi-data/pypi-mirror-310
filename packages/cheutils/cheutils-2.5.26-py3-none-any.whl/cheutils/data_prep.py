...
class SelectiveColumnTransformer(ColumnTransformer):
    def __init__(self, feature_names: list, remainder='passthrough', force_int_remainder_cols: bool=False,
        ...
        if (CONFIG_TRANSFORMERS is not None) or not (not CONFIG_TRANSFORMERS):
            LOGGER.debug('SelectiveColumnTransformer: Configured column transformers: {}', CONFIG_TRANSFORMERS)
            transformers = []
