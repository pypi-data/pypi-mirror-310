...
def bindingDB_ids(path='',query='Oc1ccc(cc1)C1CC(=O)c2c(O1)cc(cc2O)O'):
    ...
    for filn in os.listdir(path):
        if filn.startswith('bindingdb_info_lb') & filn.endswith('.csv'):
...
def lipinsky(path='',query='Oc1ccc(cc1)C1CC(=O)c2c(O1)cc(cc2O)O'):
  ...
  if len(path)==0:
    ...
    for filn in os.listdir(path):
      if filn.startswith('lipinsky_info') & filn.endswith('.csv'):
        with open(path+'/'+filn)as fin:
          for line in fin:
            line=line.rstrip()
            x+=1
  ...
  if len(path)==0:
    ...
    for filn in os.listdir(path):
      if filn.startswith('bindingdb_info_lb'):
        ...
        continue
      if filn.startswith('bindingdb_info') & filn.endswith('.csv'):
        with open(path+'/'+filn)as fin:
          for line in fin:
            line=line.rstrip()
            x+=1
