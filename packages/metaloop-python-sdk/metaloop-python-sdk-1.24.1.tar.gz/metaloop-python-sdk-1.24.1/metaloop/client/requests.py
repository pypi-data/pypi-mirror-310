...
from typing import Any, Callable, Generic, Iterable, Optional, Tuple, TypeVar, Dict
from urllib.parse import urljoin

from requests.models import Response

from metaloop.exception import ResponseError, ResponseErrorDistributor
from metaloop.utils import Tqdm, get_session
...
class Client:
    ...
    Arguments:
        ...
        url: The URL of the graviti metaloop website.

    """

    _DEFAULT_URL = "http://data.deepglint.com/"

    def __init__(self, access_key: str, url: str = "") -> None:
        url = url if url is not None else Client._DEFAULT_URL

        if not url.startswith("http://"):
            raise TypeError("url should starts with 'http://'")

        self._open_api = urljoin(url, "api/v1/")
        self._access_key = access_key

        self._login()

    def _login(self):
        params: Dict[str, Any] = {"token": self._access_key}
        self.open_api_do("GET", "auth_by_token", params=params)
    ...
    def _url_make(self, section: str, dataset_id: str = "") -> str:
        ...
        """
        if dataset_id:
            dataset_url = urljoin(self._open_api, "dataset/")
            if section:
            ...
            if section:
                ...
                url = urljoin(self._open_api, "dataset")
        return url
