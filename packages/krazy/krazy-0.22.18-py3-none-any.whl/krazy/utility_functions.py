...
def files_identifier(folder_path:Path, srch_pattern:str='.*', excl_pattern:str=None)->list:

    '''
    @params:
    folder_path  -   directory path  :   from where files are to be imported
    srch   -   optional       :   regex pattern to match with filename
    excl   -   optional        :   regext pattern to exclude from filename
    '''
    # get all files in folder
    filenames = os.listdir(folder_path)
    # retains files matching the pattern using regex
    filenames = [f for f in filenames if re.match(srch_pattern, f.lower())]
    # excludes files matching the excl_pattern using regex
    if excl_pattern:
        filenames = [f for f in filenames if not re.match(excl_pattern, f.lower())]

    # create full path
    filenames = [folder_path.joinpath(f) for f in filenames]

    return filenames

def detect_encoding(file_path)->str:
    '''Returns encoding of a file'''
    with open(file_path, 'rb') as f:
        result = chardet.detect(f.read())
    return result['encoding']

def detect_delimiter(file_path)->list[str, str]:
    '''Returns delimiter and encoding of a list'''

    encoding = detect_encoding(file_path)
    with open(file_path, 'r', encoding=encoding) as f:
        sample = f.read(5000)  # Read a small portion of the file
        try:
            # detect using sniffer
            sniffer = csv.Sniffer()
...
def read_spreadsheet_single(full_path:Path, folder:str=None, skip_rows:int=0, low_memory:bool=True, dtypes:dict={'abd':str}):
    '''Reads a file and returns a dataframe. Can handle CSV, xlsx & .xls'''
    [delimiter, encoding] = detect_delimiter(full_path)

    if full_path.suffix.lower() == '.csv':
        try:
            df_purge = pd.read_csv(full_path, skiprows=skip_rows, low_memory=low_memory, dtype=dtypes, encoding=encoding, delimiter=delimiter)
            if folder is not None:
                df_purge['folder'] = folder
            df_purge['file_name'] = full_path.name
            return df_purge
        
        except:
            print(f'File {full_path.name} could not be read')
            return None

    elif full_path.suffix.lower() == '.xlsx':
        try:
            df_purge = pd.read_excel(full_path, skiprows=skip_rows, low_memory=low_memory, dtype=dtypes)
            if folder is not None:
                df_purge['folder'] = folder
            df_purge['file_name'] = full_path.name
            return df_purge
    ...
    elif full_path.suffix.lower() == '.xls':
        try:
            df_purge = pd.read_excel(full_path, skiprows=skip_rows, low_memory=low_memory, dtype=dtypes, engine='xlrd')
            if folder is not None:
                df_purge['folder'] = folder
            df_purge['file_name'] = full_path.name
            return df_purge
        ...
        print(f'File extension not recognized for file: {full_path.name}')
        return full_path
