...
def match_one_to_many(df_one:pd.DataFrame, df_many:pd.DataFrame, col_from_one:list, col_from_many:list, match_return_col, tolerance:list=[1,-1])->list[pd.DataFrame, pd.DataFrame]:
    ...
    df_merged = pd.merge(
        left=df_one, 
        right=df_many_sum, 
        left_on='unique_code_temp',
        right_on='unique_code_temp',
        suffixes=('_x', '_y'),
        how='inner',
        indicator='Source')
    ...
    matched_dict = df_merged.set_index('unique_code_temp').to_dict()[match_return_col]
    df_one[match_return_col] = df_one['unique_code_temp'].map(matched_dict)
    df_many[match_return_col] = df_many['unique_code_temp'].map(matched_dict)
