...
class Hotkey:
    ...
    def __init__(self, keys: Set[str], on_press: Callable = False, on_release: Callable = False, suppress: bool = True):
        """
        Hotkey 객체를 생성하는 생성자
    ...
    def _handle_event(self, event_type: HotkeyEventType):
        """키가 눌러졌을 때 호출되는 메서드"""
        suppress = False
        if self._enabled:
    ...
    def handle_press(self):
        """키가 눌러졌을 때 호출되는 메서드"""
        return self._handle_event(HotkeyEventType.PRESSED)

    def handle_release(self):
        """키가 떼어졌을 때 호출되는 메서드"""
        return self._handle_event(HotkeyEventType.RELEASED)
...
class HotkeyManager:
    ...
    @add_hotkey.register
    def add_hotkey(self, keys: set, on_press: Callable, on_release: Callable, suppress: bool = True):
        """
        핫키를 등록.
        """
        hotkey = Hotkey(keys, on_press, on_release, suppress)
        self._hotkeys[hotkey.keycodes] = hotkey
    ...
    def handle_key_event(self, pressed_keycodes):
        """
        현재 눌린 키 상태와 매핑된 핫키를 확인하여 콜백 실행.
        """
        suppress = False
        for keycodes, hotkey in self._hotkeys.items():
            if all(key in pressed_keycodes for key in keycodes):  # 핫키 조건 충족
                suppress = suppress or hotkey.handle_press()

            elif (hotkey.is_pressed() and not hotkey.is_active(HotkeyEventType.RELEASED)
                  and any(key not in pressed_keycodes for key in keycodes)):  # 핫키 조건 해제
                suppress = suppress or hotkey.handle_release()
