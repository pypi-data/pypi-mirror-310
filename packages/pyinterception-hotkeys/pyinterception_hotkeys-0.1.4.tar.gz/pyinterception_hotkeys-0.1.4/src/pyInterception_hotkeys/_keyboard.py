...
class KeyboardListener(Thread):
    ...
    def __init__(self, hotkey_listener: HotkeyManager = None):
        super().__init__()
        self.daemon = True
    ...
    def _key_event_handler(self, stroke: KeyStroke):
        ...
        # 핫키 리스너에 이벤트 전달
        return self._hotkey_listener.handle_key_event(self._state_tracker.get_all_keycodes(KeyInputSource.HARDWARE))

    def is_pressed(self, keys, mode: KeyInputSource = KeyInputSource.HARDWARE):
        """
        특정 키가 눌려 있는지 확인.
        """
        key_set = set(keys) if isinstance(keys, (list, tuple, str)) else keys
    ...
    def press(self, key, delay=10, random_delay=20):
        """
    ...
    def release(self, key, delay=10, random_delay=20):
        """
    ...
    def tap(self, key, delay=70, random_delay=20, duration=50, random_duration=20):
        """
