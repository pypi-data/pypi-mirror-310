...
class Chaoxing:
	...
	async def __aenter__(self):
		...
		self.__meeting_courses = {k: v for k, v in (
			await self.course_get_meeting_courses()
		).items() if not k in self.__courses}
		for k in self.__courses:
			if k in self.__meeting_courses:
				del self.__meeting_courses[k]
		return self
	...
	def courses(self):
		...
		courses = dict(tuple(self.__courses.items())[: lim])
		courses.update(dict(tuple(
			self.__meeting_courses.items()
		)[: lim]))
		return courses
	...
	async def course_get_meeting_courses(self):
		...
		data = (await res.json()).get("data", {}).get("listData", [])
		time = _time() * 1000
		return {f"{course['id']}": {
			"class_id": f"{course['id']}",
			"course_id": f"{course['courseId']}",
			"name": course["title"],
			"teachers": [course["createrName"]],
			"status": 1, "time_start": _strftime(
				course["reserveStartTime"] // 1000
			) if course["reserveStartTime"] else "",
			"time_end": _strftime(
				course["reserveEndTime"] // 1000
			) if course["reserveEndTime"] else ""
		} for meeting in data for course in meeting["meetClasses"] if (
			(course["reserveEndTime"] or float("inf")) > time
		) and course["meetCurStatus"] != 1}
	...
	async def course_get_course_id(
		...
		course_id = course.get("course_id") or self.__courses.get(
			class_id, {}
		).get("course_id", self.__meeting_courses.get(
			class_id, {}
		).get("course_id"))
		if not course_id:
			res = await self.__session.get(
	...
	async def course_get_activities(self):
		...
		meeting_courses = [
			v for k, v in self.__meeting_courses.items()
		][ : self.__config[
			"chaoxing_course_get_activities_courses_limit"
		]]
		_sem = _Semaphore(self.__config[
		...
		await _gather(*(_mworker(course) for course in meeting_courses))
		await _tworker
		return activities
