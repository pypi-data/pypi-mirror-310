...
class Chaoxing:
	...
	async def __aenter__(self):
		...
		self.__courses = dict(tuple((
			await self.course_get_courses()
		).items())[: courses_limit])
		self.__meeting_courses = {k: v for k, v in tuple((
			await self.course_get_meeting_courses()
		).items()) if not k in self.__courses}[: courses_limit]
	...
	async def course_get_meeting_courses(self):
		...
		return {f"{course['id']}": {
			...
			((course["reserveEndTime"] or float("inf")) > time) and
			course["meetCurStatus"] != 1 and
			f"{course['puid']}" != self.__uid
		)}
