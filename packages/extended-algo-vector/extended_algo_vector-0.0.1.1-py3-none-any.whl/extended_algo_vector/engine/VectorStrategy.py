...
class VectorStrategy(ABC):
    ...
    def __init__(self, contract: Contract, start: dt.datetime, end: dt.datetime, profit_target=100., stop_loss=100.,
    ...
    def _explode_oco_entries(self):
        ...
        # TODO: This is needs to be fixed I think there

        # TODO: If there is no entries then I need to add oco
    ...
    def _generate_mfe_mae_stats(self, x):
        ...
        try:
            ...
            m = m[x.entry_time.values[0]:][['high_p', 'low_p', 'close_p']]

            m = m.loc[m.index <= x.forced_closeout.values[0]]
            m = m.iloc[1:]

            m['1_sl_exit'] = np.where(((x.signal.values[0] == -1) & (m.high_p >= x.sl.values[0])) |
                                      ((x.signal.values[0] == 1) & (m.low_p <= x.sl.values[0])), x.sl.values[0], np.nan)
            m['2_pt_exit'] = np.where(((x.signal.values[0] == -1) & (m.low_p <= x.pt.values[0])) |
                                      ((x.signal.values[0] == 1) & (m.high_p >= x.pt.values[0])), x.pt.values[0], np.nan)
            m['3_cl_exit'] = np.nan
            m.loc[m.index[-1], '3_cl_exit'] = m.iloc[-1]['close_p']

            if self.breakeven_trigger:
                m['4_bsl_exit'] = np.where(((x.signal.values[0] == -1) & (m.high_p >= x.bsl.values[0])) |
                                           ((x.signal.values[0] == 1) & (m.low_p <= x.bsl.values[0])), x.bsl.values[0], np.nan)
                m['5_bt_exit'] = np.where(((x.signal.values[0] == -1) & (m.low_p <= x.bt.values[0])) |
                                          ((x.signal.values[0] == 1) & (m.high_p >= x.bt.values[0])), x.bt.values[0], np.nan)
    ...
    def run(self):
        s = dt.datetime.now()
        assert not self.market_data.empty,  NotImplementedError('Provide register_market_data([...])\n Market data has not '
                                                               'been '
                                                      'registered....abort!')
