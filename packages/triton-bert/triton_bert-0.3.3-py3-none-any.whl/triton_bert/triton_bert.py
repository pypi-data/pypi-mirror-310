...
class TritonBert:
    def __init__(self, model: str, vocab: str, triton_host: str="localhost", triton_grpc_port: int=8001, 
        ...
        truncation: TruncationStrategy=TruncationStrategy.LONGEST_FIRST, token_fast_mode=False):
        # default fast mode is not thread safe. user has to use lock to protect.
        self.tokenizer = AutoTokenizer.from_pretrained(vocab, use_fast=token_fast_mode)
    ...
    def preprocess(self, texts, text_pairs=None):
    ...
    def _predict(self, texts, text_pairs=None):
        if text_pairs is None:
            text_pairs = []
        if not texts:
            return []
    ...
    def __call__(self, texts: Union[List[str], str], text_pairs=None):
        if text_pairs is None:
            text_pairs = []
