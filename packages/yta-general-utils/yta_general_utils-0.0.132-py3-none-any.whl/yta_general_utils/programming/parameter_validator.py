...
class PythonValidator:
    ...
    def is_instance(element, cls: Union[str, type, list]):
        ...
        if any(not PythonValidator.is_string(cls_item) and not PythonValidator.is_a_class(cls_item) for cls_item in cls):
            raise Exception(f'At least one of the provided "cls" parameter "{str(cls)}" is not a string nor a class.')
        ...
        return isinstance(element, cls_class_items) or any(getattr(type(element), '__name__', None) is cls_item for cls_item in cls_str_items)
