...
class InstallationManager:
    """Manages BSG-IDE installation and fixes"""
    ...
    def _get_install_paths(self):
        """Get appropriate installation paths based on OS and privileges"""
        paths = {}

        if self.system == "Linux":
            if self.is_admin:
                paths.update({
                    'base': Path('/usr/local/share/bsg-ide'),
                    'bin': Path('/usr/local/bin'),
                    'icons': Path('/usr/share/icons/hicolor'),
                ...
                paths.update({
                    'base': Path.home() / '.local/share/bsg-ide',
                    'bin': Path.home() / '.local/bin',
                    'icons': Path.home() / '.local/share/icons/hicolor',
                    'applications': Path.home() / '.local/share/applications',
                    'resources': Path.home() / '.local/share/bsg-ide/resources',
            ...
            if self.is_admin:
                ...
                paths.update({
                    'base': Path.home() / 'Applications/BSG-IDE.app/Contents',
                    'bin': Path.home() / '.local/bin',
                    'resources': Path.home() / 'Applications/BSG-IDE.app/Contents/Resources',
                    'python_site': Path(site.getusersitepackages()) / 'bsg_ide'
                })
            ...
            if self.is_admin:
                ...
                paths.update({
                    'base': Path(program_files) / 'BSG-IDE',
                    'bin': Path(program_files) / 'BSG-IDE/bin',
                    'resources': Path(program_files) / 'BSG-IDE/resources',
                    'python_site': Path(site.getsitepackages()[0]) / 'bsg_ide',
                    'start_menu': Path(os.environ['ALLUSERSPROFILE']) /
                                'Microsoft/Windows/Start Menu/Programs/BSG-IDE'
                })
            else:
                paths.update({
                    'base': Path(os.environ['APPDATA']) / 'BSG-IDE',
                    'bin': Path(os.environ['APPDATA']) / 'BSG-IDE/bin',
                    'resources': Path(os.environ['APPDATA']) / 'BSG-IDE/resources',
                    'python_site': Path(site.getusersitepackages()) / 'bsg_ide',
    ...
    def copy_resources(self, package_root):
        """Copy all required resources to installation directories"""
        resource_mapping = {
        ...
        search_paths = [
            ...
            package_root.parent / 'icons',
            package_root.parent / 'resources'
        ]

        for filename, dest_keys in resource_mapping.items():
            file_found = False
            for search_path in search_paths:
                src_file = search_path / filename
                if src_file.exists():
                    for dest_key in dest_keys:
                        dest_dir = self.install_paths[dest_key]
    ...
    def create_launcher(self):
        ...
        launcher_name = 'bsg-ide.bat' if self.system == "Windows" else 'bsg-ide'
        launcher_path = self.install_paths['bin'] / launcher_name
    ...
    def _get_launcher_content(self):
        """Generate launcher script content"""
        base_dir = str(self.install_paths['base'])
        python_path = str(self.install_paths['python_site'])

        if self.system == "Windows":
            return f'''@echo off
SET BSG_INSTALL_DIR={base_dir}
SET PYTHONPATH={python_path};{base_dir};%PYTHONPATH%

IF EXIST "%USERPROFILE%\\my_python\\Scripts\\activate.bat" (
    CALL "%USERPROFILE%\\my_python\\Scripts\\activate.bat"
)

python -c "import sys; sys.path.insert(0, '{base_dir}'); from BSG_IDE import main; main()"

IF ERRORLEVEL 1 (
    echo Error: Could not start BSG-IDE
    pause
)'''
        else:
            return f'''#!/bin/bash
...
export BSG_INSTALL_DIR="{base_dir}"
export PYTHONPATH="{python_path}:{base_dir}:$PYTHONPATH"
...
status=$?
if [ $status -ne 0 ]; then
    echo "Error: Could not start BSG-IDE (exit code: $status)"
...
if [ -n "$VIRTUAL_ENV" ]; then
    ...
    def setup_os_integration(self):
        """Set up OS-specific integration"""
        if self.system == "Linux":
    ...
    def _setup_linux_integration(self):
        """Set up Linux-specific integration"""
        # Set up icons
        icon_source = self.install_paths['resources'] / 'bsg-ide.png'
        if icon_source.exists():
            for size in ['16x16', '32x32', '48x48', '64x64', '128x128', '256x256']:
                icon_dir = self.install_paths['icons'] / size / 'apps'
                icon_dir.mkdir(parents=True, exist_ok=True)
                shutil.copy2(icon_source, icon_dir / 'bsg-ide.png')
            print("✓ Installed system icons")
...
Name=BSG-IDE
Comment=Beamer Slide Generator IDE
Exec={self.install_paths['bin']}/bsg-ide
Icon=bsg-ide
Terminal=false
Categories=Office;Development;Education;
Keywords=presentation;latex;beamer;slides;
StartupWMClass=bsg-ide
"""
            desktop_file = self.install_paths['applications'] / 'bsg-ide.desktop'
            desktop_file.write_text(desktop_entry)
            desktop_file.chmod(0o755)
            print("✓ Created desktop entry")
...
<dict>
    ...
    <string>bsg-ide</string>
        ...
        (self.install_paths['base'] / 'Info.plist').write_text(info_plist)
        print("✓ Created Info.plist")
        ...
        try:
            ...
            if self.is_admin:
                key = winreg.OpenKey(winreg.HKEY_LOCAL_MACHINE,
                                   ...
                                   0, winreg.KEY_ALL_ACCESS)
                path = winreg.QueryValueEx(key, 'Path')[0]
                bin_path = str(self.install_paths['bin'])
                if bin_path not in path:
                    new_path = f"{path};{bin_path}"
                    winreg.SetValueEx(key, 'Path', 0, winreg.REG_EXPAND_SZ, new_path)
                    print("✓ Added to system PATH")
            ...
            try:
                ...
                from win32com.client import Dispatch
                shell = Dispatch('WScript.Shell')
                shortcut = shell.CreateShortCut(str(self.install_paths['start_menu'] / "BSG-IDE.lnk"))
                shortcut.Targetpath = sys.executable
                shortcut.Arguments = '-m bsg_ide'
                shortcut.IconLocation = str(self.install_paths['resources'] / 'bsg-ide.ico')
                shortcut.save()
                print("✓ Created Start Menu shortcut")
            except ImportError:
                print("! Warning: winshell not available, skipping shortcut creation")
        except ImportError:
            print("! Warning: winreg not available, skipping PATH modification")
