...
class CompositionYaml(ProblemLogger):
    ...
    def _add_autosar_data_types(self, class_info, data_types, signal_name, info):
        ...
        if "Bool" in info["type"]:
            upper = 1
            lower = 0
        elif info["type"] in self.enums:
            new_data_type = {info["type"]: {"type": "ENUMERATION", "enums": self.enums[info["type"]]["members"]}}
            class_info[signal_name] = {
                "type": info["type"], "access": ("READ-ONLY" if "CVC_DISP" in info["class"] else "READ-WRITE"),
                "init": self.enums[info["type"]]["default_value"]
            }
