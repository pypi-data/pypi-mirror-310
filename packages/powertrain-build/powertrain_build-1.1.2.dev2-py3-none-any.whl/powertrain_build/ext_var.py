...
class ExtVarBase(ProblemLogger):
    ...
    def _generate_h_file(self, path):
        ...
        with path.open("w") as fh_c:
            ...
            if self._prj_cfg.enable_external_rte_headers():
                fh_c.write('#include "Rte_Type.h"\n')
                composition_name = self._prj_cfg.get_composition_config("softwareComponentName")
                fh_c.write(f'#include "Rte_{ composition_name }_Type.h"\n')
            fh_c.write("\n")
