...
class FunctionSystemPromptBuilder(SystemPromptBuilderBase):
    ...
    def build_system_prompt(self) -> str:
        allowed_functions_to_generate_names = [f.function_name for f in self.allowed_functions_to_generate]
        ...
        return SystemPromptGenerator(
            ...
            steps=[
                ...
                prompts_agent_functions.build_agent_prompt(allowed_functions_to_generate_names, topics=self.topics, _config=self._config)
            ],
...
class GraphQLSystemPromptBuilder(SystemPromptBuilderBase):
    ...
    def build_system_prompt(self) -> str:
        ...
        return SystemPromptGenerator(
            ...
            steps=[
                # TODO: could break the prompt down into steps
                prompts_agent_graphql.build_agent_prompt(self.mutations_allowed_to_generate, topics=self.topics, _config=self._config)
            ],
            output_instructions=["Your output should always be a set of zero or more GraphQL mutation calls, using only the allowed GraphQL mutations definitions."],
        )
