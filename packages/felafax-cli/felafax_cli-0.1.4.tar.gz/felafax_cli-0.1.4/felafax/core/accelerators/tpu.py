...
class TPUConfig(AcceleratorConfig):
    ...
    # zone when not specified, we will figure out the best zone
    zone: Optional[str] = None
    # backup zones to try if the primary zone is not available
    backup_zones: List[str] = field(default_factory=list)
    preemptible: bool = True
    disk_size_gb: Optional[int] = 1000
    docker_image: Optional[str] = None
    docker_env: Optional[Dict[str, str]] = field(default_factory=dict)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            **super().to_dict(),
            ...
            "zone": self.zone,
            "backup_zones": self.backup_zones,
    ...
    def from_dict(cls, data: Optional[Dict[str, Any]] = None) -> "TPUConfig":
        ...
        # Validate accelerator_type
        valid_accelerator_types = {"v3", "v5e", "v5p"}
        accelerator_type = data.get("accelerator_type", "v3-8")
        if accelerator_type not in valid_accelerator_types:
            raise ValueError(
                f"Invalid accelerator_type: {accelerator_type}. Must be one of {valid_accelerator_types}."
            )

        # Validate accelerator_core_count
        accelerator_core_count = data.get("accelerator_core_count", 8)
        if accelerator_core_count % 8 != 0 or not (8 <= accelerator_core_count <= 1024):
            raise ValueError(
        ...
        return cls(
            ...
            zone=data["zone"],
            backup_zones=data.get("backup_zones"),
            ...
            docker_env=data.get("docker_env"),
...
class TPUProvider(AcceleratorProvider):
        ...
        try:
            if state is None:
                ...
                try:
                    ...
                    logger.debug(
                        f"Successfully created TPU in primary zone {self.config.zone}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to create TPU in primary zone {self.config.zone}, trying backup zones"
                    )
                    # Try backup zones if initial zone fails
                    for backup_zone in self.config.backup_zones:
                        self.config.zone = backup_zone
                        self.node_name = f"projects/{self.config.project_id}/locations/{self.config.zone}/nodes/{self.config.name}"
                        try:
                            response = await self._create_tpu_node()
                            logger.info(
                                f"Successfully created TPU in backup zone {backup_zone}"
                            )
                            break  # Exit loop if successful
                        except Exception as backup_exception:
                            logger.debug(
                                f"Failed to create TPU in backup zone {backup_zone}: {str(backup_exception)}"
                            )
                            continue  # Try next backup zone
                    else:
                        logger.error(f"Failed to create TPU in all zones: {str(e)}")
                        raise RuntimeError(
                            f"Failed to create TPU in all zones: {str(e)}"
                        )

            elif state == "READY":
                # TPU is already running, no need to start it
            ...
            # Handle disk attachment if needed
            if self.config.attach_disk:
                logger.info("Handling disk attachment")
                disk_name = f"{self.config.name}-disk"
                disk_path = f"projects/{self.config.project_id}/zones/{self.config.zone}/disks/{disk_name}"
                if await self._disk_exists(disk_name):
                    logger.debug(f"Attaching existing disk {disk_name}")
                    await self._attach_disk_to_tpu(disk_path)
                else:
                    # create disk
                    logger.info(f"Creating new disk {disk_name}")
                    disk_name = f"{self.config.name}-disk"
                    disk_path = await self._create_persistent_disk(disk_name)
                    logger.debug(f"Attaching newly created disk {disk_name}")
                    await self._attach_disk_to_tpu(disk_path)

                # Add mount operation after disk attachment
                logger.info("Mounting disk in container")
                await self._mount_disk_in_container()

            if self.config.docker_image:
                logger.info(
                    f"Starting docker container with image {self.config.docker_image}"
                )
                await self._run_docker_container()

            logger.info("TPU node started successfully")
    ...
    def _get_region_for_accelerator(self, accelerator_type: str) -> str:
        ...
        elif accelerator_type.startswith("v3"):
            zones = [
                "europe-west4-a",
                "europe-west4-b",
                "europe-west4-c",
