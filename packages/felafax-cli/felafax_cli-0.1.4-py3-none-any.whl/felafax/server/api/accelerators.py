...
from ..models.accelerator import AcceleratorMetadata
...
async def start_accelerator(
    ...
    try:
        ...
        if request.docker_image:
            custom_config["docker_image"] = request.docker_image
        if request.attach_disk:
            custom_config["attach_disk"] = True
            custom_config["disk_size_gb"] = request.disk_size_gb
        if request.docker_env:
            custom_config["docker_env"] = request.docker_env
            
        # Create and start accelerator
        metadata = await handler.create_accelerator(
            model_name=request.model_name,
            docker_config=custom_config,
...
async def get_accelerator_status(
    ...
    try:
        handler = AcceleratorHandler(storage_provider, user_id)
        metadata = await handler._metadata_handler.get_by_id(accelerator_id)
        if not metadata:
...
async def delete_accelerator(
    ...
    try:
        ...
        # Delete metadata
        await handler._metadata_handler.delete(accelerator_id)
