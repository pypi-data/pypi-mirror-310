...
class UserHandler:
    ...
    async def get_user_info(self) -> Optional[UserMetadata]:
        """Get user metadata"""
        return await self._metadata_handler.get_by_id(self.user_id)

    async def update_user_info(self, info: Dict) -> None:
        """Update user metadata"""
        metadata = await self._metadata_handler.get_by_id(self.user_id)
    ...
    @staticmethod
    async def find_user_id(token: str, storage_client: StorageProvider) -> Optional[str]:
