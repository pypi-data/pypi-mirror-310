...
from composition_menus import COMPOSITION_MENUS
...
async def check_docker() -> None:
    ...
    from container import update_container
...
class _Compositions:
    def __init__(self: _Compositions) -> None:
        self.update()

    @mainthread
    def update(self: _Compositions) -> None:
        if self.callback:
            self.callback(self())

    def __call__(self: _Compositions) -> list[Item]:
        result: list[Item] = []
        ...
        result.append(
            ActionItem(
                ...
                color='black',
                action=functools.partial(input_docker_composition, self.update),
            ),
        )
        result.extend(
            ActionItem(
                label=(item / 'label').read_text(),
                icon='󰣆',
                action=functools.partial(
                    HeadlessMenu,
                    title=(item / 'label').read_text(),
                    items=_CompositionItems(
                        item,
                        on_delete=self.update,
                    ),
                ),
            )
            for item in COMPOSITIONS_PATH.iterdir()
        )
        return result

    def unsubscribe(self: _Compositions) -> None:
        self.callback = None
...
def _open_apps_menu() -> HeadlessMenu:
    return HeadlessMenu(
        title='󰣆Docker Apps',
        items=_Compositions(),
    )
...
def input_docker_composition(callback: Callable[[], None]) -> None:
    """Input the Docker credentials."""
    ...
    store.dispatch(
        RegisterRegularAppAction(
            menu_item=ActionItem(
                label=image.label,
                icon=image.icon,
                action=COMPOSITION_MENUS[image.id],
            ),
            key=image.id,
        ),
    )
