...
class InputData:
    ...
    def load_data(self, load_from_dump=False):
        ...
        if (
            self.input_parameters.reset_when_rain_occured
            and "rain" not in self.predictors
        ):
            raise FileValidationError(
        ...
        logging.info(self.input_parameters.compute_slope)
    ...
    def _load_predictors(
        ...
        for predictor_name, predictor in self.predictors.items():
            if load_from_dump:
                ...
                logger.info(f"Load predictor from file {predictor_name}")
                try:
            ...
            # already saved
            if (
                self.input_parameters.save_input_data
                or predictor_name in stream_dic
