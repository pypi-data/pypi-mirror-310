...
class TestServiceConfig:
    ...
    def test_tracing_config(self):
        config = ServiceConfig(applications=[{"import_path": "main:app"}])
        assert config.tracing_config is None

        config = ServiceConfig(
            applications=[{"import_path": "main:app"}],
            tracing_config=TracingConfig(exporter_import_path="path:exporter",),
        )
        assert config.tracing_config == TracingConfig(
            exporter_import_path="path:exporter"
        )

        with pytest.raises(
            TypeError, match="'tracing_config' must be a TracingConfig.",
        ):
            ServiceConfig(
                applications=[{"import_path": "main:app"}], tracing_config=["path"],
            )
