...
class TestDeploy:
    ...
    def test_deploy_from_file_override_options(self, fake_service_sdk):
        ...
        result = runner.invoke(
            deploy, ["-f", FULL_CONFIG_PATH, "--env", "ENV_KEY=UPDATED_ENV_VAL",],
        )
        assert result.exit_code == 0
        assert fake_service_sdk.deployed_config is not None
        expected_config = ServiceConfig(
        ...
        result = runner.invoke(
            deploy, ["-f", FULL_CONFIG_PATH, "--env", "NEW_ENV_KEY=NEW_ENV_VAL",],
        )
        assert result.exit_code == 0
        assert fake_service_sdk.deployed_config is not None
        expected_config = ServiceConfig(
        ...
        # Set environment variable in a config that doesn't have any.
        result = runner.invoke(
            deploy, ["-f", MINIMAL_CONFIG_PATH, "--env", "ENV_KEY=ENV_VAL",],
        )
        assert result.exit_code == 0
        assert fake_service_sdk.deployed_config is not None
        expected_config = ServiceConfig(
            applications=[{"import_path": "main:app"}], env_vars={"ENV_KEY": "ENV_VAL"},
        )
        assert fake_service_sdk.deployed_config == expected_config
