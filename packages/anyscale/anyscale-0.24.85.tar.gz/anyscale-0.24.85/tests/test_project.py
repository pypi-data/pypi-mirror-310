...
def test_get_project_id_for_cloud_from_name(project_test_data: Project) -> None:
    ...
    mock_get_default_project = Mock(return_value=Mock(id="default_project_id"))

    with patch("anyscale.project.get_default_project", mock_get_default_project):
        project_id = get_project_id_for_cloud_from_name(
            "default", project_test_data.cloud_id, mock_api_client
        )
    assert project_id == default_project_id
    mock_get_default_project.assert_called_once()
