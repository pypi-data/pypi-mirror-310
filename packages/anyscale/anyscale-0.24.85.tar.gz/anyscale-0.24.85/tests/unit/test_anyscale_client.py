...
from unittest.mock import ANY, call, MagicMock, mock_open as mock_open_factory, patch
import uuid
import zipfile
...
class FakeInternalAPIClient:
    ...
    FAKE_AZURE_FILE_URI = f"azure://{FAKE_BUCKET_NAME}/{{file_name}}"
    ...
    def generate_cloud_data_bucket_presigned_url_api_v2_clouds_cloud_id_generate_cloud_data_bucket_presigned_url_post(
        ...
        if cloud_id == "test-generic-cloud-id-azure":
            return FakeClientResult(
                result=CloudDataBucketPresignedUrlResponse(
                    url=self.FAKE_UPLOAD_URL_PREFIX + request.file_name,
                    file_uri=self.FAKE_AZURE_FILE_URI.format(
    ...
    def download_aggregated_instance_usage_csv_api_v2_aggregated_instance_usage_download_csv_get(
        self,
        start_date: str,
        end_date: str,
        cloud_id: str,
        ...
        instance_usage = [
            {
                ...
                "instance_seconds": 173.6,
            },
            {
                "organization_id": "org_2",
                "instance_type": "m5.xlarge",
                "instance_seconds": 123.4,
            },
        ]
        ...
        writer = csv.DictWriter(
            csv_buffer,
            fieldnames=["organization_id", "instance_type", "instance_seconds"],
        )
        writer.writeheader()
        writer.writerows(instance_usage)
        csv_buffer.seek(0)
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            zip_file.writestr(
                f"aggregated_instance_usage_{start_date}_{end_date}.csv",
                csv_buffer.getvalue(),
            )
        ...
        return urllib3.response.HTTPResponse(
            body=zip_buffer,  # type: ignore
            headers={
                "Content-Disposition": f"attachment; filename=aggregated_instance_usage_{start_date}_{end_date}.zip"
            },
...
class TestUploadLocalDirToCloudStorage:
    ...
    @pytest.mark.parametrize("working_dir", TEST_WORKING_DIRS)
    @pytest.mark.parametrize(
        "cloud_id", ["test-generic-cloud-id", "test-generic-cloud-id-azure"]
    )
    ...
    def test_basic_generic(
        ...
        if cloud_id == "test-generic-cloud-id":
            uri = anyscale_client.upload_local_dir_to_cloud_storage(
                working_dir, cloud_id=cloud_id,
            )
            assert isinstance(uri, str) and len(uri) > 0
            mock_smart_open.assert_called_once_with(ANY, "wb")
        else:
            mock_transport_params = MagicMock()
            mock_blob_service_client = MagicMock()

            # Mock the `from_connection_string` method
            mock_blob_service_client.from_connection_string.return_value = (
                mock_transport_params
            )

            with patch(
                "anyscale.utils.imports.azure.try_import_azure_storage_blob_BlobServiceClient",
                ...
                mock_smart_open.assert_called_once_with(
                    ANY, "wb", transport_params={"client": mock_transport_params,}
                )
...
class TestAggregatedInstanceUsage:
    def test_download_csv(
        ...
        with patch("builtins.open", mock_open_factory()) as mock_open:
            file_path = anyscale_client.download_aggregated_instance_usage_csv(
                date(2024, 11, 15),
                date(2024, 11, 16),
                cloud_id="cld_1",
                project_id="prj_1",
            ...
            mock_open.assert_called_once_with(
                "/tmp/aggregated_instance_usage_2024-11-15_2024-11-16.zip", "wb"
            )

        assert file_path == "/tmp/aggregated_instance_usage_2024-11-15_2024-11-16.zip"
