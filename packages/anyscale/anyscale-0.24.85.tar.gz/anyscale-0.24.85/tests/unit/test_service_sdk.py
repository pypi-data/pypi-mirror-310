...
class TestControllerLogs:
    def test_controller_logs(
        self,
        service_sdk_with_fakes: Tuple[
            ServiceSDK, FakeAnyscaleClient, TestLogger, FakeTimer
        ],
        ...
        # Get logs of primary version
        fake_client.add_controller_logs("primary", "primary-version-logs")
        assert sdk.controller_logs("test-service-name") == "primary-version-logs"

        # Getting canary version logs should fail
        with pytest.raises(
            ValueError,
            match=f"Service '{name}' is not currently rolling out. There is no canary version.",
        ):
            sdk.controller_logs("test-service-name", canary=True)
        ...
        fake_client.add_controller_logs("canary", "canary-version-logs")
        assert (
            sdk.controller_logs("test-service-name", canary=True)
            == "canary-version-logs"
        )
