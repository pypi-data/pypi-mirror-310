...
def test_update_machine_pool_with_temp_file(mock_auth_api_client) -> None:
    spec_content = {"key": "value"}
    with tempfile.NamedTemporaryFile("w", suffix=".yaml", delete=False) as temp_file:
        temp_file.write(yaml.dump(spec_content))
    ...
    machine_pool_controller = MachinePoolController()
    machine_pool_controller.api_client.update_machine_pool_api_v2_machine_pools_update_post = Mock(
        return_value=api_response
    )

    machine_pool_controller.update_machine_pool(
        machine_pool_name="pool1", spec_file=temp_file_path
    )

    machine_pool_controller.api_client.update_machine_pool_api_v2_machine_pools_update_post.assert_called_once_with(
        UpdateMachinePoolRequest(machine_pool_name="pool1", spec=spec_content)
    )
