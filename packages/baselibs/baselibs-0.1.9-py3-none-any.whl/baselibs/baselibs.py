...
def readjsonp(fname):
    try:
        txt = readtxtfile(fname)
        jdat = [json.loads(x) for x in txt.splitlines()]
        return jdat
    except Exception as e:
        return None
...
def savejson(dat, outfile, indent=4, method='w'):
    ...
    savetofile(dat_txt, outfile, method=method)
...
def text_splitline(text, chars='。；？?！!', maxlen=64):
    ''' 对文本进行重新分行，拆分按行组合方式，最大长度maxlen=64；
    '''
...
def get_all_folder (workpath, order=0):
    '''递归返回目录下的所有目录，包括子目录；
        ...
        if os.path.isdir(workpath):
            for dirname in os.listdir(workpath):
                ...
                if os.path.isdir(file_path):
                    ...
                    yield from get_all_folder(file_path, order=order)
...
def api_post_data (url, data, timeout=30, data_format='json', headers=None):
...
def init_logging(log_filename:str="./logs/server.log", level=logging.DEBUG, interval=1, backupCount=7):
    ''' 设置日志，按日期保存日志，默认：每1天一个日志，保留7天；s
    '''
    ...
    mkfold(log_path)
    time_rota_handler = logging.handlers.TimedRotatingFileHandler(
    ...
    logging.getLogger('').addHandler(time_rota_handler)
