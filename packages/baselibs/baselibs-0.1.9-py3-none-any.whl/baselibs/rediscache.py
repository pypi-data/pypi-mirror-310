...
class RedisCache():
    '''
    def __init__(self, host='127.0.0.1', port=6379, ttl=3600):
        self.r = redis.Redis(host=host, port=port, decode_responses=True, db=1)
        self.ttl = ttl   # 默认过期时间（秒）, 0=不过期，默认=3600s=1小时
    '''
    ...
    def setkey(self, keyname, dat, ttl=0):
        ...
        try:
            ...
            if ttl <= 0:
                ...
                # 按TTL自动超时方式保存
                self.r.setex(keyname, ttl, value)
        except Exception as e:
            print(e)
            pass
        ...
        if not ret is None:
            ...
            if get_data_fun is None:
                ...
                if ndat:
                    ...
                    logging.debug(f'svae cache key={keyname}')
                else:
