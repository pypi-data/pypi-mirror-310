...
void mexFunction(
    ...
    if (headerPoint){
        ...
        if (firstFrameField){
            mxDouble* firstFrameData = mxGetPr(firstFrameField);
            if (firstFrameData)
                // 1-based
    ...
    // Fill the data
    mxDouble* allDataPoints = mxGetPr(dataPoints);
    mxDouble* allResiduals = nullptr;
    mxDouble* allMasks = nullptr;
    if (dataMetaPoints){
        if (metadataResidual){
            allResiduals = mxGetPr(metadataResidual);
        }
        if (metadataCamMasks){
            allMasks = mxGetPr(metadataCamMasks);
        }
    }
