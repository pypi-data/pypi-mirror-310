...
class Community(cobra.Model):
    ...
    def __init__(
        ...
        if model_db is not None:
            ...
            ##############################
            # Get family and order Info
            manifest = manifest[keep_cols + ["file"]+["order"]]
            merged = pd.merge(taxonomy, manifest, on=keep_cols)
            # Get unique 'order' values
            familyInfo=list(merged['order'].unique())
            # Initialize an empty list to collect the processed rows
            processed_rows=[]
            for ids in familyInfo:
                subset = merged[merged['order'] == ids]
                if len(subset) > 1:
                    # Create a dictionary to hold the combined row data
                    combined_row = {
                               'sample_id': subset['sample_id'].iloc[0],
                               'abundance': subset['abundance'].sum(),
                               'id': subset['order'].iloc[0],
                               'file': list(subset['file']),
                               'order': subset['order'].iloc[0]
                               }
                    processed_rows.append(combined_row)
                    # Drop the original rows from the merged DataFrame
                    merged = merged[merged['order'] != ids]
            processed_df = pd.DataFrame(processed_rows)
            merged = pd.concat([merged, processed_df], ignore_index=True)
            ##############################
        ...
        for idx in index:
            ...
            if isinstance(row.file, list):
                if len(row.file) > 1:
                    Modelname=idx
                    model = join_models(row.file,Modelname)
