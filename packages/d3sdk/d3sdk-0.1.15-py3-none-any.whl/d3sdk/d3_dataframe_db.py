...
from d3sdk.model.business_model_define import DeviceMeasurementGroup, InstanceMeasurement, AlarmInstancePointsConfig, AlarmInstancePointsStatistics
from d3sdk.k2box_dataframe_db import K2boxDataFrameDB
from d3sdk.uitl.business_util import get_between
...
class D3DataFrameDB:
            ...
            with self.engine.connect() as conn:
                ...
                for row in result.fetchall():
                    record = DeviceMeasurementGroup(
                            device_code=row[0],
                            device_name=row[1],
                            measurement_group_code=row[2],
                            device_type_code=row[3],
                            measurement_group_name=row[4],
                            schema=row[5]
                        )
                    records.append(record)
                return records
    ...
    def getInstanceMeasurements(self, devices: list = None) -> List[InstanceMeasurement]:
        ...
        devices = list({deviceMeasurementGroup.device_code for deviceMeasurementGroup in deviceMeasurementGroups})
        ...
        for deviceMeasurementGroup in deviceMeasurementGroups:
            ...
            schemaDevices[schema].append(deviceMeasurementGroup.device_code)

        wildcardInstanceMeasurements = [
            item for item in instanceMeasurements if item.device_code == "*"
        ]
        if wildcardInstanceMeasurements:
                ...
                for device in schemaDevices.get(schema, []):
                    for instanceMeasurement in instances:
                        temp  = InstanceMeasurement(
                            ...
                            repo_code=instanceMeasurement.repo_code,
                            repo_name=instanceMeasurement.repo_name,
                            schema_column=instanceMeasurement.schema_column,
                            repo_column=instanceMeasurement.repo_column,
                            schema_column_name=instanceMeasurement.schema_column_name,
                            unit=instanceMeasurement.unit,
                            lower_bound=instanceMeasurement.lower_bound,
                            upper_bound=instanceMeasurement.upper_bound,
                            type=instanceMeasurement.schema,
                            measurement=None,
                            measurement_group_code=None,
            ...
            instanceMeasurements = [
                im for im in instanceMeasurements if im.device_code() != "*"
        ]
        # 填充 group_code 和 group_name
        for instanceMeasurement in instanceMeasurements:
            for deviceMeasurementGroup in deviceMeasurementGroups:
                if (instanceMeasurement.device_code == deviceMeasurementGroup.device_code and instanceMeasurement.schema == deviceMeasurementGroup.schema):
                    instanceMeasurement.measurement_group_code = deviceMeasurementGroup.measurement_group_code
                    instanceMeasurement.measurement_group_name = deviceMeasurementGroup.measurement_group_name
                    instanceMeasurement.measurement = f"{deviceMeasurementGroup.measurement_group_code}.{instanceMeasurement.schema_column}"
                    instanceMeasurement.measurement_name = instanceMeasurement.schema_column_name
                    instanceMeasurement.device_type_code = deviceMeasurementGroup.device_type_code
                    break
    ...
    def getInstanceMeasurementsFilter(self, device: str = None, alarms: list = None) -> List[InstanceMeasurement]:
        ...
        for alarmCode in alarmCodes:
            ...
            alarmInstancePointsStatistics.append(AlarmInstancePointsStatistics(device_code=device, alarm_code=alarmCode, measurements=points))
        if alarmInstancePointsStatistics:
            ...
            for alarm in alarms:
                for alarmInstancePointsStatistic in alarmInstancePointsStatistics:
                    if alarm == alarmInstancePointsStatistic.alarm_code:
