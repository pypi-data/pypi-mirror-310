...
def find_pyc_files():
    matches = []
    for root, dirnames, filenames in os.walk('your_package_name'):
        for filename in fnmatch.filter(filenames, '*.pyc'):
            # Append the path from root to filename
            matches.append(os.path.join(root[len('your_package_name')+1:], filename))
    return matches


setup(
    name='cm_course_uos',
    version='2.0.0', #always increase this number before pushing to master -- x.y.z. : small edits, change "z", big edits & new files, change "y"
    ...
    package_data={"cm_course_uos":find_pyc_files()}
)