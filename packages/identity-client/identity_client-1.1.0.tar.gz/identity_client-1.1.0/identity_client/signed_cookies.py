...
class SignedCookiesMiddleware(object):
    ...
    def __call__(self, request):
        ...
        assets_domain = "*.hub3d.pvh.com"
        if "stitch.fashion" in request.get_host():
            assets_domain = "*.stitch.fashion"
        if getattr(request, 'user', None) and request.user.is_authenticated:
            if 'HTTP_HOST' in request.META:
                add_signed_cookies(
                    request, response, assets_domain, get_domain_from_header(request.META['HTTP_HOST']))
            else:
                add_signed_cookies(request, response, assets_domain)
