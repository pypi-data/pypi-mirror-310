		...
		def hook(module, input, output):
			...
			for _targetNeuronIdx in _targetIndexes:
				if(modifyValue == "None"):
					...
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", ""), _targetNeuronIdx[0], _targetBitIdx, beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
				else:
					beforeDecRep = _singleDimensionalNeurons[_targetNeuronIdx]
					beforeBinaryRep = binary(beforeDecRep)
					...
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", ""), _targetNeuronIdx[0], "None", beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
		...
		def hook(module, input):
			...
			for _targetNeuronIdx in _targetIndexes:
				if(modifyValue == "None"):
					...
					afterBinaryRep = "".join(bits)
					_singleDimensionalNeurons[_targetNeuronIdx] = binToFloat(afterBinaryRep)
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", "")
.replace(":", ""), _targetNeuronIdx[0], _targetBitIdx, beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
				else:
					beforeDecRep = _singleDimensionalNeurons[_targetNeuronIdx]
					beforeBinaryRep = binary(beforeDecRep)
					_singleDimensionalNeurons[_targetNeuronIdx] = modifyValue(_singleDimensionalNeurons[_targetNeuronIdx])
					...
					afterBinaryRep = binary(afterDecRep)
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", "")
.replace(":", ""), _targetNeuronIdx[0], "None", beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
	...
	def offlineSingleLayerWeightInjection(self, model: nn.Module, targetLayer: str, targetLayerTypes: list=None, errorRate: float="unset", NofError: int="unset", targetBit: Union[int, str]="random", accumulate: bool=True, errorIdx: Union[list, str]="random"):
		...
		for _targetWeightIdx in _targetIndexes:
			...
			tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", "")
.replace(":", ""), _targetWeightIdx, _targetBitIdx, beforeBinaryRep, beforeDecRep, afterBinaryRep, _singleDimensionalWeights[_targetWeightIdx]))
	...
	def onlineSingleLayerBackwardInjection(self, model: nn.Module, targetLayer: str, targetLayerTypes: list=None, errorRate: float="unset", NofError: int="unset", targetBit: Union[int, str]="random", errorIdx: Union[list, str]="random"):
		...
		def hook(module, grad_input, grad_output):
			...
			for _targetNeuronIdx in _targetIndexes:
				...
				tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, str(_targetLayer).replace(":", "")
.replace(":", ""), _targetNeuronIdx, _targetBitIdx, beforeBinaryRep, beforeDecRep, afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx]))
