		...
		def hook(module, input, output):
			...
			for _targetNeuronIdx in _targetIndexes:
				if(modifyValue == "None"):
					...
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, _targetLayer.replace(":", ""), _targetNeuronIdx[0], _targetBitIdx, beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
				else:
					beforeDecRep = _singleDimensionalNeurons[_targetNeuronIdx]
					beforeBinaryRep = binary(beforeDecRep)
					...
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, _targetLayer.replace(":", ""), _targetNeuronIdx[0], "None", beforeBinaryRep, beforeDecRep[0], afterBinaryRep, _singleDimensionalNeurons[_targetNeuronIdx][0]))
		...
		def hook(module, input):
			...
			for _targetNeuronIdx in _targetIndexes:
				if(modifyValue == "None"):
					...
					afterBinaryRep = "".join(bits)
					_singleDimensionalNeurons[_targetNeuronIdx] = binToFloat(afterBinaryRep)
					tmpLog.append("{}:{}:{}:{}:{}:{}:{}:{}".format(_targetLayerIdx, _targetLayer.replace(":", "")
