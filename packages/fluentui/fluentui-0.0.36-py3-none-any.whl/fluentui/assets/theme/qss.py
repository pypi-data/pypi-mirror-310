...
class QssSheet(Enum):
    ...
    def apply_theme(cls, widget: QWidget, theme='auto', typ=None) -> None:
        ...
        if (name := c.__name__) not in cls:
            cls.apply_theme(widget, theme, c.__base__)
            return
...
class LightQss:
    ...
    ComboBox = {
        ...
        '::item': {'padding': '4 0', 'color': Light.Text},
        '::item:hover': {'background': '#eeeeef'},
        '::item:selected': {'background': '#eeeeef'},
        '::drop-down': {'border': 'none', 'width': '16', 'right': '4'},
        '::down-arrow': {'image': f'url(:/fluentui/icons/chevron_down-20.svg)'}
    }
