...
class Textarea(AbsScrollAreaMix, QTextEdit):
    ...
    def setText(self, text: str):
        ...
        div = tree.css_first('style')
        print(f'{div.html=}')

        for img in HTMLParser(text).css('img[src^="http://"], img[src^="https://"]'):
            url = Url(src := img.attributes['src'])
            if url.isEmpty() or not url.isValid():
                continue

            print(f'style: {img.attrs.get('style')}')

            if doc.resource(TextDocument.Resource.Image, url):
                print(f'contains source: {url.toString()}')
                continue

            # 占位图片源，防止请求重复的图片
            doc.addResource(TextDocument.Resource.Image, url, True)
    ...
    def __downloaded(self, reply: QNetworkReply):
        ...
        try:
            doc = self.document()
            doc.addResource(TextDocument.Resource.Image, reply.url(), image)
            doc.setPageSize(doc.pageSize())  # 刷新布局
