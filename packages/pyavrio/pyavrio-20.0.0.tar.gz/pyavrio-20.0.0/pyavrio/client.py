...
class TrinoQuery(object):
    ...
    def execute(self, additional_http_headers=None) -> TrinoResult:
        ...
        try:
            ...
            #API to fetch modified query : calling pyavrio query engine API
            if not is_capability_query:
                self._query = self._query_parser.remove_schema_from_query(self._query, session.platform)
                modified_response = self._avrio_http_handler._get_modified_query(session.user, self._query,session.access_token)
                if not modified_response.ok:
                    self._request.raise_response_error(modified_response)
                modified_avrio_query = modified_response.text
            else:
