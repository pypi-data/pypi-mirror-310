...
class TrinoDialect(DefaultDialect):
    ...
    def create_connect_args(self, url: URL) -> Tuple[Sequence[Any], Mapping[str, Any]]:
        ...
        if url.password:
            ...
            token = AvrioHTTPHandler._generate_token(url.username, url.password,url.host)
    ...
    def get_columns(self, connection: Connection, table_name: str, schema: str = None, **kw) -> List[Dict[str, Any]]:
        ...
        if platform == 'data_products':
            columns = AvrioHTTPHandler._get_columns_dp(user,token, host,table)
            return columns
        else:
            if schema is None:
            ...
            columns = AvrioHTTPHandler._get_columns_ds(user,token, host,catalog,schema,table)
            return columns
    ...
    def get_catalog_names(self, connection: Connection, **kw) -> List[str]:
        ...
        user=self._get_default_user(connection)
        if platform == 'data_products':
            catalogs = AvrioHTTPHandler._get_catalogs_dp(host, user, token)
            return catalogs
        else:
            catalogs = AvrioHTTPHandler._get_catalogs_ds(host, user, token)
            return catalogs
    ...
    def get_schema_names(self, connection: Connection, **kw) -> List[str]:
        ...
        if platform == 'data_products':
            if len(catalog)==0 or catalog == 'system':
                ...
                schemas = AvrioHTTPHandler._get_schemas_dp_1(user, token, host, params)
                return schemas
            else:
                schemas = AvrioHTTPHandler._get_schemas_dp(user, catalog, token, host)
                return schemas
        else:
            schemas = AvrioHTTPHandler._get_schemas_ds(user, catalog, token, host)
            return schemas
    ...
    def get_table_names(self, connection: Connection, schema: str = None, **kw) -> List[str]:
        ...
        if platform == 'data_products':
            if len(catalog)==0 or catalog == 'system':
                ...
                tables = AvrioHTTPHandler._get_tables_dp_1(user, token, host, params)
                table_names = [table[0] for table in tables]  
                return table_names
            else:
                tables = AvrioHTTPHandler._get_tables_dp(user, catalog, token, host, schema)
                return tables

        else:
            tables = AvrioHTTPHandler._get_tables_ds(user, catalog, token, host, schema)
            return tables
