...
def create_dynamodb_table_configuration(table_name, partition_key, sort_key, lsi_keys):
    """
    ...
    # Dynamically add LSI configurations
    for lsi_name, lsi_sort_key in lsi_keys:
        ...
        table_config[table_name]["Properties"]["LocalSecondaryIndexes"].append(lsi_config)
...
def create_service_structure():
    try:
        ...
        partition_key = inquirer.text(
            message=f"Enter the partition key (default: {partition_key}):", 
        ...
        if table_name:
            dynamodb_config = create_dynamodb_table_configuration(table_name, partition_key, sort_key, lsi_keys)
