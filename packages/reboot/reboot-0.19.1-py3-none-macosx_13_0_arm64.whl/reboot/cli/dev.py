    ...
    while True:
        ...
        async with watch(proto_globs) as protos_event_task:
            ...
            async with watch(args.watch or []) as watch_event_task:
                ...
                # Transpile and bundle TypeScript if given a `.ts` file.
                ts_input_paths = []
                ...
                if auto_transpilation:
                    ...
                    async with subprocesses.shell(
                        ...
                        if process.returncode != 0:
                            terminal.warn(
                                f'Transpilation failed with return code {process.returncode} '
                                '... waiting for modification'
                                '\n'
                            )

                            # TODO: if this is the first time that we
                            # are running this we won't have a
                            # metafile and watching `application` is
                            # not sufficient because a different file
                            # might be the culprit so we should just
                            # bail.
                    ...
                    # Now add all the input files that we need to watch.
                    ...
                    # TODO: need to explicitly add
                    # `@reboot-dev/reboot` or rather the path to it in
                    # `node_modules` to `ts_input_paths` because we
                    # marked it as 'external' when calling `esbuild`
                    # and thus it is not in the `metafile`.
