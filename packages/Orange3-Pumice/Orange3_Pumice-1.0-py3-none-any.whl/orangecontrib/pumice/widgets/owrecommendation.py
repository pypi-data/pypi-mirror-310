...
class CartoonTableModel(QAbstractTableModel):
    ...
    def data_for_person(self, row, role):
        ...
        if role == Qt.ItemDataRole.DisplayRole:
            name = self.names[row]
            friends = _tr.m[2, "Similar: "] + ", ".join(self.names[self.friends[row][0]])
...
class OWRecommendation(OWWidget):
    name = _tr.m[3, "Recommendation"]
    description = _tr.m[4, "Demo for simple network-based recommendation algorithm"]
    icon = "icons/recommendation.svg"

    class Inputs:
        network = Input(_tr.m[5, "Network"], Network, default=True)
        item_data = Input(_tr.m[6, "Items"], Table)

    class Error(OWWidget.Error):
        no_choices = Msg(
            _tr.m[7, "Network does not contain user choices. Provide separate data."])
        no_item_names = Msg(
            _tr.m[8, "Network does not contain item names."])
        no_user_names_in_net = Msg(
            _tr.m[9, "Data included in the network does not contain user names."])
        user_names_mismatch = Msg(
            _tr.m[10, "Some network nodes are missing from data columns"]
        )
        network_names_ambiguous = Msg(
            (_tr.m[11, "Network nodes contain multiple string attributes.\n"] + (_tr.m[12, "When network and data are both present, the network must have\n"] + _tr.m[13, "a single string attribute whose values match the names of persons."]))
        )
        invalid_node_data = Msg(_tr.m[14, "Network data must be a table or a 1-d array"])
    ...
    def __init__(self):
        ...
        gui.comboBox(
            ...
            label=_tr.m[15, "Person name column (in network data): "], box=True,
            contentsLength=20,
        ...
        gui.comboBox(
            ...
            label=_tr.m[16, "Item column: "], box=True,
            contentsLength=20,
