...
class MindSpawner:
    ...
    def send_evaluation(self, evaluation_score, episode_data):
        """
        Send the evaluation and step-by-step data to the server.

        Args:
            evaluation_score (float): The total reward achieved in the episode.
            episode_data (list): List of step data (state, action, reward) for the episode.
        """
        if self.agent_id:
            ...
            self.sio.emit('agent', {
                ...
                'evaluation': evaluation_score,
                'steps': episode_data,  # Include the step-by-step data
                'auth': {
                    'userId': self.user_id,
                    'agentId': self.agent_id
                }
            })
            print(f"[P2A library] Sent evaluation: {evaluation_score} with step data.")
        else:
