            ...
            try:
                while not self._stop_streaming:
                    if isinstance(stream, io.TextIOWrapper):
                        # For text mode streams (universal_newlines=True)
                            ...
                            try:
                                ...
                                for line in decoded_text.splitlines():
                                    if line:
                                        ...
                                        if is_error:
                                            console.print(f"[red]{line}[/red]")
                                            logger.error(f"Process error output (utf-8): {line}")
                                        else:
                                            console.print(line)
                                            logger.debug(f"Process output (utf-8): {line}")
                            except:
            ...
            try:
                ...
                if self.platform_info.os_type == 'windows':
                    ...
                    process = subprocess.Popen(
                        ...
                        cwd=os.getcwd(),
                        universal_newlines=True  # Text mode for Unix
                    )
