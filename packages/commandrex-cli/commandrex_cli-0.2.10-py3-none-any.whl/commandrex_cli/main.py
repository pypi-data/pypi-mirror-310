...
def handle_command(ui: CommandRexUI, translator: CommandTranslator, 
    ...
    try:
        ...
        if not cmd_lower:
            ...
            reset_terminal()  # Add explicit reset
            return True, None
        elif cmd_lower == 'enable-logging':
            Config.LOGGING_ENABLED = True
            Config.setup_logging()
            ui.console.print("[green]Logging enabled[/green]")
            return True, None
        elif cmd_lower == 'disable-logging':
            Config.LOGGING_ENABLED = False
            Config.setup_logging()
            ui.console.print("[yellow]Logging disabled[/yellow]")
            return True, None
...
def run_commandrex(enable_logging: bool = False) -> None:
    """Run CommandRex with optional logging."""
    # Configure logging
    Config.LOGGING_ENABLED = enable_logging
    Config.setup_logging()
...
def main():
    """Entry point for CommandRex CLI."""
    # Parse command line arguments
    parser = argparse.ArgumentParser(description='CommandRex CLI - Your command-line companion')
    parser.add_argument('--enable-logging', action='store_true', help='Enable debug logging')
    args = parser.parse_args()
    
    # Run CommandRex with the specified logging setting
    run_commandrex(enable_logging=args.enable_logging)
