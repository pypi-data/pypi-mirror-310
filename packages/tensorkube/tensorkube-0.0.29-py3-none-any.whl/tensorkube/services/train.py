...
from tensorkube.services.k8s_service import get_tensorkube_cluster_context_name, create_configmap, create_aws_secret, generate_kubeconfig
...
import json
import logging
from tensorkube.services.job_queue_service import deploy_job, queue_job
from tensorkube.services.deploy import DEFAULT_CPU, DEFAULT_MEMORY

# disable botocore info logs
logging.getLogger('botocore').setLevel(logging.CRITICAL)

AXOLOTL_IMAGE_TRAIN_JOB = "tensorfuse/axolotl-train:v0.0.1"
AXOLOTL_IMAGE_SCALED_JOB = "tensorfuse/axolotl-train:v0.0.2"
...
supported_base_models = ['meta-llama/Llama-3.1-70B-Instruct'] #from hugging face


def get_dataset_for_base_model(config: dict, base_model: str, s3_path: str) -> dict:
    if base_model not in supported_base_models:
        raise Exception(f"Invalid base model {base_model}")
    config['datasets'][0]['path'] = s3_path
    base_config = get_axolotl_base_config_by_model(base_model)
    config['datasets'][0] = base_config['datasets'][0]
    config['datasets'][0]['path'] = s3_path
    return config

def get_config_from_paramaters(base_model: str, dataset: str, epochs: int, micro_batch_size: int, learning_rate: float, lora_r: int, base_config: dict):
    ...
    config = get_dataset_for_base_model(config, base_model, s3_path)
    return config

def get_axolotl_base_config_by_model(base_model: str) -> dict:
    if base_model not in supported_base_models:
        raise Exception(f"Invalid base model {base_model}")
    # only support llama-3.1-70B-Instruct.yaml
...
def create_fine_tuning_job(job_name:str, job_id:str, gpus: int, gpu_type: str, max_scale: int, base_model: str, dataset: str, epochs: int, cpu: float =  DEFAULT_CPU, memory: float = DEFAULT_MEMORY, secrets:List[str] = [], micro_batch_size: Optional[int] =None, learning_rate: Optional[float] = None, lora_r: Optional[int] = None):
    ...
    try:
        deploy_job(job_name=job_name, env=env, gpus=gpus, gpu_type=gpu_type, cpu=cpu, memory=memory, max_scale=max_scale, image_tag=AXOLOTL_IMAGE_SCALED_JOB, secrets=secrets, job_type='axolotl')
    ...
    config = get_config_from_paramaters(base_model=base_model, dataset=dataset, epochs=epochs, micro_batch_size=micro_batch_size, learning_rate=learning_rate, lora_r=lora_r, base_config=base_yaml_dict)
