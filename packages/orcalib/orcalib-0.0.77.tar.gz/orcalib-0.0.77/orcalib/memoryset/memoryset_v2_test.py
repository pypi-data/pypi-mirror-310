...
def test_repository_from_uri():
    ...
    assert LabeledMemorysetV2.repository_from_uri("file:./temp/orca.db#my_collection") == MemorysetLanceDBRepository(
        collection_name="my_collection", database_uri="./temp/orca.db"
    )
    assert LabeledMemorysetV2.repository_from_uri("temp/orca.db#my_collection") == MemorysetLanceDBRepository(
        collection_name="my_collection", database_uri="temp/orca.db"
    )
    # Can parse a local Milvus URL
    assert LabeledMemorysetV2.repository_from_uri("./temp/milvus.db#my_collection") == MemorysetMilvusRepository(
        collection_name="my_collection", database_uri="./temp/milvus.db"
    )
    # Can parse a remote Milvus URL
    assert LabeledMemorysetV2.repository_from_uri(
    ...
    with mock.patch.dict("os.environ", {"MILVUS_URL": "http://milvus-standalone:19530"}):
        assert LabeledMemorysetV2.repository_from_uri("my_collection") == MemorysetMilvusRepository(
    ...
    with mock.patch.dict("os.environ", {"LANCEDB_URI": "~/.orca/lance.db"}):
        assert LabeledMemorysetV2.repository_from_uri("my_collection") == MemorysetLanceDBRepository(
            collection_name="my_collection", database_uri="~/.orca/lance.db"
        )
    # Throws when collection name is invalid
    with pytest.raises(ValueError):
        LabeledMemorysetV2.repository_from_uri("lance.db#my.collection")
    with mock.patch.dict("os.environ", {"MILVUS_URL": "http://milvus-standalone:19530"}):
        with pytest.raises(ValueError):
            LabeledMemorysetV2.repository_from_uri("my/collection")
...
@pytest.mark.parametrize("repository_type", BACKEND_TYPES)
def test_create_new_memoryset(temp_folder, repository_type):
    ...
    match repository_type:
        ...
        case "milvus":
            uri = f"{temp_folder}/milvus.db#memories"
        case _:
            raise ValueError(f"Unknown storage backend type: {repository_type}")
    memoryset = LabeledMemorysetV2(uri, EmbeddingModel.CLIP_BASE)
    # Then the correct storage backend is inferred from the URI
    match repository_type:
        case "lancedb":
            assert isinstance(memoryset.repository, MemorysetLanceDBRepository)
        case "milvus":
...
@pytest.mark.parametrize("repository_type", BACKEND_TYPES)
def test_reconnect_to_existing_memoryset(temp_folder, repository_type):
...
def test_clone_to_different_repository(temp_folder):
...
def test_update_embedding_model(memoryset: LabeledMemorysetV2):
    ...
    destination = memoryset.repository.__class__("memoryset_gte", memoryset.repository.database_uri)
