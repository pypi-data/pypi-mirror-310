...
from .return_types import AnalyzeResult
...
class FinetunePlot:
    ...
    def update(
        ...
        with self.fig_widget.batch_update():
            self.fig_widget.data[0].x = fractional_epochs
            self.fig_widget.data[0].y = loss_values
            self.fig_widget.data[1].x = fractional_epochs
            self.fig_widget.data[1].y = mean_loss_values
            self.fig_widget.data[2].x = list(range(0, len(validation_scores)))
            self.fig_widget.data[2].y = validation_scores
