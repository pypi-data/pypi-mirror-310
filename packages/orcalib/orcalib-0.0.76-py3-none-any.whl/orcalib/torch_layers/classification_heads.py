...
class ClassificationHead(ABC, nn.Module):
    def __init__(self, num_classes: int):
        ...
        self.last_memories_attention_weights: Tensor | None = None  # batch_size x num_memories
    ...
    def forward(
        ...
        Args:
            memories_labels: class labels of the memories, long tensor of shape batch_size x num_memories
            input_embeddings: embedding of the model input, float tensor of shape batch_size x embedding_dim
            memories_embeddings: embeddings of the memories, float tensor of shape batch_size x num_memories x embedding_dim
            memories_weights: optional weights for each memory should be between 0 and 1, float tensor of shape batch_size x num_memories
...
class MemoryMixtureOfExpertsClassificationHead(ClassificationHead):
    ...
    def forward(
        ...
        )  # batch_size x num_memories x num_classes
...
class NearestMemoriesClassificationHead(ClassificationHead):
    ...
    def __init__(
        ...
        Args:
            ...
            use_softmax: whether to use softmax instead of normalizing the logits, use this if your
                memory weights are not probabilities
        """
        super().__init__(num_classes)
    ...
    def forward(
        ...
        if self.min_memory_weight is not None:
            assert memories_weights is not None
            # set memory weights that are less than min_memory_weight to 0
            self.last_memories_attention_weights = (
                self.last_memories_attention_weights * (memories_weights >= self.min_memory_weight).float()
            )
        ...
        if self.use_softmax:
            ...
            logits = logits / (logits.sum(dim=-1, keepdim=True) + 1e-8)
        return logits
