...
def test_mmoe_head():
    ...
    assert memories_labels.shape == (batch_size, num_memories)
    input_embeds = torch.rand(batch_size, embedding_dim)
    memories_embeds = torch.rand(batch_size, num_memories, embedding_dim)
    ...
    # And the last memory weights should be a tensor of shape batch_size x num_memories
    assert mmoe_head.last_memories_attention_weights is not None
    assert mmoe_head.last_memories_attention_weights.shape == (batch_size, num_memories)
...
def test_weighted_knn_with_softmax():
    # Given a weighted KNN head with softmax
    num_classes = 3
    batch_size = 2
    num_memories = 5
    knn_head = NearestMemoriesClassificationHead(num_classes=num_classes, weigh_memories=True, use_softmax=True)
    # And a batch of memory labels and weights that include negative values
    memories_labels = torch.tensor([[1, 0, 0, 0, 2], [1, 1, 1, 2, 2]])
    assert memories_labels.shape == (batch_size, num_memories)
    memories_weights = torch.tensor([[2.0, -0.1, -0.8, -1.0, -0.1], [0.1, -2.0, -3.0, -4.0, 10.0]])
    assert memories_weights.shape == (batch_size, num_memories)
    # When the forward method is called
    logits = knn_head.forward(memories_labels=memories_labels, memories_weights=memories_weights)
    # Then the logits should be a tensor of shape batch_size x num_classes
    ...
    # And all logits should be positive
    assert (logits > 0).all()
