...
from ..plotting.main_window import newWindow
...
class inversion():
    ...
    def __init__(self, data, x, y, z, earth=None, data_type="m"):
        ...
        Parameters
        ----------
        data : list of one two 1D or 2D numpy float arrays
...
# respective arrays
    ...
    def run_inversion(self):
        ...
        if self.dim == 2:
            ...
            prism_add = 7
# Start inversion
        with open("prism_control.dat", "w") as fo, \
                open("par_hist.dat", "w") as fp:
            while True:
                self.iteration += 1
                if self.max_iter == 0:
...
#    C_data = np.diag(sigma_data)
#    C_param = np.diag(sigma_param)
...
#     an appreciable effect. If not, give a warning message
                if self.iteration == 0:
                    ...
                    G_Sig = mG/mSig
                    G_Smo = mG/mSmo
                    # fo.write(f"Maximum G_inv: {mG}, Maximum sigma: {mSig}, "
                    #          + f"Maximum smoothing: {mSmo}\n")
                    # fo.write(f"G_inv/sigma: {G_Sig}, G_inv/smooth: {G_Smo}\n")
...
#   values in the prism parameters and copy them into vector par_hist
                ...
                # fp.write(f"\nIteration {self.iteration}, average: "
                #          + "{self.params[-1]:0.2f}\n")
                # for key, val in self.mPrism.prisms.items():
                #     fp.write(f"{key}: sus={val.sus*1000*self.pi4:0.3f}*10-3 SI"
                #              + f" x={(val.x[0]+val.x[1])/2.:0.0f} "
                #              + f"y={(val.y[0]+val.y[1])/2.:0.0f} "
                #              + f"z={(val.z[0]+val.z[1])/2.:0.0f}\n")
# Calculate magnetic misfits and some statistics
                ...
                # fo.write("   Relative Std misfit [%]: "
                #          + f"{self.std_data_rel*100}\n")
...
# plots later on.
                ...
                # fo.write(f"     Prisms to split: {key_split}\n")
# Test whether prisms are marked for splitting
                if len(key_split) == 0:
                    print("  Prisms reached size limit\nIteration stops")
                    return
# Plot positions of prisms to be split
                # data = data[:self.n_data1].reshape(self.data1_shape)
                # self.fig_follow.append(newWindow("Split prisms", 800, 500))
                # self.fig_follow[-1].fig.tight_layout()
                # self.ax = self.fig_follow[-1].fig.subplots(1, 1)
                # br_map, norm = utils.mag_color_map(np.nanmin(data),
                #                                    np.nanmax(data))
                # im, cbar = utils.data_plot(
                #     data, self.fig_follow[-1].fig, self.ax,
                #     title="Magnetic difference sensor1",
                #     cbar_title="nT", cmap=br_map, norm=norm,
                #     extent=[xplt_min, xplt_max, yplt_min, yplt_max])
                # for pos in maxima:
                #     self.ax.plot(x_col[pos[1]], y_row[pos[0]], "k*")
                # for key in key_split:
                #     x1 = self.mPrism.prisms[key].x[0]
                #     x2 = self.mPrism.prisms[key].x[1]
                #     y1 = self.mPrism.prisms[key].y[0]
                #     y2 = self.mPrism.prisms[key].y[1]
                #     self.ax.plot([x1, x2, x2, x1, x1], [y1, y1, y2, y2, y1])
                # self.fig_follow[-1].show()
                # event = self.w.get_mouse_click(self.fig_follow[-1].fig)
                # self.fig_follow[-1].fig.savefig(
                #     f"prism_follow{len(self.fig_follow)}.png")
...
# eliminated. Therefore the factor 7 in the next line.
                ...
                for _, key in enumerate(key_split):
                    ...
                    # fo.write(f" \nReplace prism {key}:\n     "
                    #          + f"x:{self.mPrism.prisms[key].x}, "
                    #          + f"y:{self.mPrism.prisms[key].y}, "
                    #          + f"z:{self.mPrism.prisms[key].z}, "
                    #          + f"sus:{self.mPrism.prisms[key].sus*self.pi4}\n"
                    #          + "  by\n")
                    ...
                    for k in key_add:
                        ...
                        # fo.write(f"   prism {k}:\n    "
                        #          + f"x:{self.mPrism.prisms[k].x}, "
                        #          + f"y:{self.mPrism.prisms[k].y}, "
                        #          + f"z:{self.mPrism.prisms[k].z}, "
                        #          + f"sus:{self.mPrism.prisms[k].sus*self.pi4}"
                        #          + "\n")
...
# Save inversion control parameters to file parameters.dat
            ...
            while True:
                event = self.w.get_event(self.fig_inv2.fig)
                self.fig_inv2.close()
...
# Plot model parameter distribution
        ...
        self.w.setHelp("Press ENTER to finish, R to continue iterations",
                       self.fig_par)
        self.fig_par.show()
...
# Plot misfit evolution
        ...
        # _ = self.w.get_mouse_click(self.fig_par.fig)
        while True:
            event = self.w.get_event(self.fig_par.fig)
            if event.name == "key_press_event":
...
# plot prism contours
        ...
        _ = self.w.get_mouse_click(self.fig_syn.fig)
        self.fig_syn.close()
