...
from .plotting.floating_plots import newWindow
...
#   defined, create earthMag calss and store the field data.
                ...
                self.earth = io.get_mag_field(line_dir, 0., 0., 0.)
            ...
            if not self.config_flag:
                with open(fconfig, "w") as fo:
                    ...
                    if self.dat[-1].data["grad_data"]:
                        ...
                        self.dist_sensors = \
                            self.dat[-1].data["height"] - self.height2
                    else:
...
# Skip already joint data sets (they contain the character "+" in their name)
            ...
            for key, val in d.data.items():
                ...
                if add_flag:
                    ...
                    if self.dat[-1].data[il]["direction"] in\
                            ("N", "S", 0., 180.):
                        _, index = np.unique(self.dat[-1].data[il]["y"],
                                             return_index=True)
                    else:
                        _, index = np.unique(self.dat[-1].data[il]["x"],
                                             return_index=True)
                    self.dat[-1].data[il]["x"] =\
                        self.dat[-1].data[il]["x"][index]
                    self.dat[-1].data[il]["y"] =\
    ...
    def join_gridded(self):
        ...
        self.x_inter = np.arange(xmin, xmax+self.dx/2., self.dx)
        self.y_inter = np.arange(ymin, ymax+self.dy/2., self.dy)
        nx = len(self.x_inter)
        ny = len(self.y_inter)
    ...
    def original(self):
        ...
        self.data.set_values(earth=self.earth, treatments=self.treatments,
                             inter_flag=False)
        self.plotActual()
    ...
    def plotOriginal(self):
        ...
        self.fig, self.ax =\
            self.w.plot_triang(
                data, title=f"{self.data.data['title']}, ",
                percent=self.percent, mincol1=self.mincol1,
                maxcol1=self.maxcol1, mincol2=self.mincol2,
                maxcol2=self.maxcol2, mincolg=self.mincolg,
                maxcolg=self.maxcolg, grad_flag=self.gradient_flag,
                c=self.color)
    ...
    def plotActual(self):
        ...
        if self.data.inter_flag:
            ...
            self.fig, self.ax =\
                self.w.plot_image(self.data, title=title, percent=self.percent,
            ...
            if title[-1] == ",":
                title = title[:-1]
            self.fig, self.ax =\
                self.w.plot_triang(self.data, title=title,
...
# Wait for mouse click to choose line to be plotted first
    ...
    def diurnal(self):
        ...
        result = u.diurnal_correction(self.data, self.base.base,
                                      base_flag=self.base_flag, degree=deg)
        if result:
    ...
    def inver2D(self):
        ...
        earth.dec -=\
            self.data.data["line_declination"]
        data = []
        x = []
        y = []
        z = []
...
# Set overall data and parameter variances
    ...
    def inver3D(self):
        ...
        if self.inter_flag:
            ...
            inv = inversion(data, x, y, z, earth=earth,
                            data_type=data_type)
# Get area of initial prisms and extract data to be inverted
            ret = inv.get_area3D()
            if not ret:
...
# Do inversion
    ...
    def synthetic_model(self):
            ...
            while True:
                results, okButton = dialog(
                    [f"Prism {nprism+1}\n\n", "xmin [m]", "xmax [m]",
                     ...
                     "remanence declination [°]", "remanence inclination [°]",
                     "density [kg/m3]"],
                    ["l", "e", "e", "e", "e", "e", "e", "l", "e", "e", "e",
                     "e", "e"],
                    ["b", 0., 50., 0., 50., 1., 5., None, s, 0., 0., 0., r],
                    "Synthetic model")
                if not okButton:
                    if nprism == 0:
        ...
        ymin = self.data.ymin
        ymax = self.data.ymax
        dx = np.round((xmax - xmin)/50., 0)
        dy = np.round((ymax - ymin)/50., 0)
        ...
        earth.dec -=\
            self.data.data["line_declination"]
        xx = np.arange(xmin, xmax+dx/2., dx)
        yy = np.arange(ymin, ymax+dy/2., dy)
