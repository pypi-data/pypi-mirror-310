...
class StepContext:
    ...
    def __enter__(self):
        ...
        """
        if self.on_context_enter:
            try:
                self.on_context_enter(self.step_id)
            except Exception as e:
                logger.error(f"Error running on_context_enter: {e}")
    ...
    def __exit__(
        ...
        if self.on_context_exit:
            try:
                self.on_context_exit()
            except Exception as e:
                logger.error(f"Error running on_context_exit: {e}")
