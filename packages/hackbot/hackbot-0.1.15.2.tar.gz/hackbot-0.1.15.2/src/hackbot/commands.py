...
def get_args() -> argparse.Namespace:
    ...
    if not get_repo(args.source):
        log.error(
            f"❌ Error: The source folder is not the root of a git repository ({args.source} is not a git repository)"
        )
        return 1
    if not os.path.exists(os.path.join(args.source, "foundry.toml")):
        log.error(
            f"❌ Error: The source folder is not the root of a proper foundry project (foundry.toml not found in {args.source})"
        )
        return 1
...
def hackbot_run(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        results = asyncio.run(
            cli_hack_target(args.address, args.port, args.api_key, args.source, args.output)
        )
