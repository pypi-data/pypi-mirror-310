...
class myUniversalRobot:
    ...
    def moveJ(self, _angles, acc=1.0, vel=0.5, duration=None, unit_is_DEG=True):
        if not self.connected:
            return
        if unit_is_DEG:
            _angles = np.deg2rad(_angles)
        if duration is None:
            command = 'movej({angles}, a={acc}, v={vel})'.format(angles=_angles.tolist(), acc=acc, vel=vel)
            self.s.send(self._toByte(command))
        elif duration > 0:
            command = 'movej({angles}, t={time})'.format(angles=_angles.tolist(), time=duration)
            self.s.send(self._toByte(command))
            time.sleep(duration)

    def moveL(self, position, acc=1000, vel=150, duration=None, unit_is_DEG=False):
        if not self.connected:
            return
        _goal_position = np.zeros(6)
        _goal_position[self.Pos.x:self.Pos.z+1] = position[self.Pos.x:self.Pos.z+1]/1000
        if unit_is_DEG:
            _goal_position[self.Pos.rx:self.Pos.rz+1] = np.deg2rad(position[self.Pos.rx:self.Pos.rz+1])
        else:
            _goal_position[self.Pos.rx:self.Pos.rz+1] = position[self.Pos.rx:self.Pos.rz+1]
        if duration is None:
            command = 'movel(p{position}, a={acc}, v={vel})'.format(position=_goal_position.tolist(), acc=acc/1000, vel=vel/1000)
            self.s.send(self._toByte(command))
        elif duration > 0:
            command = 'movel(p{position}, t={time})'.format(position=_goal_position.tolist(), time=duration)
            self.s.send(self._toByte(command))
            time.sleep(duration)
    ...
    def isMoving(self, th=0.1):
        if np.linalg.norm(self.joint_velocity) > th:
            return True
        else:
            return False
