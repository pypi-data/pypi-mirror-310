...
class AlaMake:
    ...
    def get_port(self, vid_pid):
        try:
            ...
            svc.log.warn('module pyserial is not installed')
            return None
        ...
        for port, desc, hwid in sorted(ports):
            if vid_pid in hwid:
                return port
            svc.log.line(f'port {port}: {desc} [{hwid}]')
        svc.log.err(f'vid-pid not found: "{vid_pid}"')
        svc.log.err('check USB is connected and powered on')
        return None
