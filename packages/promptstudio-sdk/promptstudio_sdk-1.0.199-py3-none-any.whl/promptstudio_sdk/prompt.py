...
class PromptManager(Base):
    ...
    async def summarizedmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            summary_task = self._create_summary_in_thread(
                platform_name=prompt_details["ai_platform"],
                prompt_details=prompt_details,
            ...
            create_summary = await summary_task
            ...
            # Save to persistent cache in a separate thread
            await self._execute_in_thread(
                self._save_interaction_data, cache_key, interaction_data
            )
    ...
    async def _create_summary_in_thread(
        self,
        platform_name: str,
        prompt_details: Dict,
        messages: List,
        ...
        """Execute create_summary in a thread"""
        return await self._make_ai_platform_request(
            platform_name=platform_name,
            prompt_details=prompt_details,
            messages=messages,
    ...
    def _save_interaction_data(self, cache_key: str, interaction_data: Dict):
        """Save interaction data to persistent cache"""
        self._save_to_persistent_cache(cache_key, interaction_data)

    def _create_summary_and_save(
        self,
        platform_name: str,
        prompt_details: Dict,
        messages: List,
        ...
        """Create summary and save interaction data in a single thread"""
        try:
            ...
            try:
                summary_response = loop.run_until_complete(
                    self._make_ai_platform_request(
                        platform_name=platform_name,
                        prompt_details=prompt_details,
                        messages=messages,
                        system_message=system_message,
            ...
            logger.error(f"Error in create_summary_and_save: {str(e)}")
            raise
