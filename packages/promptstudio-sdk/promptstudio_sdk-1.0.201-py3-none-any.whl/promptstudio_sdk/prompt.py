...
class MessageQueue:
    ...
    async def _process_queue(self):
        ...
        try:
            while self.queue and not self._shutdown:
                ...
                try:
                    # Use ThreadPoolExecutor to run the store operation
                    loop = asyncio.get_event_loop()
                    await loop.run_in_executor(
                        self.executor, self._store_message, message_data
                    )
    ...
    def _store_message(self, message_data: Dict):
        """Store message in PromptStudio database synchronously"""
        try:
            ...
            try:
                # Run the request synchronously in this thread
                loop.run_until_complete(
                    self.prompt_manager.request(
                        f"/save_bypass_logs/{message_data['user_prompt_id']}",
                        method="POST",
                        json={
                            "user_message": message_data["user_message"],
                            "ai_response": message_data["ai_response"],
                            "session_id": message_data["session_id"],
                            "memory_type": message_data["memory_type"],
                            "window_size": message_data["window_size"],
                            "summarized_content": message_data["summarized_content"],
                            "request_from": message_data["request_from"],
                        },
                    )
                )
            finally:
...
class PromptManager(Base):
    ...
    # async def _store_messages_to_promptstudio(
    #     self,
    #     user_prompt_id: str,
    #     session_id: str,
    ...
    # ) -> None:
    #     """Asynchronously store messages in PromptStudio database"""
    #     try:
    #         payload = {
    ...
    #         await self._request(
    #             f"/store_messages/{user_prompt_id}",
    #             method="POST",
    #             json=payload
    #         )
    #     except Exception as e:
    #         # Log error but don't raise exception since this is background task
    #         logger.error(f"Failed to store messages in PromptStudio: {str(e)}")
    ...
    def update_messages_collection(self, platform_name, system_message, old_messages):
        logger.info(f"platform_name update_messages_collection: {platform_name}")
        logger.info(f"system_message update_messages_collection: {system_message}")
        logger.info(f"old_messages update_messages_collection: {old_messages}")
        messages_collection = []
        ...
        if platform_name == "gemini":
            ...
            if old_messages and len(old_messages) > 0:
                for message in old_messages[0].get("messages", []):
                    ...
                    messages_collection.append(simplified_message)
        logger.info(
            f"messages_collection update_messages_collection @@@@@: {messages_collection}"
        )
    ...
    async def old_summarizedmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(f"prompt_details summary: {prompt_details}")
            ...
            logger.info(f"old_messages: {old_messages}")
            ...
            logger.info(f"messages_collection: {messages_collection}")

            if session_type == "new_session":
                logger.info(f"new_session: {new_user_message}")
                ...
                logger.info(f"existing_session: {new_user_message}")
                content = [
                    {
                        "type": "text",
                ...
                for message in new_user_message:
                    logger.info(f"message 2232323: {message}")
            ...
            logger.info(
                f'prompt_details["response_format"]: {prompt_details["response_format"]}'
            )

            logger.info(
                f"prompt_details['ai_platform']: {prompt_details['ai_platform']}"
            )
            ...
            logger.info(f"summary_message_collection: {summary_message_collection}")
            ...
            logger.info(f"create_summary: {create_summary}")
    ...
    async def summarizedmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            summary_messages.append(
                {
                    "role": "user",
                    "content": [
                        {"type": "text", "text": PROMPT_TO_GENERATE_SUMMARIZED_CONTENT}
                    ],
                }
            )

            # Make another request for summary using the same response object
            summary_prompt_details = prompt_details.copy()
            summary_prompt_details["system_prompt"] = (
                PROMPT_TO_GENERATE_SUMMARIZED_CONTENT
            )

            summary_response = await self._make_ai_platform_request(
                platform_name=prompt_details["ai_platform"],
                prompt_details=summary_prompt_details,
                messages=summary_messages,
                system_message=PROMPT_TO_GENERATE_SUMMARIZED_CONTENT,
                platform_key=platform_key,
                variables=variables,
            )

            if prompt_details["ai_platform"] == "claude":
                new_summarized_content = summary_response["content"][0]["text"]
                ai_response = {"type": "text", "text": response1}
            else:
                new_summarized_content = summary_response["response"]
                ai_response = {"type": "text", "text": response1}

            # Create new messages for caching
            current_time = datetime.now().isoformat()
            new_messages = [
                {
                    "id": str(uuid.uuid4()),
            ...
            # Prepare cache data
            cache_key = f"{user_prompt_id}_{interaction_request['session_id']}"
            interaction_data = {
                "messages": new_messages,
                "lastResponseAt": current_time,
                "memory_type": "summarizedMemory",
            ...
            # Run cache save and summary generation in background
            asyncio.create_task(
                self._background_cache_save(
                    cache_key,
                    interaction_data,
                    prompt_details,
                    messages_collection,
            ...
            # Return result immediately with just the response
            result = {
                "message": "AI interaction saved successfully for memory type: summarized memory",
                "user_prompt_id": str(user_prompt_id),
                "response": {
                    "response1": response1
                },  # Summary will be available in next interaction
    ...
    async def _background_cache_save(
        self,
        cache_key: str,
        interaction_data: Dict,
        prompt_details: Dict,
        messages_collection: List,
        response1: str,
        platform_key: str,
        variables: Dict,
    ):
        """Save cache data and generate summary in background with error handling"""
        try:
            # Generate summary using the same messages plus the new response
            summary_messages = messages_collection.copy()
            summary_messages.append(
            ...
            try:
                ...
                # Update interaction data with new summary
                interaction_data["summarized_content"] = new_summarized_content

            except Exception as e:
                logger.error(f"Error generating summary in background: {str(e)}")
                # Use empty summary if generation fails
                interaction_data["summarized_content"] = ""

            # Save to persistent cache
            self._save_to_persistent_cache(cache_key, interaction_data)

        except Exception as e:
            logger.error(f"Error in background cache save: {str(e)}")
    ...
    async def _no_cache_direct_ai_request(
        ...
        try:
            ...
            logger.info(result["messages"]["messages"])
            messages = result["messages"]["messages"]
            messages.extend([{"role": "user", "content": user_message}])

            platform_config = result.get("platformConfig", {})

            # logger.info(f"systemMessage222: {prompt['messages']['systemMessage']}")

            # Extract and format the prompt details
            prompt_details = {
                "ai_platform": ai_platform["platform"],
                "model": ai_platform["model"],
                "system_prompt": result["messages"]["systemMessage"],
                "temperature": ai_platform["temp"],
                "max_tokens": ai_platform["max_tokens"],
    ...
    async def chat_with_prompt(
        ...
        try:
            logger.info(f"is_session_enabled: {is_session_enabled}")
            if self.bypass:
                ...
                return OKResponse(
                    content=response,
                    message="AI interactions successful !!",
                )
        except Exception as e:
...
class OKResponse(JSONResponse):
    def __init__(self, content: Optional[Any] = None, message: str = ""):
        if content is not None:
            content = {"message": message, "data": convert_object_ids(content)}
        else:
            content = {"message": message}
        super().__init__(content=content, status_code=status.HTTP_200_OK)
...
def gemini_interaction_chat_with_prompt(
    ...
    logger.info(f"Generated history: {generated_history}")

    # Prepare the message to send
    last_message_parts = generated_history[-1]["parts"]
    logger.info(f"last_message_parts 11: {last_message_parts}")
    ...
    logger.info(f"last_message_parts 22: {last_message_parts}")
    ...
    try:
        ...
        logger.info(f"Assistant reply 11111111111: {assistant_reply}")
        # assistant_reply = json.loads(assistant_reply)
