...
class PromptManager(Base):
    ...
    async def _no_cache_direct_ai_request(
        ...
        try:
            ...
            messages = messages.extend(user_message)
            platform_config = result.get("platformConfig", {})

            logger.info(f"result: {result}")

            logger.info(f"messages: {messages}")
            logger.info(f"ai_platform: {ai_platform}")
            logger.info(f"systemMessage111: {result['messages']['systemMessage']}")
            ...
            return await self._make_ai_platform_request(
                ...
                prompt_details=prompt_details,
                messages=messages,
                system_message=messages.get("systemMessage", ""),
...
def gemini_interaction_chat_with_prompt(
    ...
    [
        {
            "role": "user",
            "parts": ["Hello, I have a patient Narpat, check his face for symptoms"],
        },
        {
            "role": "model",
            "parts": [
                "{'action': 'information_gathering', 'diagnosis': [], 'doctorDetails': {'name': ''}, 'familyHistory': [], 'history': [], 'patientDetails': {'age': 0, 'firstName': 'Narpat', 'gender': 'male', 'height': 0, 'lastName': '', 'weight': 0}, 'response': 'Could you please describe the symptoms Narpat is experiencing on his face?  Additionally, please provide some background information about Narpat, including his age, any relevant medical history, and family history.', 'symptoms': [], 'treatment': []}"
            ],
        },
        {
            "role": "user",
            "parts": [
                "Summarize the above conversation in a detailed, concise, and well-structured manner. ensuring the summary does not exceed 250 words. Capture the key points and context, including important questions, answers, and relevant follow-up, while avoiding unnecessary repetition. Present the summary in a well-structured paragraph, focusing on the most important content exchanged"
            ],
        },
    ]
    ...
    try:
        ...
        assistant_reply = json.loads(assistant_reply)
        logger.info(f"Assistant reply 22222222222: {assistant_reply}")
