...
class PromptManager(Base):
    ...
    async def windowmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            logger.info(
                f"is_session_enabled windowmemory_save_log_ai_interaction_prompt  {is_session_enabled}"
            )
            ...
            logger.info(
                f"response123 windowmemory_save_log_ai_interaction_prompt  {response}"
            )
    ...
    async def fullmemory_save_log_ai_interaction_prompt(
        ...
        try:
            logger.info(
                f"is_session_enabled fullmemory_save_log_ai_interaction_prompt  {interaction_request.get('is_session_enabled')}"
            )
            ...
            logger.info(
                f"interaction_request fullmemory_save_log_ai_interaction_prompt  {interaction_request}"
            )

            is_session_enabled = interaction_request.get("is_session_enabled")
            logger.info(
                f"is_session_enabled fullmemory_save_log_ai_interaction_prompt  {is_session_enabled}"
            )
            prompt_details["is_session_enabled"] = is_session_enabled
            ...
            logger.info(
                f"prompt_collection_msg fullmemory_save_log_ai_interaction_prompt  {prompt_collection_msg}"
            )
            ...
            logger.info("response _make_ai_platform_request")
    ...
    async def summarizedmemory_save_log_ai_interaction_prompt(
        ...
        try:
            ...
            if prompt_details["ai_platform"] == "claude":
                logger.info(
                    f"is_session_enabled claude summarizedmemory_save_log_ai_interaction_prompt {response}"
                )
                ...
                logger.info(
                    f"summary_response claude summarizedmemory_save_log_ai_interaction_prompt {summary_response}"
                )
                if (
    ...
    async def _make_openai_request(
        ...
        try:
            ...
            logger.info(prompt_details.get("is_session_enabled"))
    ...
    async def _no_cache_direct_ai_request(
        ...
        try:
            ...
            logger.info(f"response _no_cache_direct_ai_request  {response}")
            if not response.get("data") or not response["data"].get("result"):
                logger.error(f"Invalid response format for prompt_id: {prompt_id}")
                raise ValueError("Invalid response format from API")
            ...
            logger.info(f"platform_config   {platform_config}")
    ...
    async def chat_with_prompt(
        ...
        logger.info(f"chat_with_prompt  {prompt_id}")
        logger.info(f"is_session_enabled zzzzzzzzzz  {is_session_enabled}")
        ...
        logger.info(f"bypass payload  {self.bypass}")
        ...
        try:
            if self.bypass:
                ...
                if memory_type == "summaryMemory":
                    logger.info(f"memory_type summaryMemory payload  {payload}")
                    # raise ValueError(
                    ...
                    logger.info(f"memory_type windowMemory payload  {payload}")
                    return await self.windowmemory_save_log_ai_interaction_prompt(
                        prompt_id, payload
                    )
                elif memory_type == "fullMemory":
                    logger.info(f"memory_type fullMemory payload  {payload}")
    ...
    async def _make_ai_platform_request(
        ...
        logger.info(
            f"is_session_enabled _make_ai_platform_request 123  {prompt_details.get('is_session_enabled')}"
        )

        logger.info(f"prompt_details _make_ai_platform_request 123  {prompt_details}")
...
def gemini_interaction_chat_with_prompt(
    ...
    try:
        ...
        # assistant_reply = json.loads(assistant_reply)
        logger.info(
            f"gemini_interaction_chat_with_prompt assistant_reply {assistant_reply}"
        )
