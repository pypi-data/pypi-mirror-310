...
def add_ome_zarr_group_from_path(viewer: napari.Viewer, path: str, use_zip: bool | None = None,
    ...
    return add_ome_zarr_group(viewer,
                              zarr_group,
...
def add_ome_zarr_array_from_path(viewer: napari.Viewer, path: str, use_zip: bool | None = None,
    ...
    zarr_group = ome_io.load_zarr_group_from_path(path, 'r', use_zip)
    return add_ome_zarr_array(viewer, zarr_group, merge_channels=merge_channels, **kwargs)
...
def add_ome_zarr_group(viewer: napari.Viewer,
    ...
    layer = None
    if '0' in zarr_group:
        layer = add_ome_zarr_array(viewer,
                                   zarr_group,
                                   merge_channels=merge_channels,
                                   **kwargs)
    if 'labels' in zarr_group:
        ...
        for group_key in lbls_group.group_keys():
            ...
            cur_layer = add_ome_zarr_array(viewer, lbl_group, name=group_key, **lbl_kwargs)
            if layer is None:
                layer = cur_layer
    return layer
...
def add_ome_zarr_array(viewer: napari.Viewer,
    ...
    if kwargs.get('array_slices') is not None:
        slices = kwargs.get('array_slices')
