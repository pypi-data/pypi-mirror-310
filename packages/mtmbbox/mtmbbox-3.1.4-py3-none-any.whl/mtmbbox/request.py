...
class BBoxCryptoFromQujing(QujingByHttp):
    def __init__(self, ip, invoke_port=None):
        super().__init__(ip)
        self.invoke_port = invoke_port

    def encrypt(self, strs: str, typeint: int):
        params = BBoxQujingFunctionParams.ea3(strs, typeint)
        resp = self.invoke(self.invoke_port, params)
        return resp
...
class BBoxRequest:
    ...
    def encrypt_before_request(data: dict, crypto: BBoxCryptoTemplate, typeint: int, **kwargs):
        ...
        strs = None
        is_by_http = False
        if "encryptdictdb" in kwargs and str_json_request in kwargs["dictdb"]:
            strs = kwargs["encryptdictdb"][str_json_request]
        ...
        # 将结果依次存入数据库
        encrypt_save = kwargs.get("encryptsave", True)
        if encrypt_save and is_by_http:
            if "encryptdictdb" in kwargs:
                kwargs["encryptdictdb"][str_json_request] = strs
            if "encryptredisdb" in kwargs:
                key = f"{typeint}#{str_json_request}"
    ...
    def decrypt_after_response(data: bytes, crypto: BBoxCryptoTemplate, typeint: int, **kwargs):
        ...
        # 优先从数据库中查找, 如果没有则使用crypto.encrypt对象去请求
        str_json_response = None
        is_by_http = False
        if "decryptdictdb" in kwargs and str_base64_response in kwargs["dictdb"]:
            str_json_response = kwargs["decryptdictdb"][str_base64_response]
        ...
        # 将结果依次存入数据库
        decrypt_save = kwargs.get("decryptsave", True)
        if decrypt_save and is_by_http:
            if "decryptdictdb" in kwargs:
                kwargs["dictdb"][str_base64_response] = str_json_response
            if "decryptredisdb" in kwargs:
                key = f"{typeint}#{str_base64_response}"
    ...
    @staticmethod
    def request(method: str, url: str, data: dict, crypto, typeint, **kwargs):
        # 更新请求头
