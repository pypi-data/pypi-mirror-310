    ...
    def __init__(
        ...
        if self.with_topas_control:
            try:
                ...
                self.tctrl = WinTopas(ip_address, dev_sn=dev_sn, port=kwargs.get('topas_port', 8000))
            except Exception as excpt:
            ...
            if self.tctrl is not None and self.tctrl.connected:
                ...
                raise RuntimeError("Could not connect to Topas4 server")
...
class C3Pv2Control(C3PControl_Base):
    def __init__(self, ip_address, dev_sn=None, **kwargs):
        self.hw_ver = 'c3pv2'
        super().__init__(ip_address, dev_sn=dev_sn, **kwargs)

        # Determine analog input adu to Volts factor
        try:
            file_name = "C:\Programs\WinTopas4\Resources\SelfHostedServer" + \
                    f"\{dev_sn}\Configuration\Settings\LightComposer" + \
                    "\Channels\IDL\ExternalAttenuatorControl.json"
            self.ain_adu_per_volt = json.load(open(file_name, 'r'))['InputAduPerVolt']
        except Exception as excpt:
