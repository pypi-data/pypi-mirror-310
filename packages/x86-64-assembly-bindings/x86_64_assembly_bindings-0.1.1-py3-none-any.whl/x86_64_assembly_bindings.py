...
class InstructionData(Enum):
    ...
    movsx:InstructionDataType = ("movsx", [[0, 0], [0, int], [0, str]], [0,0,0])
    movzx:InstructionDataType = ("movzx", [[0, 0], [0, int], [0, str]], [0,0,0])
    ...
    mul:InstructionDataType = ("mul", [[0], [int], [str]], ["0","0","0"])
    div:InstructionDataType = ("div", [[0], [int], [str]], [("0","0"),None,None])

    imul:InstructionDataType = ("imul", [[0], [int], [str]], ["0","0","0"])
...
class Instruction:
    def __init__(self, instruction:str|InstructionData, *arguments:list[Register|str|int|Variable|Block]):
        ...
        self.err_msg = None
        self.__ret = None
        if not self:raise Exception(f"Invalid instruction: \"{self}\".\nReason: {self.err_msg}")
    ...
    def __get_ret(self, r_ind:int):
        ...
        if isinstance(index, int):
            ...
            ind_1, ind_2 = tuple([int(i) for i in index])
            return Register(RegisterData.from_size(self.arguments[ind_1].size))
        elif index is None:
            return None
...
class Function(Block):
    ...
    def __init__(self, arguments:list[Register|None], signed_args:set[int] = {}, return_register:Register|None = None, label:str|None = None):
...
if __name__ == "__main__":
    ...
    print(Program.CURRENT.write())

    Program.CURRENT.save("./test.asm")
