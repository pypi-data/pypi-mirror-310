...
def query_df(df: pd.DataFrame, **filters: Union[str, List[Any], Dict[str, Any]]) -> pd.DataFrame:
    ...
    ranges, and lists. Supports escaping for literal wildcards.
    ...
    **filters : dict
        ...
        - Ranges: {"column": {"gte": min_value, "lte": max_value}}
          Ranges also support wildcards, e.g., {"column": {"neq": "value*"}}.
    ...
    def _apply_condition(column: str, condition: Any) -> pd.Series:
        ...
        condition : Any
            The condition to apply (exact match, wildcard, range, etc.).
