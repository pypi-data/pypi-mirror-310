...
class Agent(BaseModel):
    ...
    def __init__(self, openapi_spec_uri: str, auth_header: str = "", cache_dir: str = ".cache"):
        super().__init__()
        
        spec_path = self._get_local_spec_path(openapi_spec_uri)
    ...
    def _get_local_spec_path(self, spec_uri: str) -> Path:
        """Convert URI to local path, downloading if necessary."""
        parsed = urlparse(spec_uri)
        if parsed.scheme in ('http', 'https'):
            response = requests.get(spec_uri)
            response.raise_for_status()
            
            # Create temp file without specific extension
            with tempfile.NamedTemporaryFile(delete=False) as tmp:
                tmp.write(response.content)
                return Path(tmp.name)
        
        return Path(spec_uri)
