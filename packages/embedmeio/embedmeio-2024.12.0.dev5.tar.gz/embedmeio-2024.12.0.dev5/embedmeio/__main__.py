...
def find_executable(executable, path=DEFAULT_EMBEDME_PATH):
    scv = sysconfig.get_config_vars()
    bindir = Path(scv["BINDIR"]).stem
    file = (path / bindir / executable).with_suffix(scv["EXE"])
...
def create_venv():
    ...
    subprocess.run(["python", "-m", "pip", "install", get_embedme_package()], env=get_env())
    return True
