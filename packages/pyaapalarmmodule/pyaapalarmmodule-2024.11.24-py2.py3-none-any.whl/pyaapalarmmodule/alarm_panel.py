import logging
from pyaapalarmmodule import AAPModuleClient
...
class AAPAlarmPanel():
    """This class represents AAP IP / Serial Module alarm panel."""
        
    def __init__(self, connectionType, host, port=5002, code='0000',
    ...
    def start(self):
        """Connect to the AAP IP / Serial Module, and listen for events to occur."""
        if self._connectionType == 'ip':
            logging.info(str.format("Connecting to AAP IP Module using host: {0}, port: {1}", self._host, self._port))
        elif self._connectionType == 'serial':
            logging.info(str.format("Connecting to AAP Serial Module using serial port: {0}", self._port))
        self._client = AAPModuleClient(self, self._eventLoop)
        self._client.start()
