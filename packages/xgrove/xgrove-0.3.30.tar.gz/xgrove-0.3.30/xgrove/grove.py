...
class grove():
    ...
    def getGBM(self):
        grove = GradientBoostingRegressor(max_depth=1, n_estimators=max(self.ntrees), learning_rate=self.shrink, subsample=self.b_frac, random_state=self.seed, criterion='squared_error', min_impurity_decrease=0)
        grove.fit(self.data, self.surrTar)
