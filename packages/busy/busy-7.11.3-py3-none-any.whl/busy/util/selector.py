...
class Selector:
    ...
    @classmethod
    def get_criterium(cls, word, index_ok=True):
        for criterium_type in cls.CRITERIUM_TYPES:
            if criterium_type.match(word):
                if index_ok or not criterium_type.index:
                    return criterium_type(word)
        raise BusyError(f"Invalid selection criterium {word}")

    def __init__(self, args, index_ok=True):
        """Receives filters as arguments and initializes the selector. The
        index_ok parameter allows rejection of numeric (index-type) filters."""
        result = {}
        for arg in args:
            criterium = self.get_criterium(arg, index_ok)
