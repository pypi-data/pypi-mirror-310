...
def handle_content_response(response, encoding="utf-8", xml_auto_escape=True):
    """
    ...
    :type xml_auto_escape: bool
    :param xml_auto_escape: If True, this parameter allows to escape the
    bad unicode characters in the XML of the response (default: True)
    """
    try:
    ...
    # We escape some unicode characters that are not valid in XML
    if xml_auto_escape:
        decoded = escape_xml_invalid_chars(
            raw.decode(encoding), XML_INVALID_CHARS_REPLACEMENT
        )
        raw = decoded.encode(encoding)
...
def escape_xml_invalid_chars(xml, replacement=" "):
    """
    Replace the invalid unicode characters in the XML

    Note: the range of values is defined in `fnapy.config.XML_INVALID_CHARS`
    """
    invalid_ranges = [fr'{chr(low)}-{chr(high)}' for (low, high) in XML_INVALID_CHARS]
    xml_invalid_chars_regex = '[' + ''.join(invalid_ranges) + ']'
    return re.compile(xml_invalid_chars_regex).sub(replacement, xml)
