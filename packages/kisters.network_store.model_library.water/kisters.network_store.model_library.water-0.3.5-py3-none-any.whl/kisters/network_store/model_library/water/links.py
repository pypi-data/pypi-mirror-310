...
class HydraulicCrossSectionStation(Model):
    lr: float = Field(
...
class HydraulicLongitudinalStation(Model):
    roughness: float = Field(..., gt=0.0, description="Friction coefficient")
...
class LongitudinalDelimitedStation(Model):
...
class MuskingumLongitudinalStation(Model):
...
class MuskingumCungeLongitudinalStation(Model):
...
class ReservoirLongitudinalStation(Model):
...
class PumpTurbineHeadTWCorrection(Model):
    link_uid: str = Field(..., pattern="^[a-zA-Z]\\w*$")
    power: int = Field(..., ge=0.0)
    value: float


class PumpTurbineOtherConstraints(Model):
