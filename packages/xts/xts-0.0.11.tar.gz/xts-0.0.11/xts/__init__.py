        ...
        if not company_id:
            try:
                response = requests.patch(
                    headers=self.headers,
                    url=f"{self.base_url}/api/agent/{agent_name}/command",
            ...
            try:
                response = requests.patch(
                    headers=self.headers,
                    url=f"{self.base_url}/api/agent/{agent_name}/command/{company_id}",
                    json={"command_name": command_name, "enable": enable},
                )
                if self.verbose:
                    parse_response(response)
    ...
    def get_agent_extensions(self, agent_name: str = "XT", company_id: str = None):
        if not company_id:
            try:
                response = requests.get(
                    headers=self.headers,
                    url=f"{self.base_url}/api/agent/{agent_name}/extensions",
                )
                if self.verbose:
                    parse_response(response)
            ...
            try:
                response = requests.get(
                    headers=self.headers,
                    url=f"{self.base_url}/api/agent/{agent_name}/extensions/{company_id}",
                )
                if self.verbose:
                    parse_response(response)
                return response.json()["extensions"]
