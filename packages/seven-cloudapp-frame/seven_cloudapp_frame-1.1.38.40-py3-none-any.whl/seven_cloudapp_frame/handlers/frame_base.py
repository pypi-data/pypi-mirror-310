...
@LastEditTime: 2024-11-21 13:46:24
...
class FrameBaseHandler(FrameBaseApiHandler):
    ...
    def get_act_id(self):
        ...
        if act_id_config:
            act_info_model = ActInfoModel(context=self)
            act_info_dict = act_info_model.get_cache_dict(where=act_id_config.get("where","app_id=%s and is_release=1"), order_by=act_id_config.get("order_by","id desc"), field="id", params=act_id_config.get("params",[app_id]))
            if act_info_dict:
                act_id = act_info_dict["id"]
                return act_id
