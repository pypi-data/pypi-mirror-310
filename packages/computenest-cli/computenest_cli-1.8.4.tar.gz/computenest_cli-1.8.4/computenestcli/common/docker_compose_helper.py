...
class DockerComposeHelper:
    ...
    def generate_sed_commands(custom_parameters, docker_compose_path, specified_env_file_path):
        ...
        # 检查原env文件中是否有内容，如果有的话就复制到.env中
        default_env_path = os.path.join(docker_compose_dir, '.env')
        if not env_files:
            env_files.add(default_env_path)
            # 判断env_file_path中是否有设置环境变量，有的话就复制到.env中
            if specified_env_file_path and os.path.exists(specified_env_file_path):
                with open(specified_env_file_path, 'r') as file:
                    env_content = file.read()
                if env_content:
                    with open(default_env_path, 'w') as file:
