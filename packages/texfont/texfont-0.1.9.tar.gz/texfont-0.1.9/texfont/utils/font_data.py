...
def fetch_ttf_url_download_list_by_name(font_name, is_admin=False):
    ...
    if font_name not in all_font_names:
    ...
    for ofl in ofl_list:
        if ofl["name"] == font_name:
            ...
            url_content = json.loads(requests.get(ofl["url"], headers=HEADERS if is_admin else None).text)
