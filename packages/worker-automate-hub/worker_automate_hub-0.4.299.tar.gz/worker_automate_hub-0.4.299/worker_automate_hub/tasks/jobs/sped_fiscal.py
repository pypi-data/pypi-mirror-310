    ...
    try:
        ...
        try:
            ...
            if 'arquivo do sped ger' in captured_text.lower():
                ...
                upload_result_sped = send_file()

                if upload_result_sped.sucesso == True:
                    os.remove(temp_path_to_txt)
                    console.log(upload_result_sped.retorno, style="bold green")
                    return RpaRetornoProcessoDTO(
                        sucesso=True,
                        retorno="Arquivo do SPED gerado com sucesso e arquivo salvo no Google Drive !",
                        status=RpaHistoricoStatusEnum.Sucesso,
                    )
                else:
                    console.print(upload_result_sped.retorno, style="bold red")
                    return RpaRetornoProcessoDTO(
                        sucesso=False,
                        retorno=f"Arquivo do SPED gerado com sucesso, porém gerou {upload_result_sped.retorno} - Arquivo ainda salvo na dispositivo utilizado no diretório {temp_path_to_txt}",
                        status=RpaHistoricoStatusEnum.Falha,
                    )
            else:
                return RpaRetornoProcessoDTO(
