...
from sema4ai.di_client.document_processor_api.models.extracted_document_content import ExtractedDocumentContent
from sema4ai.di_client.document_processor_api.models.transformed_document_content import TransformedDocumentContent
from sema4ai.di_client.document_processor_api.models.computed_document_content import ComputedDocumentContent
...
class PreprocessingSummary(BaseModel):
    ...
    charge_type_sections_identified: Optional[List[str]] = Field(default_factory=list)

class TableExtractionMetrics(BaseModel):
    tables_per_page: Optional[Dict[int, int]] = Field(default_factory=dict)
    empty_columns_dropped: Optional[List[str]] = Field(default_factory=list)
    columns_renamed: Optional[Dict[str, str]] = Field(default_factory=dict)

class ValidationCheckpoint(BaseModel):
    stage: Optional[str] = None
    checks_performed: Optional[List[str]] = Field(default_factory=list)
    checks_passed: Optional[List[str]] = Field(default_factory=list)
    checks_failed: Optional[List[str]] = Field(default_factory=list)
