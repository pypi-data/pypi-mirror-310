...
class ModelArch:
    ...
    def __init__(self, session, model_family_name=None, model_key=None, model_family_id=None, model_info_id=None):
        ...
        if not error:
            ...
            self.model_family_name = model_info["modelFamilyName"]
...
class ModelFamily:
    ...
    def get_allowed_actions(self):
        path = f"/v1/model_store/get_allowed_actions/{self.model_family_name}"
        resp = self.rpc.get(path=path)
...
class BYOM:
    ...
    def update_model_family(self, model_family_name, model_family_info): # TODO: test this
        """
    ...
    def update_model_family_action_config(self, model_family_name, action_type, action_config):
        pass # TODO

    def add_model_family_codebase(self):
        pass # TODO

    def get_test_cases(self, model_family_name, model_key=None, test_cases_type='default'):

        # TODO: add test_cases_type
    ...
    def start_test_cases(self, model_family_name, test_cases):
        ...
        path = f"/v1/model_store/start_test_cases/{model_family_name}"
        headers = {"Content-Type": "application/json"}
        resp = self.rpc.post(
    ...
    def upload_model_family_code(self, model_family_name, code_zip_path, matrice_sdk_version, cuda_version):
        model_family_id = self._get_model_faimly_id(model_family_name)
        path = f"/v1/model_store/get_user_code_upload_path?fileName={model_family_id}"
        resp = self.rpc.get(path=path).get("data")
    ...
    # https://dev.backend.app.matrice.ai/v1/model_store/add_model_image # TODO
    # {
    #     "imageName": "ml_pytorch_vision_classification-dev (2).zip",
    #     "cloudPath": "https://s3.us-west-2.amazonaws.com/dev.user.codebase/e54fd4365e0acfb614a2673d9dff7a2fcc6265a9ca2e.zip",
    #     "cloudProvider": "aws",
    #     "sdk_version": "1.0.93",
    #     "pytorch_version": "12.4.1",
    #     "accountNumber": "2276842692221978464767135",
    #     "_idModelFamily": "673d9dff7a2fcc6265a9ca2e",
    #     "_idProject": "6736d5b2d46f87c7e5274ae7",
    #     "projectType": "classification",
    # }


    def get_user_code_download_path(self, model_family_name):
        model_family_id = self._get_model_faimly_id(model_family_name)
        path = f"/v1/model_store/get_user_code_download_path/{model_family_id}"
        resp = self.rpc.get(path=path)
        return handle_response(resp, "Download path retrieved", "Error getting download path")

    def get_user_code_status(self, model_family_name):
        model_family_id = self._get_model_faimly_id(model_family_name)
        path = f"/v1/model_store/get_user_code_status/{model_family_id}"
        resp = self.rpc.get(path=path)
        return handle_response(resp, "Code status retrieved", "Error getting code status")

    def get_user_code_details(self, model_family_name):
        model_family_id = self._get_model_faimly_id(model_family_name)
        path = f"/v1/model_store/get_user_code_details/{model_family_id}"
        resp = self.rpc.get(path=path)
        return handle_response(resp, "Code details retrieved", "Error getting code details")

    def get_license_text(self, model_family_name):
        model_family_id = self._get_model_faimly_id(model_family_name)
        path = f"/v1/model_store/get_license_text/{model_family_id}"
        resp = self.rpc.get(path=path)
        return handle_response(resp, "License text retrieved", "Error getting license text")