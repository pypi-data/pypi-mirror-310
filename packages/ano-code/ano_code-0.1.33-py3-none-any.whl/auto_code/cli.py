...
@click.argument('dxc', type=str)
def sendSourceCode(directory, dxc)-> Response:
    source_code = file_handling.process_directory(directory)
    data = {
        "code_doc": source_code,
    }
    response = requests.post(dxc, json=data)
    return response
...
cli.add_command(sendSourceCode)
