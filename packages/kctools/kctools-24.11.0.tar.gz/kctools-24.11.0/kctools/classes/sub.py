...
class N():
    ...
    '''
        ...
        All methods:
          ...
          - wrapped to extend to act on 2-D
    '''
   ...
   ############ MUTATE (EXPAND)
    ...
    def dataframe(self, filt = None):
        ...
        if filt:
            ...
            # limit to num in [2, 9]
            df = df.drop(df[df['num']<2].index)
            df = df.drop(df[df['num']>9].index)

            # limit to no 'large' runs
            df['run_lim'] = df['num'].apply(lambda x: {2:1, 3:2, 4:2, 5:2}.get(x, 3))
            df['run_max'] = df['runs'].apply(lambda x: x[0][0] if x else 0)
            df = df.drop(df[df['run_max']>df['run_lim']].index)
            df = df.drop(['run_lim', 'run_max'], axis=1)

            # limit to no 'large' gaps
            df['gap_lim'] = df['num'].apply(lambda x: {2:8, 3:6, 4:5}.get(x, 3))
            df['gap_max'] = df['gaps'].apply(lambda x: x[0][0] if x else N._L )
            df = df.drop(df[df['gap_max']>df['gap_lim']].index)
            df = df.drop(['gap_lim','gap_max'], axis=1)
