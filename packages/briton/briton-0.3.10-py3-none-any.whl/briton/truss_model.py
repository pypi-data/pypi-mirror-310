...
class Model:
    def __init__(self, **kwargs):
        ...
        truss_trtllm_runtime_config = TrussTRTLLMRuntimeConfiguration(**(trtllm_config.get("runtime") or {}))
        ...
        self._kv_cache_free_gpu_mem_fraction = (
            truss_trtllm_runtime_config.kv_cache_free_gpu_mem_fraction
        )
        self._enable_kv_cache_reuse = (
            truss_trtllm_build_config.plugin_configuration.use_paged_context_fmha
        )
        self._enable_chunked_context = truss_trtllm_runtime_config.enable_chunked_context
        self._max_num_tokens = truss_trtllm_build_config.max_num_tokens
        self._max_seq_len = truss_trtllm_build_config.max_seq_len
        self._batch_scheduler_policy = batch_scheduler_policy_to_int(
            truss_trtllm_runtime_config.batch_scheduler_policy,
            logger=logger,
        )
        self._request_default_max_tokens = truss_trtllm_runtime_config.request_default_max_tokens
    ...
    async def predict(self, model_input: Dict[str, Any], request: fastapi.Request):
        ...
        if "max_tokens" not in model_input and self._request_default_max_tokens is not None:
            ...
            model_input["max_tokens"] = min(model_max_allowed_tokens, self._request_default_max_tokens)
