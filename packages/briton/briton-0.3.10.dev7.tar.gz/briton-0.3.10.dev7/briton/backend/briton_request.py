...
async def model_input_to_briton_request(
    ...
    add_schema_to_cache: Callable[[FsmCache], Awaitable[str]],
    ...
    # length_penalty
    if model_input.length_penalty is not None:
        request.length_penalty = model_input.length_penalty
    ...
    if model_input.stop is not None:
        request.stop_words = model_input.stop

    # bad words
    if model_input.bad_words is not None:
        request.bad_words = model_input.bad_words
...
def calc_request_output_len(
    ...
    num_max_tokens = model_input_max_tokens or default_max_tokens
    if num_max_tokens is None:
        return model_max_allowed_tokens

    if num_input_ids > num_max_tokens:
        raise HTTPException(
            status_code=400,
            detail=f"Number of input tokens {num_input_ids} exceeds max tokens {num_max_tokens}",
        )
    return min(model_max_allowed_tokens, num_max_tokens)
        ...
        if streaming:
            ...
            return create_completion_chunks(
                ...
                completion_tokens_fn=(
                    completion_tokens_fn if include_stream_usage else None
                ),
