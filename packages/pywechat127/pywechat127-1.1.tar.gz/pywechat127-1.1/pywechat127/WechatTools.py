...
class Tools():
    ...
    def move_window_to_center():
        '''该函数用来将已打开的微信界面移动到屏幕中央'''
        desktop=Desktop(**Independent_window.Desktop)
        screen_with,screen_height=win32api.GetSystemMetrics(win32con.SM_CXSCREEN),win32api.GetSystemMetrics(win32con.SM_CYSCREEN)
        ...
        new_left=(screen_with-window_width)//2
        new_top=(screen_height-window_height)//2
        win32gui.MoveWindow(win32gui.FindWindow('WeChatMainWndForPC','微信'),new_left,new_top,window_width,window_height,True)
            ...
            for _ in range(10):
                ...
                if friend_button:
                    if index:
                        ...
                        mouse.click(coords=(int(rec.left+rec.right)//2,rec.top+1))
                    else:
