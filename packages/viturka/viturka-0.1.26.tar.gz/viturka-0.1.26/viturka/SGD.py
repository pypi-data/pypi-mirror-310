...
def generate_synthetic_dataset(
    ...
    """
    Generate synthetic data efficiently with minimal RAM usage.
    ...
    Parameters:
        ...
        chunk_size (int): Number of synthetic samples to generate in each chunk.

    Returns:
        pd.DataFrame: Combined original and synthetic dataset.
    """
    ...
    for _ in range(0, num_synthetic_samples, chunk_size):
        # Generate a chunk
        current_chunk_size = min(chunk_size, num_synthetic_samples - len(synthetic_chunks) * chunk_size)

        # Randomly sample indices for the chunk
        sampled_indices = np.random.randint(0, len(df), size=current_chunk_size)

        # Numerical data with noise
        num_data = df[numerical_columns].iloc[sampled_indices].values
        noise = np.random.normal(0, noise_level, size=num_data.shape) * np.std(num_data, axis=0, keepdims=True)
        num_data = num_data + noise

        # Categorical data
        cat_data = {col: np.random.choice(df[col].unique(), size=current_chunk_size) for col in categorical_columns}

        # Target column
        target_data = df[target_column].iloc[sampled_indices].values
        if pd.api.types.is_numeric_dtype(df[target_column]):
            target_noise = np.random.normal(0, noise_level, size=current_chunk_size) * np.std(df[target_column])
            target_data = target_data + target_noise

        # Combine into a DataFrame
        chunk = pd.DataFrame(num_data, columns=numerical_columns)
        for col in categorical_columns:
            chunk[col] = cat_data[col]
        chunk[target_column] = target_data
