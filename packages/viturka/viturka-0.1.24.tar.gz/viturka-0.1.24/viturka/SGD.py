...
def generate_synthetic_dataset(
    ...
    chunk_size = int(num_synthetic_samples / 10)
    augmented_data = []
    for _ in range(10):
        synthetic_chunk = resample(df, n_samples=chunk_size, random_state=42, replace=True)

        # Add noise to numerical columns
        for col in numerical_columns:
            std_dev = df[col].std() if df[col].std() > 0 else noise_level
            synthetic_chunk[col] += np.random.normal(0, noise_level * std_dev, size=chunk_size)

        # Mix categorical values if enabled
        if mix_categorical and len(categorical_columns) > 1:
            ...
            sampled_combinations = np.random.choice(len(unique_combinations), size=chunk_size)
            for idx, col in enumerate(categorical_columns):
                synthetic_chunk[col] = [unique_combinations[i][idx] for i in sampled_combinations]

        augmented_data.append(synthetic_chunk)
