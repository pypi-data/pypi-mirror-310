...
def generate_synthetic_data(df, numerical_columns, categorical_columns, target_column, synthetic_size=2.0):
    """
    ...
    smote = SMOTE(sampling_strategy=synthetic_size, random_state=42)
...
def model(file, target_column, numerical_columns, categorical_columns, n_iter=100, rank=8, init_stdev=0.01, step_size= 0.001, l2_reg_w=0.1, l2_reg_V=0.1, test_size=0.2, generate_synthetic_data= False):
    ...
    if generate_synthetic_data:
        df = generate_synthetic_data(df, numerical_columns, categorical_columns, target_column)
