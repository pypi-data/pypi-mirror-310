...
from ai_term.flows.chat_agent import builder
from ai_term.utils.stream import stream_graph_values
from ai_term.utils.system import read_system_message, SystemPaths
from ai_term.utils.tools import tools
from ai_term.utils.visualize import visualize_graph
...
def main():
    ...
    parser.add_argument('--env', action='store_true', help='Create .env file in ~/.ai-lib/')
    args = parser.parse_args()

    if args.env:
        # Create ~/.ai-lib directory if it doesn't exist
        ai_lib_dir = os.path.expanduser("~/.ai-lib")
        os.makedirs(ai_lib_dir, exist_ok=True)
        
        # Copy .example.env to ~/.ai-lib/.env
        example_env_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), '.example.env')
        env_path = os.path.join(ai_lib_dir, ".env")
        shutil.copy2(example_env_path, env_path)
        sys.exit(0)

    # Load .env from ~/.ai-lib/.env
    env_path = os.path.expanduser("~/.ai-lib/.env")
