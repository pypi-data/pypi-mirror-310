...
from basecommand.api.utils import (
    get_storage_resource_ownership_query_param,
...
class DatasetAPI:
    ...
    def download(
        ...
        if storage_type == STORAGE_TYPE_OBJECT:
            ...
            owner_client_id_url_query = get_storage_resource_ownership_query_param(
                self.client, org_name, dataset_meta.creatorUserId
            )
            ...
            if files or dirs or exclude:
                if not dataset_meta or not hasattr(dataset_meta, "files") or not dataset_meta.files:
                ...
                download_files, download_size = get_download_files(
                    {f.path: f.fileSize for f in _files}, [d.path for d in _dirs], files, dirs, exclude
                )
            elif resume:
                try:
            ...
            if do_zip:
                ...
                if owner_client_id_url_query:
                    filenames_url = self.get_dataset_filenames_url(file_download_url)
                    try:
