...
class Configuration(metaclass=ParentClassSingleton):
    ...
    def get_ace_list(self):
        ...
        if self.app_key or (self.starfleet_kas_session_key and self.starfleet_kas_email):
            ...
            try:
                ace_choices = ace_api.get_aces(self.org_name)
            except (AuthenticationException, AccessDeniedException):
                ace_choices = []
...
class ConfigAPI:
    ...
    def get_aces(self, org_name):
        """Get list of ACEs. Given team name filters on basis of it."""

        query = f"{API_VERSION}/org/{org_name}/aces?page_size={PAGE_SIZE}"
        return chain(
            *[
                AceListResponse(res).aces
                for res in pagination_helper(self.connection, query, org_name=org_name, operation_name="get aces")
