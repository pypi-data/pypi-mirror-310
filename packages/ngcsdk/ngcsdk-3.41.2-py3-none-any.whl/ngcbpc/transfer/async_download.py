...
class AsyncDownload:
    ...
    async def _get_filenames_from_file(self, session, dest, filenames_url, params=None):
        ...
        while attempts <= NGC_CLI_DOWNLOAD_RETRIES:
            ...
            temp_path = get_incremented_filename(os.path.join(dest, f"{self.download_id}_filenames.json"))
            ...
            with TemporaryFileCreator(temp_name, dest) as temp_file:
                ...
                if attempts >= NGC_CLI_DOWNLOAD_RETRIES and not read_entire_content:
                    raise NgcException(f"Unable to download {filenames_url}. Reached maximum retries.") from None
                if read_entire_content:
                    try:
                        filenames = json.loads(open(temp_file, "r").read())
                    except (
                        OSError,
                        FileNotFoundError,
                        ValueError,
                        ...
                        raise NgcException(f"Unable to read {filenames_url}.") from None
                    break
            attempts += 1
    ...
    async def _download_files(
        ...
        if not files and filenames_url:
            ssl_context = ssl.create_default_context(cafile=certifi.where())
            connector = aiohttp.TCPConnector(ssl=ssl_context)
            async with aiohttp.ClientSession(connector=connector, trust_env=True) as session:
                self.files_to_download = await self._get_filenames_from_file(
                    session, dest, filenames_url, params=params
                )
