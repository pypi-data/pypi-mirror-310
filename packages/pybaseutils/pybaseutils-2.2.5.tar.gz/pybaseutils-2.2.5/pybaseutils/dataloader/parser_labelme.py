...
class LabelMeDataset(Dataset):
    ...
    @staticmethod
    def get_targets(data_info: dict, targets, keys=['points', 'boxes', 'labels', 'groups', 'names', 'keypoints']):
...
def draw_keypoints_image(image, boxes, keypoints):
    """绘制keypoints"""
    ...
    if len(keypoints) > 0:
        ...
        image = image_utils.draw_key_point_in_image(image, keypoints, pointline=target_bones["skeleton"],
                                                    colors=target_bones["colors"], thickness=2,
                                                    boxes=boxes, vis_id=False)
    return image


def show_target_image(image, boxes, labels, points, keypoints=[], color=(), thickness=1):
    ...
    image = draw_keypoints_image(image, boxes, keypoints)
