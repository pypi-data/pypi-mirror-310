...
class IdeGuiManager extends IdeZipManager {
  ...
  ideScreenModeResize(options={}){
    ...
    if(minLines===undefined){
      ...
      const nLines     = Math.max(1, Math.floor( availableH/lineH ))
...
class IdeSplitScreenManager extends IdeFullScreenManager {
  ...
  async switchSplitScreenFromButton(){
    ...
    this.switchSplitScreen(wasFullScreen)
  }
   ...
   * */
  switchSplitScreen(fromFullScreen=false){
    ...
    if(this.activeSplitScreen && !this.isSplitScreen){
      ...
      this.activeSplitScreen.switchSplitScreen()
    }
