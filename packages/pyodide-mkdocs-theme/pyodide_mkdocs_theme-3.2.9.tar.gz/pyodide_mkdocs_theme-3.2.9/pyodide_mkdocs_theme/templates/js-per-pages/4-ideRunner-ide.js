...
class _IdeEditorHandler extends IdeStorageAndZipManager {
  ...
  setupTopDivHeight(topDiv, header){
    topDiv ??= $("#pmt-top-div")
    if(!topDiv[0]) return;
    const headerH  = cssPx( header ?? $('body > header') )
    topDiv.css('height', `calc( 100vh - ${ headerH }px)`)
    this._ideScreenModeResizer(true)
  }

  windowResizeHandler(){
    this.setupTopDivHeight()
    this.terminalAutoWidthOnResize()
  }


  makeUpYourGui(){
    if($(this.globalIdH).is(':hidden')) return false    // Nothing to do yet (tabbed content).

    const todo = [
      this.fillInitAceGutter(),
      this.resizeVertTerm(),
    ]
   ...
   * */
  fillInitAceGutter(){
    if(this.guiIdeFlags.gutter) return true
    this._fillAceGutter(this.minIdeLines)
   ...
   * user is deleting lines (probably because they use "remove first match" kind of logic).
   * */
  _fillAceGutter(nLines){
    const gutter  = $(this.globalIdH).find('div.ace_gutter-layer')
    const model   = gutter.children().first().clone().removeClass('ace_gutter-active-line').addClass('pmt-gutter')
    const inners  = $('<span style="display: none;" tabindex="0"></span><span style="display: none;" tabindex="0"><span></span></span>')
    const height  = cssPx(model)
    const numbers = gutter.find('.pmt-gutter')
  ...
  setupAceEditor() {
    ...
    $(this.globalIdH).on('keyup', this.switchFullScreenFromKbd.bind(this))
  ...
  bindIdeButtons(){
    ...
    const glob = $(this.globalIdH)
    glob.find(".comment.tooltip").on("click", this.toggleComments.bind(this))
    glob.find(".ide-full-screen").on("click", this.switchFullScreenFromButton.bind(this))
    glob.find(".ide-split-screen").on("click", this.switchSplitScreenFromButton.bind(this))
  ...
  switchFullScreenFromKbd(event){
    if(event.key!='Escape') return;
    if(!document.fullscreenElement){
      $(this.globalIdH)[0].requestFullscreen().then(this.adaptFullScreen())
    }
  }
  switchFullScreenFromButton(){
    if(!document.fullscreenElement){
      this.storeInitTermHeightIfNeeded()
      $(this.globalIdH)[0].requestFullscreen().then(this.adaptFullScreen())
    }else{
  ...
  adaptFullScreen(){
    return async _=>{
      ...
      const splitScreenBtn = $(this.globalIdH).find(".ide-split-screen")
      ...
      while(document.fullscreenElement) await sleep(50)
      ...
      this._ideScreenModeResizer(false, minLines, maxLines)
  ...
  async switchSplitScreenFromButton(){
    const fromFullScreen = Boolean(document.fullscreenElement)
    ...
    if(fromFullScreen){
      ...
      // Everything is async with various delays, so wait for the internal flag instead of
      // document.fullscreenElement, so that the IDE can apply its logic when exiting it.
      // NOTE: Add an extra delay otherwise the glob dimension might be wrong at minLines
      //       computation time...
    ...
    this._splitScreenHandler(fromFullScreen)
  }


  /**When then current IDE is in neither split nor full screen, store the current height of the
   * terminal so that it can be restored when exiting from both modes.
   ...
   * */
  storeInitTermHeightIfNeeded(){
    if(!document.fullscreenElement && this !==_IdeEditorHandler.SPLITTED){
      this.guiIdeFlags.initTermH = cssPx( $(this.globalIdH).find('.term_wrapper') )
    }
  }


  /**Handle "split screen" logic, setting it "up" when the `#pmt-top-div` element doesn't
   * exist or if clicking/coming from full screen mode. Exit "split screen" mode otherwise.
   *
   * If the split screen mode is already activated for a different IDE, "exit" it then setup
   * split screen mode for the current instance.
   * */
  _splitScreenHandler(fromFullScreen=false){

    // Allow to switch from one IDE to another, staying in "split" mode:
    if(_IdeEditorHandler.SPLITTED && _IdeEditorHandler.SPLITTED!==this){
      jsLogger('[ScreenMode]', "Exit previous splitted")
      _IdeEditorHandler.SPLITTED._splitScreenHandler()
    }

    let topDiv = $(`#pmt-top-div`)
    if(!topDiv.length) topDiv = $(`<div id="pmt-top-div"></div>`)

    const wasSplit = this.guiIdeFlags.splittedLines
    const corrRems = $(this.solutionH)
    const header   = $('body > header')
    const glob     = $(this.globalIdH)
    const btn      = glob.find(".ide-split-screen")

    const page = $("div.md-container").detach()
    glob.detach()
      // Reminder: If ever css properties have to be computed, extract their values BEFORE
      //           any element gets detached from the DOM.

    if(!wasSplit || fromFullScreen){
      jsLogger('[ScreenMode]', "Setup split screen mode")

      // Reorganize DOM elements
      topDiv.insertAfter(header)
      topDiv.append(page, glob)
      this.setupTopDivHeight(topDiv, header)
      ...
      glob.addClass('split')
      _IdeEditorHandler.SPLITTED = this
      this.guiIdeFlags.splittedLines = this.editor.getOption('minLines')
      ;(corrRems.prev()[0] || corrRems.parent()[0]).scrollIntoView()

    }else{
      jsLogger('[ScreenMode]', "Teardown split screen mode")

      // Reorganize DOM elements
      page.insertAfter(header)
      glob.insertBefore(corrRems)
      topDiv.remove()

      // Resize ACE editor to default values/behaviors
      this._ideScreenModeResizer(false, this.minIdeLines, this.maxIdeLines)

      // Finalize states
      btn.addClass('deactivated')
      glob.removeClass('split')
      _IdeEditorHandler.SPLITTED = null
      this.guiIdeFlags.splittedLines = 0
      glob[0].scrollIntoView()
    }

    // Scroll down by the header's height, so that the extraction/insertion point is visible.
    // Adding 10px so that it ends up _below_ the header
    window.scrollBy(0, -(cssPx(header) + 10) )
    jsLogger('[ScreenMode]', "split handling - DONE")
  }



  /**Handle the ACE editor gutter display, enforcing minLines line umbers so that the editor
   * fills the desired height in `div.global_editor_xxx`.
   * May adapt the terminal height on the fly, if "required".
   * */
  _ideScreenModeResizer(setup, minLines, maxLines){

    const glob  = $(this.globalIdH)
    const term  = glob.find('.term_editor')
    let   termH = this.guiIdeFlags.initTermH

    if(minLines===undefined){
      const btns  = glob.find('.ide_buttons_div_wrapper')
      const globH = cssPx(glob)
      const btnsH = cssPx(btns) + cssPx(btns, 'margin-top') + cssPx(btns, 'margin-bottom')
      const lineH = cssPx(glob.find('div.ace_gutter-layer').children().first())
      // let   termH = cssPx(term)
      ...
      const availableH  = globH - termH - btnsH
      const nLines = Math.floor(availableH/lineH)
      /* DROPPED: it's causing a hell of a mess with window resize stuff, so just get rid of this!
      if(!this.isVert && setup){
        setTimeout(()=>{
          const boxGlob = glob[0].getBoundingClientRect()
          const boxBtns = btns[0].getBoundingClientRect()
          const extraH  = boxGlob.bottom - boxBtns.bottom
          term.css("height", termH + extraH + 'px')
        });
      }
      //*/
    ...
    this._fillAceGutter(minLines)
    this.editor.resize()

    term.css('resize', setup?'unset':'vertical')
    term.css("height", (setup?termH:this.guiIdeFlags.initTermH)+'px')

    jsLogger('[ScreenMode]', "IDE resizer done -", this.editor.getOption('minLines'), this.editor.getOption('maxLines'))
  }
