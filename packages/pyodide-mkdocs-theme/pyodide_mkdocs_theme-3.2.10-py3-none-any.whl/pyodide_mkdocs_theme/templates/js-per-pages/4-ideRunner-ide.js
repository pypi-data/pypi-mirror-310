...
class IdeGuiManager extends IdeZipManager {
  ...
  setupTopDivHeight(topDiv, header){
    topDiv ??= $("#pmt-top-div")
    if(!topDiv[0]) return;
    const headerH = ( header??$('body>header') ).css('height')
    topDiv.css('height', `calc( 100vh - ${ headerH })`)
    return cssPx(topDiv)
  }


  windowResizeHandler(){
    if(IdeGuiManager.SPLITTED===this){
      const topDivH = this.setupTopDivHeight()
      this._ideScreenModeResizer({topDivH})
    }
  ...
  _fillAceGutter(nLines){
    const gutter  = this.global.find('div.ace_gutter-layer')
  ...
  setupAceEditor() {
    ...
    this.global.on('keyup', this.switchFullScreenFromKbd.bind(this))
  ...
  switchFullScreenFromKbd(event){
    if(event.key=='Escape' && !document.fullscreenElement){
      this.global[0].requestFullscreen().then(this.adaptFullScreen())
    }
  }
  ...
  adaptFullScreen(){
    return async _=>{
      ...
      this._ideScreenModeResizer({setup:false, minLines, maxLines})
  ...
  async switchSplitScreenFromButton(){
    ...
    if(fromFullScreen){
      ...
      // Everything is async with various delays/things waiting on ea ch others, so wait for the
      // internal flag instead of document.fullscreenElement, so that the IDE can apply its logic
      // when exiting it.
      // NOTE: Add an extra delay otherwise glob's height might be wrong when computing minLines.
  ...
  storeInitHeightsIfNeeded(){
    if(!document.fullscreenElement && this !==IdeGuiManager.SPLITTED){
  ...
  _splitScreenHandler(fromFullScreen=false){
    ...
    if(IdeGuiManager.SPLITTED && IdeGuiManager.SPLITTED!==this){
      jsLogger('[ScreenMode]', "Exit previous splitted")
      IdeGuiManager.SPLITTED._splitScreenHandler()
    }

    const wasSplit = this.guiIdeFlags.splittedLines
    const setup    = !wasSplit || fromFullScreen
    ...
    const changes  = setup ? {
      ...
      rearrangeDom: ()=>{ topDiv.insertAfter(header)
                          topDiv.append(page, this.global)
    ...
    this._ideScreenModeResizer(changes.ideResizeArgs)
   ...
   * */
  _ideScreenModeResizer(options={}){
    let {setup, minLines, maxLines, topDivH} = {
      setup:true, topDivH:Infinity, ...options
    }
    ...
    if(minLines===undefined){
      ...
      if(!this.isVert && setup){
        setTimeout(()=>{
          const boxGlob = this.global[0].getBoundingClientRect()
...
class IdeRunner extends IdeGuiManager {
