...
class _CipherStream_Encryptor(_CipherStream_Base):
    def _Factory_CipherStream(self, encryption_key:bytes):
        _AssertNonce(self.headers.nonce)
...
class CrossCryptV2:
    ...
    def _GenerateNonce(self):
        ...
        '''
        nonce = _os.urandom(CrossCryptV2.Headers.DEF_NONCE_LENGTH)
        _AssertNonce(nonce)
        return nonce
    
def _AssertNonce(nonce:bytes):
    if not isinstance(nonce, bytes):
        raise TypeError("Bad Nonce: wrong datatype")
    if len(nonce) != CrossCryptV2.Headers.DEF_NONCE_LENGTH:
