...
def process_audio_files(access_token, folder_path, image_path, ruta_videos, aspect_ratio, prompt, seed_input, seed, continue_fragment):
    # Listar los archivos de audio ordenados
    file_list = sorted([f for f in os.listdir(folder_path) if f.endswith('.mp3')])

    if not continue_fragment:
        # Si `continue_fragment` es False, procesar solo el primer archivo
    ...
    for file_name in file_list:
        ...
        # Si `continue_fragment` es False, detenerse después del primer archivo
        if not continue_fragment:
            break
