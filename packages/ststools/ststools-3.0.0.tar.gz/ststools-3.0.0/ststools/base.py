...
def Grid_plot(interactive = False):
    ...
    if interactive:
        ...
        x_array, y_array, V_array, current_matrix = file[0], file[1], file[2], file[3]
        current_matrix = np.array(current_matrix, copy=True) * 1e9  # Convert current to nA

        # Output widget for heatmap
        output_heatmap = Output()
        ...
        # Calculate the current map, derivative map, gap map, doping map, and integral map
        ...
        def update_heatmap(*args):
            ...
            data = (current_map if map_type == 'Current' else
                    derivative_map if map_type == 'Derivative' else
                    gap_map if map_type == 'Gap' else
                    doping_map if map_type == 'Doping' else
                    integral_map)

            with output_heatmap:
                output_heatmap.clear_output(wait=True)
                # Explicitly set a larger plot size
                fig, ax = plt.subplots(figsize=(8, 5))  # Adjusted figsize for larger plot
                interp_method = 'nearest' if not interpolate_data else 'bilinear'
                cax = ax.imshow(data, cmap=cmap, origin='upper', interpolation=interp_method, vmin=vmin, vmax=vmax)
                fig.colorbar(cax, ax=ax, label=f'{map_type} Map')
                plt.title(f'{map_type} (V = {V_value:.3f} V)', fontsize=16)
        ...
        # Interactive widgets
        V_slider = FloatSlider(value=V_array[0], min=V_array.min(), max=V_array.max(), step=np.round(V_array[1] - V_array[0], 3), description='V (Potential)')
        ...
        V_max_slider = FloatSlider(value=V_array[-1], min=V_array.min(), max=V_array.max(), step=np.round(V_array[1] - V_array[0], 3), description='V Max')
        delta_slider = FloatSlider(value=0.1, min=0.01, max=1.0, step=0.01, description='Delta')
        ...
        map_selector = Dropdown(options=['Current', 'Derivative', 'Gap', 'Doping', 'Integral'], value='Current', description='Map Type')
        cmap_selector = Dropdown(options=plt.colormaps(), value='copper', description='Colormap')
        interpolate_checkbox = Checkbox(value=False, description='Interpolate Data')
        save_button = Button(description='Save Maps to ASC')
        ...
        # Arrange sliders and widgets vertically
        controls = VBox([V_slider, V_min_slider, V_max_slider, delta_slider, color_min_slider, color_max_slider,
                        map_selector, cmap_selector, interpolate_checkbox, save_button],
                        layout=Layout(width='600px'))

        # Combine the heatmap and controls in a horizontal layout
        layout = HBox([output_heatmap, controls], layout=Layout(align_items='flex-start', width='100%'))
