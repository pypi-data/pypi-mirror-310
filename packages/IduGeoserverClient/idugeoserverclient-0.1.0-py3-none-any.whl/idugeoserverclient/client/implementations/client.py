...
from src.idugeoserverclient.client.client_interface import IIduGeoserverClient
from src.idugeoserverclient.models.datastore_model import DatastoreModel
from src.idugeoserverclient.models.layer_model import LayerModel
...
class IduGeoserverClient(IIduGeoserverClient):
    ...
    async def get_layers(self, workspace_name: str, layer_type: str | None = None, *args) -> list[LayerModel]:
        ...
        if layer_type:
            data = list(filter(
                lambda v: match(
                    f".*_{layer_type}{''.join([f'_{str(arg)}' for arg in args])}", v["name"]
                ), data))
    ...
    async def get_datastores(
        ...
        if datastore_type:
            data = list(filter(
                lambda v: match(
                    f".*_{datastore_type}{''.join([f'_{str(arg)}' for arg in args])}", v["name"]
                ), data))
