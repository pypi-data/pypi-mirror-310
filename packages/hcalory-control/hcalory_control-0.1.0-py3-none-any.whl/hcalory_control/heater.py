...
class HeaterState(int, ListableEnum):
    ...
    cooldown = 65
    cooldown_what = 67
    cooldown_something = 69
    igniting = 128
    igniting_something_else = 129
    something = 131  # ??????
    running = 133
    ...
    def running(self) -> bool:
        match self.heater_state:
            case (
                ...
                | HeaterState.cooldown_something
                | HeaterState.error
            ):
                return False
            case (
                HeaterState.igniting
                | HeaterState.something
                | HeaterState.running
                | HeaterState.idle
            ):
                return True
...
class HCaloryHeater:
    ...
    def handle_disconnect(self, _: bleak.BleakClient) -> None:
        ...
        if not self._intentional_disconnect:
            asyncio.get_event_loop().create_task(self.connect(self.device))
    ...
    async def send_command(self, command: Command):
        logger.debug(
            "Sending %s to characteristic %s of heater %s",
            command.name,
