...
async def client(server):
    ...
    api = asyncopenapi3.OpenAPI(data)
    return api
...
def randomPet(name=None):
    return {
        "data": {
            "pet": {"name": str(name) if name else random.choice(["dog", "cat", "mouse", "eagle"])}
        }
    }
...
async def test_getPetUnexpectedResponse(server, client):
    ...
    async with client:
        with pytest.raises(
            asyncopenapi3.UnexpectedResponseError,
