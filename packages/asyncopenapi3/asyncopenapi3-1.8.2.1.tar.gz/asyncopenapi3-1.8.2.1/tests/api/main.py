...
@app.post(
    ...
    response_model=Pet,
    responses={201: {"model": Pet}, 409: {"model": Error}},
)
def createPet(
    response: Response,
...
@app.get("/pet", operation_id="listPet", response_model=Pets)
def listPet(limit: int | None = None) -> Pets:
    return list(ZOO.values())


@app.get(
    "/pets/{pet_id}", operation_id="getPet", response_model=Pet, responses={404: {"model": Error}}
)
...
@app.delete(
    ...
    responses={204: {"model": None}, 404: {"model": Error}},
)
def deletePet(response: Response, pet_id: int = Path(..., alias="pet_id")) -> Pets:
    for k, v in ZOO.items():
        if pet_id == v.id:
            del ZOO[k]
