...
from asyncopenapi3.general import Reference
from asyncopenapi3.object_base import ReferenceProxy
from asyncopenapi3.schemas import Schema
...
def test_ref_resolution(petstore_expanded_spec):
    ...
    """
    ref = (
        petstore_expanded_spec.paths["/pets"]
...
def test_openapi_3_1_0_references(with_openapi_310_references):
    ...
    example_ref = (
        spec.components.pathItems["example"].get.responses["200"].content["application/json"]
    )
...
def test_reference_referencing_reference(with_reference_referencing_reference):
    ...
    assert isinstance(spec.components.schemas["Example"].properties["real"], Schema), (
        "Real property was not a schema?"
    )
    assert isinstance(spec.components.schemas["Example"].properties["reference"], Schema), (
        "Reference property was not resolved"
    )
    assert isinstance(
        spec.paths["/test"]
        ...
        Schema,
    ), "Reference reference was not resolved"
