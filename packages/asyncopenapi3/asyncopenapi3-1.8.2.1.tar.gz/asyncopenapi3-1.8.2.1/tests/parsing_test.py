...
from asyncopenapi3 import OpenAPI, ReferenceResolutionError, SpecError
...
def test_parsing_fails(broken):
    ...
    with pytest.raises(
        SpecError,
        match=r"Expected .info to be of type Info, with required fields \['title', 'version'\]",
    ):
        spec = OpenAPI(broken)
...
def test_parsing_float_validation(float_validation_expanded):
    ...
    properties = (
        spec.paths["/foo"].get.responses["200"].content["application/json"].schema.properties
    )
...
def test_parsing_broken_links(with_broken_links):
    ...
    assert sorted([
        ...
        "one of operationId or operationRef must be specified",
    ]) == sorted(error_strs)
