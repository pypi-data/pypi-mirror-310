...
def raise_on_unknown_type(parent, field, object_types, found):
    ...
    if len(object_types) == 1:
        if isinstance(object_types[0], str):
            ...
            raise SpecError(
                f"Expected {parent.get_path()}.{field} to be of type {object_types[0]}, with required fields {expected_type.required_fields}",
                path=parent.path,
                element=parent,
            )
    elif (
        ...
        raise SpecError(
            f"Expected {parent.get_path()}.{field} to be of type {expected_type_str} or Reference, but did not find required fields {expected_type.required_fields} or '$ref'",
            path=parent.path,
            element=parent,
        )
    raise SpecError(
...
class ObjectBase:
    ...
    def _get(self, field, object_types, is_list=False, is_map=False):
        ...
        try:
            if not isinstance(object_types, (tuple, list)):
                # maybe don't accept not-lists
                object_types = [object_types]

            elif "*" in object_types and len(object_types) > 1:
    ...
    @staticmethod
    def key_contained(key, target_list):
        """
    ...
    def get_object_type(cls, typename):
        ...
        if not hasattr(cls, "_subclass_map"):
            ...
            cls._subclass_map = {t.__name__: t for t in cls.__subclasses__()}

        # TODO - why?
        if typename not in cls._subclass_map:  # pylint: disable=no-member
            raise ValueError(f"ObjectBase has no subclass {typename}")
    ...
    def parse_list(self, raw_list, object_types, field=None):
        ...
        python_types = [
            ObjectBase.get_object_type(t) if isinstance(t, str) else t for t in object_types
        ]
        ...
        for i, cur in enumerate(raw_list):
            ...
            if not found_type:
                raise SpecError(
                    "Could not parse {}.{}, expected to be one of [{}]".format(
            ...
            if isinstance(value, reference_type):
                ...
                if not reference_path.startswith("#/"):
                    raise ReferenceResolutionError(
                        f"Invalid reference path {reference_path}",
                        path=self.path,
                        element=self,
                    )
