    ...
    for c in instance.parameters:
        if c.in_ == "path":
            if c.name not in allowed_path_parameters:
                raise SpecError(f"Parameter name not found in path: {c.name}", path=instance.path)
...
class Parameter(ObjectBase):
    ...
    def _parse_data(self):
        ...
        self.description = self._get("description", str)
        self.example = self._get(
            "example", [str, int, bool, float]
        )  # Spec notes 'Any' but just limited to primitives
...
class Operation(ObjectBase):
    ...
    def _request_handle_parameters(self, base_url=None, parameters=None, data=""):
        # Parameters
        ...
        for name, spec in accepted_parameters.items():
            try:
                ...
                if spec.required and name not in parameters:
                    err_msg = f"Required parameter {name} not provided"
        ...
        return (self.path[-1], base_url + self.path[-2].format(**path_parameters)), dict(
            content=data, headers=self._headers, params=self._params, **self._aux
        )
    ...
    def _request_prepare(
        self, base_url, security=None, data=None, parameters=None, session=None, raw_response=False
    ):
        if session is None:
...
class SecurityRequirement(ObjectBase):
    ...
    def _parse_data(self):
        ...
        if len(self.raw_element.keys()) == 1:
            self.name = next(iter(self.raw_element.keys()))
    ...
    def can_parse(cls, dct):
        """
        This needs to ignore can_parse since the objects it's parsing are
        not regular - they must always have only one key though or be empty
        for Optional Security Requirements
        """
        return (
            len(dct.keys()) == 1
            and isinstance(next(iter(dct.values())), list)
            or len(dct.keys()) == 0
        )
...
class Link(ObjectBase):
    ...
    def _parse_data(self):
        ...
        if self.operationId and self.operationRef:
            raise SpecError("operationId and operationRef are mutually exclusive")
        if not (self.operationId or self.operationRef):
            raise SpecError("one of operationId or operationRef must be specified")
